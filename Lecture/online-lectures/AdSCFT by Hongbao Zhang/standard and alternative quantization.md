...
1. $\displaystyle{K^{2}< 0}$, $\displaystyle{K}$ is timelike. then we require $\displaystyle{\lambda=i\sqrt{ -K^{2} }}$ for positive frequency solution and $\displaystyle{\lambda=-i\sqrt{ -K^{2} }}$ for negative frequency solution, ingoing boundary condition.
