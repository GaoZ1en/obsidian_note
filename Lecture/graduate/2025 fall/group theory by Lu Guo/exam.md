...
\end{align}$$
2. $\displaystyle{D_{4}}$群
    1. 写出一组生成元和$\displaystyle{D_{4}}$的秩
    2. 写出所有不变子群和类，判断每个类是否是自逆类
    3. 构造出$\displaystyle{D_{4}}$群的所有$\displaystyle{I}$型非固有点群和$\displaystyle{P}$型非固有点群
*sol*.
...
4. 将$\displaystyle{D_{3}}$群元素看作二维空间中点的坐标变换，$\displaystyle{\begin{pmatrix}x' \\ y'\end{pmatrix}=R\begin{pmatrix}x\\y\end{pmatrix}}$，$\displaystyle{R\in D_{3}}$。取生成元$\displaystyle{D,A}$，其对应的变换矩阵为$\displaystyle{D=\frac{1}{2}\begin{pmatrix}-1 & -\sqrt{ 3 }\\\sqrt{ 3 } & -1\end{pmatrix},A=\begin{pmatrix}1 & 0 \\ 0 & -1\end{pmatrix}}$. 已知$\displaystyle{\psi _{1}(x,y)=x^{2}-y^{2},\psi _{2}(x,y)=2xy,\psi _{3}(x,y)=x^{2}+y^{2}}$架设的三维函数空间对$\displaystyle{D_{3}}$群保持不变。
    ...
    2. 将此表示约化为$\displaystyle{D_{3}}$群不可约表示的直和
    3. 将函数集重新组合为分属各不等价不可约表示的函数集
*sol*.
- we have
$$\begin{align}
...
4. 对置换群$\displaystyle{S_{5}}$
    1. 写出杨图$\displaystyle{[2,2,1]}$对应的最大正则杨表的杨算符（无需展开乘积）
    2. 计算元素$\displaystyle{(2,3)}$在表示$\displaystyle{[2,2,1]}$中的表示矩阵
    3. 写出$\displaystyle{(2,3)}$在表示$\displaystyle{[2,2,1]}$中的表示矩阵的实正交形式
*sol*.
...
2. $\displaystyle{S_{6}}$群的不可约表示$\displaystyle{[3,3]}$，作为子群$\displaystyle{S_{3}\otimes S_{3}}$的分导表示，按子群不可约表示约化，并验证维数
...
3. 求转动矩阵$\displaystyle{R(\alpha,\beta,\gamma)=\begin{pmatrix}-\frac{1}{2}&-\frac{\sqrt{ 3 }}{2} & 0 \\ -\frac{\sqrt{ 3 }}{4} & \frac{1}{4} & \frac{\sqrt{ 3 }}{2} \\ -\frac{3}{4} & \frac{\sqrt{ 3 }}{4} & -\frac{1}{2} \end{pmatrix}}$的欧拉角（先转z轴，再转y轴，最后转z轴）
...
4. 写出绕$\displaystyle{\hat{n}=\frac{\sqrt{ 6 }}{4}\hat{e}_{1}+\frac{\sqrt{ 2 }}{4}\hat{e}_{2}+\frac{\sqrt{ 2 }}{2}\hat{e}_{3}}$方向转$\displaystyle{\frac{\pi}{3}}$角的$\displaystyle{\mathrm{SU}(2)}$群元素的具体形式
*sol*. we have
$$\begin{align}
...
\end{align}$$
5. 由球谐函数$\displaystyle{Y^{1/2}_{-1/2}(\hat{n}),Y^{1/2}_{1/2}(\hat{n})}$组合出沿$\displaystyle{\hat{n}=\frac{\sqrt{ 3 }}{2}\hat{e}_{2}-\frac{1}{2}\hat{e}_{3}}$方向轨道角动量的本征值为$\displaystyle{-\frac{1}{2}, \frac{1}{2}}$的本征函数
...
1. 简答题
    1. 简述群的重排定理
    2. 写出用$\displaystyle{D_{3}}$群构造出的$\displaystyle{I}$型和$\displaystyle{P}$型非固有点群的所有元素
    3. 特征标表填空
    4. 写出$\displaystyle{\mathrm{SU}(2)}$群绕$\displaystyle{\hat{n}=\frac{1}{\sqrt{ 2 }}(\hat{e}_{1}+\hat{e}_{2})}$方向，转角$\displaystyle{\omega=\frac{\pi}{3}}$矩阵的具体形式
    5. 写出$\displaystyle{S_{6}}$的不可约表示$\displaystyle{[4,2]}$，关于子群$\displaystyle{S_{3}\otimes S_{3}}$的分导表示按不可约表示约化，并验证维数
2. some easy problem
3. just like exam 1 Q4
4. $\displaystyle{S_{4}}$群不可约表示$\displaystyle{[3,1]}$
    1. 写出最大杨表对应的杨算符，结果用无公共客体轮换来表示
    2. 将元素$\displaystyle{(24)}$写成相邻客体对换乘积形式，并求出在正交表示下，$\displaystyle{(24)}$的矩阵表示
5. 给了$\displaystyle{\mathrm{SO}(3)}$群$\displaystyle{d^{1}(\beta)}$矩阵形式，用$\displaystyle{Y^{1}_{1}(\hat{n}),Y^{1}_{0}(\hat{n}),Y^{1}_{-1}(\hat{n})}$线性组合出沿$\displaystyle{\hat{r}=\frac{3}{4}\hat{e}_{1}+\frac{\sqrt{ 3 }}{4}\hat{e}_{2}-\frac{1}{2}\hat{e}_{3}}$方向、本征值为$\displaystyle{1,0,-1}$的本征函数
