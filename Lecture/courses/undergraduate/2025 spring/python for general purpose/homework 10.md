...
1. 数据集：
	1. 使用 statsmodels.datasets 中的 boston 数据集。这个数据集包含以下字段（部分）：
		1. CRIM: 城镇人均犯罪率
		1. ZN: 划为超过 25,000 平方英尺居住用地的比例
		1. INDUS: 每个城镇非零售业务的比例
		1. CHAS: 是否靠近查尔斯河 (1 如果靠近；0 否则)
		1. NOX: 一氧化氮浓度 (每千万份)
		1. RM: 每户住宅的平均房间数
		1. AGE: 1940 年以前建造的自有住房单位的比例
		1. DIS: 到波士顿五个就业中心的加权距离
		1. RAD: 径向公路的可达性指数
		1. TAX: 每 10,000 美元的全额物业税率
		1. PTRATIO: 城镇的师生比例
		1. B: 1000(B_k−0.63)^2 ，其中 Bk 是城镇中黑人的比例
		1. LSTAT: 人口中地位较低人群的百分比
		1. MEDV: 自有住房的中位数估值（以千美元计）
1. 任务：
	1. 加载数据：
		1. 使用 statsmodels.api 加载 boston 数据集。
		1. 将数据集转换为 Pandas DataFrame，包含特征（data）和目标变量（target，即 MEDV）。
		1. 为 DataFrame 添加列名，列名可以从 boston.feature_names 获取，目标变量的列名设置为 ’MEDV’。
	1. 数据探索与预处理：
		1. 查看 DataFrame 的基本信息（如 head(), info(), describe()）。
		1. 检查是否存在缺失值。如果存在，请考虑如何处理（例如，删除包含缺失值的行或使用均值/中位数填充，你需要根据实际情况判断哪种方法更合适，这里假设数据集中没有缺失值）。
		1. 绘制犯罪率 (CRIM) 和房价 (MEDV) 的散点图，初步观察它们之间的关系。
		1. 计算犯罪率 (CRIM) 和房价 (MEDV) 的相关系数。
	1. 构建线性回归模型：
	...
	1. 模型分析：
		...
		1. 根据模型的斜率和 p-value，解释犯罪率对房价的影响方向和统计显著性。
	1. 拓展（可选）：
		1. 尝试使用其他特征（例如 RM，每户住宅的平均房间数）作为自变量构建新的线性回归模型，并比较模型的结果。
		1. 构建包含多个自变量（例如 CRIM 和 RM）的多元线性回归模型，并分析结果。
	1. 提交内容：
		1. 加载数据并创建 Pandas DataFrame 的代码
		1. 数据探索和预处理的代码以及相应的解释
		1. 构建和拟合线性回归模型的代码
		1. 模型摘要信息的输出以及对模型参数的解释
		1. （如果完成拓展任务）拓展任务的代码和结果分析
1. 提示：
	1. 确保你的代码有清晰的注释。
	1. 对于模型结果的解释，要结合实际意义进行分析。
1. # 完整的Python代码：波士顿房价与犯罪率分析
...
1. crime_regression.png - 展示线性回归拟合结果
1. model_comparison.png - 比较三个不同模型的预测效果
