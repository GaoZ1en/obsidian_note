...
1. The Lagrangian density for a complex scalar field $\displaystyle{\phi(x)}$ is
...
1. A real scalar field has a a Lagrangian density
...
1. The Lagrangian density for the electromagnetic 4-vector potential $\displaystyle{A_{\mu}(x)}$ is
