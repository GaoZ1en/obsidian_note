...
1. **Schwarzschild Geodesics**
    ...
    1. Reduce the geodesic equation to an effective potential problem: $\frac{1}{2}\dot{r}^2 + V_{eff}(r) = \frac{E^2-m^2}{2m}$. Explicitly write out $V_{eff}(r)$.
    1. Determine the radius of the Innermost Stable Circular Orbit (ISCO).
    1. Calculate the radius of the photon sphere (unstable circular orbit for massless particles).
*sol*.
- we have
...
1. **Cosmology**
    ...
    1. Discuss the cases $w=0$ (dust), $w=1/3$ (radiation), and $w=-1$ (vacuum energy).
    1. Calculate the particle horizon distance $d_{H}(t) = a(t) \int_0^t \frac{dt'}{a(t')}$ for the case $w > -1/3$. Does the horizon exist?
*sol*.
- we have
...
1. **Black Hole Thermodynamics**
    ...
    1. Calculate the surface gravity $\kappa$ at the event horizon.
    1. Discuss why this system is generally not in thermodynamic equilibrium (compare temperatures at $r_h$ and $r_c$).
*sol*.
...
1. **Linearized Gravity & Gravitational Waves**
    ...
    1. For a binary system with mass distribution $T_{00} \approx \rho(t, \mathbf{x})$, derive the quadrupole approximation for the spatial perturbation $\bar{h}_{ij}$.
    1. Using the quadrupole formula for radiated power $P = \frac{G}{5} \langle \dddot{Q}_{ij} \dddot{Q}^{ij} \rangle$, estimate the decay rate of the orbital period $\dot{P}$ for a circular binary.
*sol*.
...
1. **Kerr Black Hole Geometry**
    ...
    1. Define the Ergosphere limits. Explain physically what happens to a timelike observer inside the Ergosphere.
    1. Calculate the angular velocity of the "zero angular momentum observer" (ZAMO), $\omega = -g_{t\phi}/g_{\phi\phi}$.
    1. Verify that at the horizon, this matches the horizon angular velocity $\Omega_H$.
    1. **Penrose Process**: Show that energy extraction is possible if a particle decays into two particles inside the Ergosphere, where one particle has negative energy $E < 0$. What is the condition on the Killing vector $\xi^\mu_{(t)}$ for this to happen?
*sol*.
...
1. **ADM Formalism**
    ...
    1. Write the definition of Extrinsic Curvature $K_{ij}$ in terms of the Lie derivative of the spatial metric.
    1. State the Hamiltonian constraint ($G_{00}$ component) and Momentum constraint ($G_{0i}$ component) in terms of $^{(3)}R$ and $K_{ij}$.
*sol*.
...
1. **Penrose Diagrams & Causal Structure**
    1. Draw the Penrose diagram for an Extremal Reissner-Nordström black hole ($|Q|=M$).
    1. Label the horizon, singularity, and conformal infinity.
    1. Is the singularity spacelike, timelike, or null? Can an observer avoid the singularity after crossing the horizon?
*sol*.
...
1. **Raychaudhuri Equation**
    ...
    1. Assuming the Null Energy Condition (NEC) holds, and the congruence is hypersurface orthogonal ($\omega=0$), prove the Focusing Theorem: if $\theta_0 < 0$, $\theta \to -\infty$ in finite affine parameter.
    1. Briefly define a "Trapped Surface" using the expansions of ingoing and outgoing null geodesics.
*sol*.
...
1. **Vaidya Metric (Dynamic Black Hole)**
    ...
    1. Show that $l_\mu = \partial_\mu v$ is a null vector field. Is it geodesic?
    1. Calculate the Einstein tensor component $G_{vv}$ (or argue its form) to relate it to the energy flux of the null dust.
*sol*.
- **Inverse Metric**:
...
1. **Field Theory in Curved Spacetime**
...
1. **Killing Tensors & Hidden Symmetries**
    1. Define a Rank-2 Killing Tensor $K_{\mu\nu}$.
    1. Prove that if $K_{\mu\nu}$ is a Killing tensor and $u^\mu$ is a geodesic tangent vector, then $Q = K_{\mu\nu}u^\mu u^\nu$ is conserved along the geodesic.
    1. Discuss the significance of the Carter Constant in the separability of the Hamilton-Jacobi equation for the Kerr metric.
*sol*.
...
1.  **Photon Sphere & Gravitational Capture**
    ...
    1.  For a photon ($m=0$), find the radius of the circular orbit (Photon Sphere) where $V'_{eff}(r) = 0$. Is it stable?
    1.  Calculate the impact parameter $b = |L/E|$ for a photon to be barely captured by the black hole.
*sol*.
- the
    *sol*.
    - **Effective Potential**:
...
1.  **Lie Derivatives & Symmetries**
    ...
    1.  For Minkowski space in polar coordinates ($ds^2 = -dt^2 + dr^2 + r^2 d\phi^2$), check if $\xi = \partial_\phi$ is a Killing vector by explicit calculation.
    1.  Is $\zeta = r \partial_r$ a conformal Killing vector? (i.e., $\mathcal{L}_\zeta g_{\mu\nu} = \lambda(x) g_{\mu\nu}$).
    *sol*.
...
1.  **Thermodynamics & Surface Gravity**
    ...
    1.  For a generic static metric $ds^2 = -f(r)dt^2 + \frac{dr^2}{f(r)} + r^2 d\Omega^2$, derive the formula $\kappa = \frac{1}{2}f'(r_h)$.
    1.  Calculate the Hawking temperature $T_H$ for a Schwarzschild black hole ($f=1-2M/r$).
    *sol*.
...
1.  **Cosmology: Energy Conservation**
    In an FRW universe, the energy-momentum tensor is $T^{\mu\nu} = (\rho+p)u^\mu u^\nu + p g^{\mu\nu}$.
    1.  Using $\nabla_\mu T^{\mu\nu} = 0$, derive the continuity equation $\dot{\rho} + 3H(\rho+p) = 0$.
    1.  For radiation ($p = \rho/3$), how does $\rho$ scale with scale factor $a(t)$?
    1.  For a cosmological constant ($p = -\rho$), solve for $\rho(a)$.
    *sol*.
    - **Continuity Eq**:
...
1.  **Linearized Gravity & Gauges**
    Metric $g_{\mu\nu} = \eta_{\mu\nu} + h_{\mu\nu}$ with $|h| \ll 1$.
    1.  How does $h_{\mu\nu}$ transform under a coordinate transformation $x^\mu \to x^\mu + \xi^\mu$?
    1.  Define the Transverse Traceless (TT) gauge.
    1.  Given a wave propagating in $z$-direction, write the general form of $h_{ij}^{TT}$.
    *sol*.
    - **Transformation**: $h'_{\mu\nu} = h_{\mu\nu} - \partial_\mu \xi_\nu - \partial_\nu \xi_\mu$.
    - **TT Gauge**:
      1. $h_{0\mu} = 0$ (Spatial).
      1. $\partial^j h_{ij} = 0$ (Transverse).
      1. $\eta^{ij} h_{ij} = 0$ (Traceless).
    - **Form**:
...
1.  **Friedmann Equations & Background Dynamics**
    ...
    1.  Define the Critical Density $\rho_c$ and the Density Parameter $\Omega$. Write the Friedmann equation in terms of $\Omega$ and curvature density $\Omega_k$.
    1.  Show that for a static universe ($\dot{a}=\ddot{a}=0$), one requires $\rho + 3p < 0$ (if $\Lambda=0$) or a Cosmological Constant (if $\rho>0, p\approx 0$), motivating Einstein's introduction of $\Lambda$.
    *sol*
...
1.  **Distance Measures & Redshift**
    1.  Define the redshift $z$ in terms of the scale factor $a(t)$ (assuming $a(t_0)=1$).
    1.  Define **Luminosity Distance** $d_L$ and **Angular Diameter Distance** $d_A$.
    1.  Prove the reciprocity relation $d_L = (1+z)^2 d_A$. (Or just argue using surface brightness conservation/Liouville theorem).
    1.  In a flat universe dominated by a single component with equation of state $w$ ($p=w\rho$), how does the Particle Horizon $d_H(t) \propto \int_0^t dt'/a(t')$ scale with time? Condition on $w$ for the horizon to exist (be finite)?
    *sol*.
...
1.  **Cosmological Problems & Inflation**
    1.  Briefly explain the **Flatness Problem**. Why is $\Omega=1$ an unstable fixed point in standard cosmology?
    1.  Briefly explain the **Horizon Problem**. Why is the CMB isotropic?
    1.  How does Inflation (a period of $\ddot{a} > 0$) solve these problems? Use the behavior of the comoving Hubble radius $(aH)^{-1}$.
    *sol*.
    - **Flatness Problem**:
