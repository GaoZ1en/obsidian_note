...
-   Relations:
    ...
    1.  Electron ends: $2I_e + E_e = 2 \cdot V$.
    1.  Loops: $L = (I_\gamma + I_e) - V + 1$.
...
1.  **Order**: $\text{adjoint} \cdot \text{propagators/vertices} \cdot \text{spinor}$.
1.  **Loops**: Multiply by $(-1)$ for each closed fermion loop. Take trace of the Dirac indices in the loop.
1.  **Sign**: Relative minus sign between diagrams that differ by an exchange of identical external fermions.
