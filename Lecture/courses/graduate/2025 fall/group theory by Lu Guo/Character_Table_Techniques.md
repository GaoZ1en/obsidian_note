...
2.  Form the Quotient Group $Q = G/N$. Even if $Q$ is small (like $C_2$), utilize its table.
3.  **Lift**: Copy the character of $\bar{g} \in Q$ to all elements $g \in G$ that map to $\bar{g}$.
...
2.  Count fixed points for each class to get $\chi_{perm}$.
3.  **Reduce**: We know $\Gamma_{perm}$ always contains the trivial representation $A_1$ (sum of elements is invariant).
...
1.  **Fill $A_1$**: All 1s.
2.  **Find Quotient $G/N$**: Lift its chars to get $A_2, B_1, B_2$ etc. (Look for $1, 1, \dots, -1, -1$).
3.  **Count Fixed Points**: Find a permutation rep (dim $>$ 1) and subtract $A_1$ to find $E$ or $T$.
4.  **Orthogonality Check**: Only use $\sum \chi_i \chi_j^* = 0$ as a final sanity check or to solve for a single missing variable.
