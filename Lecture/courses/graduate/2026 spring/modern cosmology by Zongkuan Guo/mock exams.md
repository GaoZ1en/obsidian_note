...
1. `FRW 背景`（20 分）
   ...
   1. 计算 $\Gamma^1_{01}$ 和 $\Gamma^0_{11}$。
   1. 写出理想流体能动张量，并推出连续性方程。

1. `热历史`（20 分）
   1. 推导非相对论极限下的粒子数密度。
   1. 推导复合时期的 Saha 方程。
   1. 解释为什么复合温度远小于 $13.6\,\mathrm{eV}$。

1. `结构形成`（20 分）
   1. 写出从原初曲率扰动功率谱 $P_{\mathrm{ini}}(k)$ 到星系功率谱 $P_{gg}(k,z)$ 的关系。
   1. 说明在辐射主导时期进入视界的 CDM 扰动和辐射扰动分别如何演化。

1. `观测与 CMB`（20 分）
   1. 简述 CMB 原初各向异性的四种基本来源。
   1. 简述 AP 效应的原理。
   1. 简述 SNIa 如何测 $D_L$、BAO 如何测 $D_A$。

1. `Inflation`（20 分）
   1. 说明标准大爆炸中的视界问题。
   1. 解释 inflation 为什么能解决该问题。
...
1. `观测与 CMB`
   ...
   1. AP 效应本质是：若用错误宇宙学把 $(\Delta\theta,\Delta z)$ 转成三维距离，则本征各向同性结构会表现出表观各向异性。横向与径向距离分别是
...
1. `背景与距离`（25 分）
   ...
   1. 给出 $D_L(z)$ 在低红移的 $z^3$ 阶展开。
   1. 解释 $D_L=(1+z)^2D_A$ 的来源。

1. `数值题：平直 $\Lambda$CDM`（20 分）
...
1. `线性扰动`（20 分）
   1. 写出带压强流体的牛顿线性扰动方程。
   1. 在辐射主导长波极限下求 $\delta\propto t^n$ 的指数 $n$。
   1. 写出 Jeans 波长和 Jeans 质量。

1. `早期宇宙`（15 分）
   1. 定义 e-fold 数 $N$。
   1. 写出单场 slow-roll inflation 下的两条基本方程。
...
\end{align}$$
   ...
   1. 因为光子能量红移给一因子 $1/(1+z)$，到达率时间膨胀再给一因子 $1/(1+z)$，所以
...
\end{align}$$
   ...
   1. 中微子先退耦，随后 $e^\pm$ 湮灭只加热光子。由光子-$e^\pm$ 热浴共动熵守恒得
...
1. `标量场宇宙学`（20 分）
   1. 写出最小耦合标量场的作用量。
   1. 写出其能量密度和压强。
   1. 说明为什么 $\dot\phi^2\ll V(\phi)$ 时会导致加速膨胀。

1. `复合与 BBN`（20 分）
   1. 解释“复合”和“光子退耦”为什么不是同一件事。
   1. 由 $n/p$ 比估算 $^4\mathrm{He}$ 质量丰度 $Y_p$。

1. `功率谱形状`（20 分）
   1. 解释 $k_{\mathrm{eq}}$ 的物理意义。
   1. 说明 $P_{mm}(k)$ 在 $k\ll k_{\mathrm{eq}}$ 和 $k\gg k_{\mathrm{eq}}$ 的渐近行为。
...
1. `CMB`
   ...
   1. Silk 阻尼是由于复合前光子有有限平均自由程，会发生扩散，把小尺度各向异性抹平，因此高 $\ell$ 功率下降。
...
\end{align}$$
   ...
   1. BAO 是早期重子-光子流体声学振荡留下的声学视界遗迹，物理尺度 $r_d$ 已知，因此能作为标准尺，用来约束 $D_A(z)$ 和 $H(z)$。
...
1. `相对论标量微扰`（25 分）
   1. 写出一般标量扰动的线元参数化。
   1. 写出规范变换下 $A,B,C,E$ 的变换公式。
   1. 定义 Bardeen potentials。

1. `规范不变量`（15 分）
   证明 $\Phi_B$ 与 $\Psi_B$ 在上述规范变换下不变。

1. `Newtonian gauge`（20 分）
   1. 写出 Newtonian gauge 的条件。
   1. 写出该规范下的一阶 Einstein 方程 `00`、`0i` 和 `ij` 无迹部分。

1. `相对论 Poisson 方程`（20 分）
   1. 说明何时有 $\Phi=\Psi$。
   1. 写出 relativistic Poisson equation。
   1. 说明其如何在亚视界极限退化为牛顿 Poisson 方程。

1. `Bardeen equation`（20 分）
...
\end{align}$$
   ...
   1. 在 $k\gg aH$ 且势变化缓慢时，可忽略时间导数项，退化为
...
1. `弱引力透镜`（20 分）
   ...
   1. 解释星系弱透镜和 CMB 透镜的测量差异。

1. `CMB 统计量`（20 分）
   1. 定义球谐系数 $a_{\ell m}$ 和角功率谱 $C_\ell$。
   1. 写出无偏估计量 $\hat C_\ell$。
   1. 推导 cosmic variance 的相对误差。

1. `标准尺与标准烛光`（20 分）
   1. 区分标准尺、标准烛光、标准警报器。
   1. 说明 BAO 横向和径向分别给出什么信息。
...
\end{align}$$
   ...
   1. $b_g$ 描述星系分布相对于总物质分布的响应强度。在线性尺度上常近似为常数。
   1. 若考虑红移空间畸变，
...
\end{align}$$
   ...
   1. 星系透镜直接测大量背景星系的形状统计；CMB 透镜则通过温度/偏振模耦合和 E-to-B 转换重建透镜势。
...
1. `标准尺与标准烛光`
   ...
   1. BAO 横向给 $D_A(z)/r_d$，径向给 $c/[H(z)r_d]$。
...
1. `FRW 与流体`（20 分）
   ...
   1. 对 $p=w\rho$ 推出 $\rho(a)$ 的标度。
   1. 给出物质、辐射、真空能的 $w$。

1. `数值模板`（20 分）
   1. 写出平直 $\Lambda$CDM 中 $H(z)$ 的公式。
   1. 说明如何计算 $\chi(z)$、$D_A(z)$、$D_L(z)$、$t(z)$。
   1. 代入 $z=1$、$\Omega_{m0}=0.3$、$H_0=70$ 给出结果。

1. `热历史`（20 分）
   1. 写出辐射主导时期的 $H(T)$。
   1. 解释 $g_*$ 和 $g_{*s}$ 的物理意义。
   1. 说明为什么今天 $g_{*,0}\neq g_{*s,0}$。
...
1. `inflation 扰动`（20 分）
   1. 写出标量谱和张量谱的无量纲振幅。
   1. 写出张量-标量比 $r$ 与 slow-roll 参数的关系。
...
\end{align}$$
   ...
   1. 今天中微子温度低于光子温度，因此能量密度和熵密度加权方式不同，故
...
1. `BBN`（20 分）
   ...
   1. 说明 deuterium bottleneck 的物理意义。
   1. 从 $n/p$ 比解释 $Y_p\sim 0.25$ 的来源。

1. `视界问题`（20 分）
   1. 解释“共动哈勃半径在标准大爆炸中增大”为什么导致视界问题。
   1. 说明 inflation 的解决机制。

1. `再热与 e-fold`（20 分）
   1. 若题目给出 inflation 结束能标、再热温度、再热阶段 $w$，说明估算 $N_{\min}$ 的通用套路。
   1. 对本课程现有题面参数，写出 $N_{\min}$ 的典型结果。
...
1. `BBN`
   ...
   1. deuterium bottleneck 指必须等到温度降到
...
1. `牛顿扰动与相对论扰动比较`（20 分）
   1. 说明牛顿微扰适用的物理条件。
   1. 说明为什么大尺度和早期宇宙必须使用相对论微扰。

1. `Bardeen 势与物理含义`（20 分）
   1. 写出 $\Phi_B$、$\Psi_B$。
   1. 说明它们在 Newtonian gauge 中分别对应什么。
   1. 说明无各向异性应力时为什么可以令两者相等。
...
1. `CMB 与弱透镜`（20 分）
   ...
   1. 写出 $C_\ell$ 与 cosmic variance。
   1. 说明弱透镜如何“再加工”原初 CMB。

1. `结构形成`（20 分）
   1. 区分 $T(k)$ 和 $D(z)$。
   1. 解释 BAO wiggles 与 $k_{\mathrm{eq}}$ 转折点的不同来源。

### 参考答案

1. `牛顿扰动与相对论扰动比较`
   1. 牛顿微扰适用于亚视界、速度非相对论、势弱、时空曲率效应可忽略的情形。
   1. 超视界模、辐射主导时期、规范依赖问题和 CMB 大角尺度都必须用相对论微扰处理。
...
\end{align}$$
   ...
   1. CDM 虽无压强，但背景由辐射主导，无法主导势阱形成，只能缓慢增长为

$$\begin{align}
\delta_m\propto \ln a.
\end{align}$$

   1. Jeans 尺度区分了压强支撑主导与引力坍缩主导的波长范围。短于 Jeans 尺度更倾向振荡，长于 Jeans 尺度更可能增长。
...
\end{align}$$
   ...
   1. CMB 透镜在传播途中通过透镜势诱导模耦合，并把一部分 E 模转换为 B 模，所以它不是原初源项，而是后期传播修饰。

1. `结构形成`
   1. $T(k)$ 是尺度依赖的早期演化遗留，$D(z)$ 是纯时间依赖的晚期线性增长。
   1. $k_{\mathrm{eq}}$ 决定整条功率谱在哪里拐弯；BAO wiggles 来自声学视界 $r_d$，决定的是谱上细小振荡周期。
...
1. `inflation 动力学`（20 分）
   ...
   1. 说明 $\epsilon_H\simeq \epsilon_V$ 如何得到。

1. `张量模式`（20 分）
   1. 写出张量扰动的横向无迹条件。
   1. 写出张量二阶作用量。
   1. 定义规范化变量 $v_\lambda$ 并写出模方程。

1. `Bunch-Davies 与张量谱`（20 分）
...
\end{align}$$
   ...
   1. 这保证今天的大尺度模在早期曾位于视界内，从而解决视界问题；同时这些模后来被推出视界，再在晚期重新进入视界，留下近乎标度不变的原初谱。
...
1. `综合微扰与结构形成题`（20 分）
   1. 写出物质增长方程。
   1. 写出从 $P_{\mathrm{ini}}(k)$ 到 $P_{gg}(k,z)$ 的关系。
   1. 写出 $\xi(r)$ 与 $P(k)$ 的关系。

1. `综合 CMB 与观测题`（20 分）
   1. 写出 CMB 温度涨落的球谐展开和 $C_\ell$ 的定义。
   1. 写出 cosmic variance。
   1. 解释 SNIa、BAO、AP 分别测什么。

1. `综合 inflation 题`（20 分）
   1. 说明视界问题。
   1. 写出最小 e-fold 估算公式。
   1. 写出张量谱公式。
...
\end{align}$$
   ...
   1. SNIa 测光度距离 $D_L$；BAO 测标准尺对应的 $D_A$ 和 $H(z)$；AP 测
