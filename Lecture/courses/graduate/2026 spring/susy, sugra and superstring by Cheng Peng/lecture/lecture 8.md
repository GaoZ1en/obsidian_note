...
1. the Hilbert space factorizes into left and right sectors.
1. physical states carry two levels, $N_{L}$ and $N_{R}$.
1. level matching $N_{L}=N_{R}$ is required by single-valuedness and closed-string periodicity.
1. vertex operators are bulk operators with weights $(h,\tilde{h})=(1,1)$.

for open strings:

1. the worldsheet has a boundary.
1. boundary conditions relate left and right movers.
1. there is only one oscillator level $N$.
1. vertex operators are boundary operators with weight $h=1$.
1. massless spin-one states appear naturally, and Chan-Paton factors can turn them into gauge bosons.
...
1. $N=1$: the massless vector, represented by $A_{-1}^{i}\ket{p}$, with $24$ physical polarizations.
1. $N=2$: the first massive level, generated by $A_{-2}^{i}\ket{p}$ and $A_{-1}^{i}A_{-1}^{j}\ket{p}$.
...
1. $N_{L}=N_{R}=1$: the massless tensor states $A_{-1}^{i}\tilde{A}_{-1}^{j}\ket{p}$.
1. higher matched levels: massive towers built by independent left- and right-moving transverse oscillators.
...
1. it is not tachyon-free, because both the open and closed spectra still contain negative mass-squared ground states.
