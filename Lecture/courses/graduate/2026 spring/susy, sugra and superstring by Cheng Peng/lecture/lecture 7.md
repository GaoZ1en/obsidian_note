...
3. physical vertex operators in the worldsheet CFT
...
2. DDF states are generated by $D-2$ transverse oscillators and therefore have positive norm.
3. in the critical theory, every physical state can be represented by a DDF state up to null states.
...
2. $A_{n}^{i}$ commutes with the Virasoro generators.
3. $A_{n}^{i}$ maps physical states to physical states.
4. the algebra of $A_{n}^{i}$ is the same as that of the transverse light-cone oscillators.
5. DDF states reproduce the full physical mass spectrum.
