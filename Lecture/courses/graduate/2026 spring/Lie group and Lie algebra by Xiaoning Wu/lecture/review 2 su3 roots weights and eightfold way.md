...
2. Subtract the full weight diagram of that component from the product character.
3. Repeat with the highest remaining weight.
4. Stop when no weights remain.
...
2. If the object is off-diagonal $E_{ij}$, its root is $\epsilon_i-\epsilon_j$.
3. To identify the representation, find the highest weight and express it as $(p,q)$.
4. To check a proposed tensor decomposition, compare both the highest weights and the total dimension.
5. For $SU(3)$ dimensions, use $\dim(p,q)=\frac{1}{2}(p+1)(q+1)(p+q+2)$ before doing anything more elaborate.
