...
2. Choose positive roots.
3. A finite-dimensional irreducible representation is determined by a dominant integral highest weight.
4. The lowering operators generate the remaining weights.
...
2. The largest dominant weight is the highest weight of one component.
3. Subtract the full weight system of that component.
4. Repeat.
...
2. Group the resulting weights into $H$-irreducible weight diagrams.
3. Check dimensions.
...
2. Highest-weight check: the largest weight in the product must appear.
3. Center check: for groups like $SU(n)$, center charges must match.
4. Reality check: real, complex, and pseudoreal properties must be compatible.
5. Multiplicity check: zero weights often have multiplicity greater than one.

# Exam moves

1. Start with the smallest faithful representation.
2. Compute rank and dimension before trying to identify a representation.
3. Use Dynkin labels for bookkeeping, but translate back to dimensions for sanity.
4. For $SU(3)$, memorize $3,\bar{3},6,\bar{6},8,10,\overline{10},15,27$ by labels.
5. When the answer is a tensor decomposition, write both the symbolic decomposition and the dimension check.
