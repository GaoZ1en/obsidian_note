...
$$\begin{align}
G_{c}^{(4)}(x_{1},x_{2},x_{3},x_{4}) & =\int \mathrm{d}^{4}z\mathrm{d}^{4}w\mathrm{d}^{4}v\mathrm{d}^{4}uG_{c}^{(1)}(x_{1},z)G_{c}^{(2)}(x_{2},w)(i\Gamma ^{(4)}(z,w,v,u))G_{c}^{(2)}(v,x_{3})G_{c}^{(2)}(u,x_{4}) \\
...
act $\displaystyle{\frac{\delta}{\delta J(x_{4})}}$ on $G_{c}^{(3)}(x_{1},x_{2},x_{3})$, by similar reasoning, we can get the required expansion for $G_{c}^{(4)}$. and we can also get the expansion for higher-point connected Green functions by acting $\displaystyle{\frac{\delta}{\delta J(x)}}$ on lower-point connected Green functions as
...
take a shift of variable $\displaystyle{\xi \to \xi + A^{-1}\cdot \eta}$, we have

$$\begin{align}
\int \mathrm{d}\xi ^{*}\mathrm{d}\xi \exp\left(-\xi ^{*}\cdot A\cdot \xi+\eta ^{*}\cdot \xi+\xi ^{*}\cdot \eta\right) & =\int \mathrm{d}\xi ^{*}\mathrm{d}\xi \exp\left(-(\xi + A^{-1}\cdot \eta)^{*}\cdot A\cdot (\xi + A^{-1}\cdot \eta)+\eta ^{*}\cdot (\xi + A^{-1}\cdot \eta)+(\xi + A^{-1}\cdot \eta)^{*}\cdot \eta\right) \\
