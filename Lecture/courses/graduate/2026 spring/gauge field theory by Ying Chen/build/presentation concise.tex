...
% latexmk -pdf -interaction=nonstopmode -halt-on-error -outdir=build "presentation concise.tex"
...
\title[Where Naive BRST Fails]{Where Naive BRST Becomes Insufficient}
\subtitle{Reducibility, open algebras, and the BV master equation}
\author[Zi-en Gao]{Zi-en Gao}
\date{\today}
...
% Keep section markers in the outline without adding divider slides.
\AtBeginSection[]{}
...
\begin{frame}{Thesis}
  \begin{alertblock}{Main claim}
    The usual field-only BRST complex is sufficient for closed irreducible
    gauge algebras. BV extends it to reducible systems and open gauge
    algebras.
  \end{alertblock}
...
\begin{frame}{2-Form: Singular FP Determinant}
  Choose
  \[
  ...
  The exact parameter direction lies in the kernel:
  \[
  ...
  \begin{alertblock}{Obstruction}
    The naive determinant $\det M$ is singular because the gauge fixing did not
    divide by the redundancy of the gauge parameter.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Poisson Sigma Model: On-Shell Closure}
  ...
  {\footnotesize
  More precisely, the $X^i$ transformations close by the Poisson identity,
  while the $A_i$ transformations close modulo
  $dX^i+\pi^{ij}A_j=0$.}
...
\begin{frame}{Why ``EOM Terms'' Are Not Harmless in the Path Integral}
...
\begin{frame}{Minimal BV Action: First Layers}
  ...
  \slidecite{Only the schematic layers are displayed here; the master equation fixes the exact coefficients and signs.}
\end{frame}
...
\begin{frame}{Master Equation and Nilpotency}
  The central condition is
  \[
    (\BV,\BV)=0 .
  \]

  It imposes, in one equation:
  \begin{itemize}
    \item invariance of $S_0$;
    \item closure, reducibility, and higher identities;
    \item cancellation of open-algebra \EOM{} terms.
  \end{itemize}

  The BV differential is then off-shell nilpotent:
  \[
    s_{\mathrm{BV}}^2F
    =\frac12(F,(\BV,\BV))=0 .
  \]
  ...
  \begin{exampleblock}{Interpretation}
    Closed irreducible BRST is the truncation with no ghost-for-ghosts and no
    antifield-quadratic open-algebra correction.
  \end{exampleblock}
\end{frame}

\section{Gauge Fixing}

\begin{frame}{Antifields and Non-Minimal Pair}
  ...
  To impose a gauge condition, add a contractible pair
  \[
  ...
  Gauge fixing then eliminates antifields by
  \[
...
\begin{frame}{2-Form: Minimal BV Complex}
  The minimal fields are
  \[
    B_{\mu\nu},\qquad c_\mu,\qquad \rho,
  \]
  where $c_\mu$ is the ghost for $\Lambda_\mu$ and $\rho$ is the
  ghost-for-ghost for $\Lambda_\mu\sim\Lambda_\mu+\partial_\mu\alpha$.
...
\begin{frame}{2-Form: Gauge Fixing the Complex}
  ...
  \slidecite{Schematic: the full non-minimal sector contains additional auxiliary fields; this slide only shows why the ghost direction must also be fixed.}

  The naive operator has zero modes, but BV also fixes the ghost direction:
  \[
    M_1=d^\dagger d,
    \qquad
...
\begin{frame}{Poisson Sigma Model BV Action}
  \small
  Minimal fields are $X^i,A_i,c_i$, with antifields $X_i^*,A^{*i},c^{*i}$.
...
\begin{frame}{Closing Point}
  ...
  \begin{itemize}
    ...
    \item $(S,S)=0$ encodes gauge invariance, closure, reducibility, Jacobi
      identities, and open-algebra \EOM{} terms.
  \end{itemize}
\end{frame}
