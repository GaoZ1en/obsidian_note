...
2.  **Point Group Reduction**: Given basis functions (like $x^2, xy$), find matrices, characters, reduce to Irreps, and find new basis.
3.  **Young Tableaux**: Draw Tableau $\to$ Write Operator $\to$ Find Representation Matrix (Orthogonal form).
4.  **Branching Rules**: $S_{m+n} \downarrow S_m \times S_n$ reduction.
5.  **Rotations**: Euler Angles from Matrix, Axis-Angle representation of SU(2), Rotating Spherical Harmonics.
...
2.  **Column Orthogonality**: $\sum_{irreps} \chi_i(C_k) \chi_i^*(C_l) = \frac{|G|}{n_k} \delta_{kl}$.
3.  **Dimensions**: $\sum d_i^2 = |G|$.
*Example (Exam 1 Q3)*:
...
3.  **Trace & Character**: Sum diagonal elements.
4.  **Reduction Formula**: $n_\mu = \frac{1}{|G|} \sum_i n_i \chi(C_i) \chi^{(\mu)*}(C_i)$.
...
2.  **Fill Table**: I'll give you a slightly broken table (like Exam 1 Q3).
3.  **Rep Reduction**: I'll use $D_4$ or $C_{4v}$ instead of $D_3$, and bases like $xz, yz$.
4.  **Young Operator**: Change shape to $[3,1]$ or $[2,2]$.
5.  **SU(2)**: Change axis to $\frac{1}{\sqrt{2}}(\hat{e}_1 + \hat{e}_3)$.
