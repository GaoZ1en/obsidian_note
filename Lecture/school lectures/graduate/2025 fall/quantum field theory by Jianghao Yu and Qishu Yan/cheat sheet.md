...
2.  **S-Matrix**: $\langle f | T \exp( i \int d^4x \mathcal{L}_{int} ) | i \rangle$.
3.  **Wick's Theorem**: $T(\phi_1 \dots \phi_n) = : \phi_1 \dots \phi_n : + \text{all contractions}$.
4.  **Contraction**: $\langle 0 | T \phi_I(x) \phi_I(y) | 0 \rangle = \Delta_F(x-y)$.
    *   Draw diagrams.
