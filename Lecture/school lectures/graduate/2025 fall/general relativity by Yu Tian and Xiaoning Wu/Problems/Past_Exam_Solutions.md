...
2.  **Deflection of Light**: Light rays bend near massive bodies (Sun) by angle $\delta = 4GM/bc^2$ (twice Newtonian value). Confirmed by Eddington (1919).
3.  **Gravitational Redshift**: Verified by Pound-Rebka experiment.
...
2.  **Angular Momentum**: To have $E_B < 0$, we need $L_B$ to be sufficiently negative (counter-rotating). The absorption reduces BH angular momentum $J$.
3.  **Irreducible Mass**: $M^2 = M_{irr}^2 + J^2/(4M_{irr}^2)$. $M_{irr} \propto \sqrt{A}$. Since $\delta A \ge 0$, $M_{irr}$ increases. Energy extraction ($\delta M < 0$) is possible only if $\delta J$ is large negative.
