...
1. anyons in the toric code
	...
	1. show that when acting on a toric code ground state the operator $$\begin{align}V_{\hat{C}} & =\prod _{\ell \bot\hat{C}}Z_{\ell}\end{align}$$creates a state which violates only the plaquette operators in the boundary of $\displaystyle{\hat{C}}$, $\displaystyle{\partial \hat{C}}$
	1. show that a bound state of an $\displaystyle{e}$ partivle and an $\displaystyle{m}$ particle in the $\displaystyle{2\mathrm{d}}$ toric code must be a fermion.
