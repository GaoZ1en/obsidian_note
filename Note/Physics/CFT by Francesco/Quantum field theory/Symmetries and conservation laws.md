...
1. under Lorentz transformation $\displaystyle{x'^{\mu}=\Lambda^{\mu}_{~\nu}x^{\nu}}$
...
1. under scale $\displaystyle{x'=\lambda x}$, suppose the field $\displaystyle{\phi_{i}}$ has the scaling dimension $\displaystyle{\Delta_{i}}$
