...
=\frac{1}{s_P}
\sum_{P=RS}\sum_{Q=TU}
\left(\Phi_{R|T}\Phi_{S|U}-(R\leftrightarrow S)\right).
\]
...
选定 Green 算子 \(G\)。对 Cauchy 问题通常用 retarded \(G_R\)，对散射或边界值问题则按物理问题选 advanced、Feynman、Euclidean、Dirichlet/Neumann Green kernel。积分方程为
...
\[
K\phi_n
=-\frac12\sum_{a+b=n-1}\phi_a\phi_b,
\qquad n\ge1,
\]
...
\[
\phi_1(x)
=-\frac12\int d\mu_y\,G(x,y)\phi_0(y)^2,
\]

\[
\phi_2(x)
=-\int d\mu_y\,G(x,y)\phi_0(y)\phi_1(y)
=\frac12\int d\mu_y d\mu_z\,
G(x,y)\phi_0(y)G(y,z)\phi_0(z)^2.
\]
...
\[
\phi_1(x)
=-\frac1{3!}\int d\mu_y\,G(x,y)\phi_0(y)^3.
\]
...
\phi_N(x)
=-\sum_{r\ge2}\frac{1}{r!}
\sum_{\substack{n_1+\cdots+n_r=N-1}}
g_r
\int d\mu_y\,G(x,y)
...
\[
\omega(\phi;\epsilon;\delta_1\phi,\delta_2\phi)
=\delta_1\Theta(\phi;\epsilon;\delta_2\phi)
-\delta_2\Theta(\phi;\epsilon;\delta_1\phi).
\]
...
\[
\phi(\alpha;\epsilon)
=\phi_0(\alpha)+\epsilon\phi_1(\alpha)+\epsilon^2\phi_2(\alpha)+\cdots,
\]

其中 \(\alpha^A\) 是自由解空间的坐标，例如 mode coefficients \(a_{\bf k},a_{\bf k}^*\) 或离散模式 \(a_n,a_n^\dagger\)。定义 solution map

\[
p_\epsilon:\mathcal S_0\to \mathcal S_\epsilon,
\qquad \alpha\mapsto \phi(\alpha;\epsilon).
\]

那么微扰后的相空间形式不是直接用 \(\Omega_0\)，而是完整 symplectic current 拉回：

\[
\Omega_{\rm eff}(\epsilon)
=p_\epsilon^*\Omega_\epsilon
=\int_\Sigma p_\epsilon^*\omega(\phi;\epsilon).
\]
...
\[
\Omega_{\rm eff}
=\Omega^{(0)}+\epsilon\Omega^{(1)}
+\epsilon^2\Omega^{(2)}+\cdots.
\]
...
\[
\Theta=\sum_{r\ge0}\epsilon^r\Theta_r,
\qquad
\omega=\sum_{r\ge0}\epsilon^r\omega_r.
\]
...
\qquad
\delta\phi=\delta\phi_0+\epsilon\delta\phi_1+\epsilon^2\delta\phi_2+\cdots
\]
...
\left[
\frac1{s!}
\frac{d^s}{d\epsilon^s}
\omega_r(\phi(\epsilon);\delta\phi(\epsilon),\delta\phi(\epsilon))
\right]_{\epsilon=0}.
\]
...
3. **若 \(\Omega_{\rm eff}\neq\Omega_0\)**：做 order-by-order canonical transformation
   \[
   \alpha^A\mapsto A^A(\alpha;\epsilon)
   =\alpha^A+\epsilon f_1^A(\alpha)+\epsilon^2 f_2^A(\alpha)+\cdots
   \]
   使
   \[
   \Omega_{\rm eff}
...
\[
\Omega_{\rm eff}
=i Z_{IJ}(\epsilon)\, da_I^*\wedge da_J+\cdots,
\]
...
\[
j_\lambda
=\Theta(\phi;\delta_\lambda\phi)-\mu_\lambda
\]
...
=\int_\Sigma \omega(\phi;\delta\phi,\delta_\lambda\phi).
\]
...
\[
\delta H_\lambda
=\int_{\partial\Sigma} k_\lambda(\phi;\delta\phi),
\]
...
\[
H_\lambda[\phi]-H_\lambda[\bar\phi]
=\int_{\bar\phi}^{\phi}\iota_{X_\lambda}\Omega.
\]
...
\qquad
X_{\lambda,{\rm eff}}:=p_\epsilon^{-1}{}_*\;X_\lambda\;p_{\epsilon *}.
\]
...
\[
\Omega_{\rm eff}
=\sum_{n\ge0}\epsilon^n\Omega^{(n)},
\qquad
X_{\lambda,{\rm eff}}
=\sum_{m\ge m_0}\epsilon^m X_\lambda^{(m)}.
\]
...
\delta H_\lambda^{(N)}
=\sum_{n=0}^{N}
\Omega^{(n)}(\delta\phi,X_\lambda^{(N-n)}).
\]
...
\[
\delta H_\lambda^{(N)}
=\Omega^{(N)}(\delta\phi,T_\lambda\phi)
\]
...
\[
\{H_\lambda,H_\eta\}
=\Omega_{\rm eff}(X_\lambda,X_\eta)
=H_{[\lambda,\eta]}+K(\lambda,\eta).
\]
...
\[
K(\lambda,\eta)
=\sum_N\epsilon^N K^{(N)}(\lambda,\eta).
\]
...
\[
y=c\left(-\sin[(r-1)s]x_0+\cos[(r-1)s]y_0\right),
\qquad c=(1+4q)^{-1/4}.
\]
...
=-i\,\delta a\wedge\delta a^\dagger
\]
...
\[
H_{\rm low}
=f_1\left(a^\dagger a+\frac12\right)
\]
...
\[
y_{\rm low}
=-\frac{i}{\sqrt{2\sqrt{1+4\lambda\omega}}}
\left(ae^{-if_1t}-a^\dagger e^{if_1t}\right).
\]

相应地

\[
[x(t_1),y(t_2)]_{\rm low}
=+\frac{i}{\sqrt{1+4\lambda\omega}}\cos f_1(t_1-t_2).
\]
...
\[
u_{n,m}
=e^{-i\omega_{n,m}t}e^{im\varphi}
N_{n,m}(\cos x)^\Delta(\sin x)^{|m|}
P_n^{(|m|,\Delta-1)}(\cos2x),
\]
...
若有 gauge symmetry，先定义 proper gauge quotient 和 large/edge modes。

### Step 2：选 Green 算子

明确 \(G\) 是 retarded、advanced、Feynman、Euclidean、Dirichlet、Neumann 还是投影掉 zero mode 后的 pseudo-inverse。这个选择同时决定：
...
\[
\Omega_{\rm eff}
=\int_\Sigma \omega(\phi(\alpha;\epsilon);\delta\phi(\alpha;\epsilon)).
\]
...
\Omega_{\rm eff}
=\int_\Sigma
\omega(\phi_0+\epsilon\phi_1+\cdots;
\delta\phi_0+\epsilon\delta\phi_1+\cdots).
\]
...
\[
\Omega_{\rm eff}
\xrightarrow{\text{Darboux, order by order}}
i\sum_I dA_I^*\wedge dA_I.
\]
...
\[
\delta H_\lambda^{(N)}
=\int_{\partial\Sigma} k_\lambda^{(N)}.
\]
