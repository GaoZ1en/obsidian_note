...
ds^2=\frac{1}{\cos^2 x}
\left(
-dt^2+dx^2+\sin^2x\,d\varphi^2
\right),
\qquad
0\le x<\frac{\pi}{2},\quad \varphi\sim\varphi+2\pi.
\]
...
\[
u_{n,m}(t,x,\varphi)
=e^{-i\omega_{n,m}t}e^{im\varphi}R_{n,m}(x),
\qquad n=0,1,\ldots,\quad m\in\mathbb Z,
\]
...
\[
d\Sigma^t=\tan x\,dx\,d\varphi
=\frac{r}{1+r^2}\,dr\,d\varphi.
\]

归一化要求

\[
(u_{n,m},u_{n',m'})_{\rm KG}
=\delta_{nn'}\delta_{mm'}.
\]
...
N_{n,m}^2
=
\frac{n!\,\Gamma(n+|m|+\Delta)}
{2\pi\,\Gamma(n+|m|+1)\Gamma(n+\Delta)}.
\]
...
\Omega_0
=-i\sum_{n,m}
\delta a_{n,m}\wedge\delta a_{n,m}^*
\]

或按相反 wedge convention 写成 \(i\,\delta a^*\wedge\delta a\)。量子化时

\[
[\hat a_{n,m},\hat a_{n',m'}^\dagger]
=\delta_{nn'}\delta_{mm'}.
\]
...
\[
G_R\sim \theta(t-t')
\frac{\sin\omega_\alpha(t-t')}{\omega_\alpha}
\]
...
\[
\phi_{a,b}|_{\Sigma_{t_0}}=0,
\qquad
\partial_t\phi_{a,b}|_{\Sigma_{t_0}}=0
\quad (a,b)\ne(0,0).
\]
...
C_{\alpha\beta\gamma}
=\int d^3X\sqrt{-g}\,
u_\alpha^*(X)u_\beta(X)u_\gamma(X)
\]
...
\[
\int dt\,
e^{i(\omega_\alpha-\sigma_\beta\omega_\beta-\sigma_\gamma\omega_\gamma)t}
\times
\int_0^{\pi/2}dx\,d\varphi\,
\frac{\sin x}{\cos^3x}\,
...
\[
\Omega_{\rm eff}
=\Omega_{\rm int}\big|_{\Sigma_{t_0}}
=\Omega_0.
\]
...
\[
H=\int_{\Sigma_t}N\sqrt{h}\,
\left[
\frac12N^{-2}(\partial_t\phi)^2
...
\[
H_0=\sum_{n,m}\omega_{n,m}
\left(a_{n,m}^*a_{n,m}+\frac12\right)
\]
...
\[
\delta H_\xi=\iota_{X_\xi}\Omega,
\qquad
X_\xi\phi=\mathcal L_\xi\phi=\xi^\mu\nabla_\mu\phi.
\]
...
\[
\ddot q_\alpha+\omega_\alpha^2 q_\alpha
...
\[
q_\alpha^{\rm particular}\sim
\frac{F_\alpha}{2i\omega_\alpha}\,t\,e^{-i\omega_\alpha t}.
\]
...
\[
\omega_{\beta}+\omega_{\gamma}-\omega_{\delta}
=
\Delta+\big(N_\beta+N_\gamma-N_\delta\big).
\]
...
=
\sum_{\beta,\gamma,\delta}^{\rm resonant}
...
\[
\omega_\alpha+\omega_\delta
=\omega_\beta+\omega_\gamma,
\qquad
...
4. CPS 量子化时应区分：
   \[
   \text{bare initial-data coordinates}
   \quad\text{vs.}\quad
   \text{dressed resonant coordinates}.
   \]
...
\[
u_{n,m}=e^{-i\omega_{n,m}t}e^{im\varphi}
...
\[
\sum_i\sigma_i\omega_{\alpha_i}=\omega_\alpha,
\qquad
\sum_i\sigma_i m_{\alpha_i}=m_\alpha.
\]
