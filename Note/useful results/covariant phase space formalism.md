# formalism

$$\begin{align}
S & =\int _{M}\mathbf{L}+\int _{\Gamma}\ell
\end{align}$$

# models

## scalar

## fermion systems

## gauge theory

### Yang-Mills

the action of Yang-Mills theory is given by
$$\begin{align}
S & =-\frac{1}{4}\int _{M} \mathrm{d}^{4}x\sqrt{ -g }F^{a}_{\mu \nu}F_{a}^{\mu \nu}
\end{align}$$

where $\displaystyle{F^{a}_{\mu \nu}=\nabla _{\mu}A^{a}_{\nu}-\nabla _{\nu}A^{a}_{\mu}+f^{a}_{~bc}A^{b}_{\mu}A^{c}_{\nu}}$, and $\displaystyle{f^{a}_{~bc}}$ is the structure constant of the gauge group. the Lie algebra indices are raised and lowered with the Killing metric $\displaystyle{g_{ab}=-\frac{1}{2}f^{c}_{~ad}f^{d}_{~bc}}$. take a variation of the action

$$\begin{align}
\delta S & =\int _{M}\mathrm{d}^{4}x\sqrt{ -g }E^{\nu}_{a}\delta A^{a}_{\nu}+\theta|_{\Sigma _{f}}-\theta|_{\Sigma _{i}} \\
E^{\nu}_{a} & =\nabla _{\mu}F^{\mu \nu}_{a}+f^{c}_{~ab}A^{b}_{\mu}F^{\mu \nu}_{c} \\
\theta & =\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu}F_{a}^{\mu \nu}\delta A^{a}_{\nu}
\end{align}$$

consider a diffeomorphism generated by vector $\displaystyle{\xi}$

$$\begin{align}
X_{\xi} & =\int \mathrm{d}^{4}x\mathcal{L}_{\xi}A^{a}_{\mu} \frac{\delta}{\delta A^{a}_{\mu}}
\end{align}$$

act $\displaystyle{X_{\xi}}$ on the action, we have

$$\begin{align}
X_{\xi}\cdot \delta S & =\alpha_{\xi}|_{\Sigma _{f}}-\alpha _{\xi}|_{\Sigma _{i}} \\
\alpha_{\xi} & =\frac{1}{4}\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau ^{\mu}\xi ^{\nu}g_{\mu \nu}F_{\rho \sigma}^{a}F^{\rho \sigma}_{a}
\end{align}$$

the corresponding Noether charge is then

$$\begin{align}
H_{\xi} & =X_{\xi}\cdot \theta-\alpha_{\xi} \\
 & =\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau ^{\mu}\xi ^{\nu}\left(F_{\mu \rho}^{a}F_{a\nu}^{~~\rho}-\frac{1}{4}g_{\mu \nu}F_{\rho \sigma}^{a}F^{\rho \sigma}_{a}\right)
\end{align}$$

this action is gauge invariant, that is, consider a gauge transformation generated by $\displaystyle{\varepsilon ^{a}(x)}$

$$\begin{align}
X_{\varepsilon} & =\int \mathrm{d}^{4}x(\nabla _{\mu}\varepsilon ^{a}+f^{a}_{~bc}A^{b}_{\mu}\varepsilon ^{c}) \frac{\delta}{\delta A^{a}_{\mu}}
\end{align}$$

then act $\displaystyle{X_{\varepsilon}}$ on the action, we have

$$\begin{align}
X_{\varepsilon}\cdot \delta S & =\int _{\Sigma _{f}-\Sigma _{i}}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu} \alpha ^{\mu}_{\varepsilon} \\
\alpha ^{\mu}_{\varepsilon} & =F^{\mu \nu}_{a}\nabla _{\nu}\varepsilon ^{a}
\end{align}$$

then the corresponding Noether current is

$$\begin{align}
H_{\varepsilon} & =X_{\varepsilon}\cdot \theta-\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu}\alpha ^{\mu}_{\varepsilon} \\
 & =\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu}F_{a}^{\mu \nu}(\nabla _{\nu}\varepsilon ^{a}+f^{a}_{~bc}A^{b}_{\nu}\varepsilon ^{c})-\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu}F^{\mu \nu}_{a}\nabla _{\nu}\varepsilon ^{a} \\
 & =f^{a}_{~bc}\int _{\Sigma}\mathrm{d}^{3}x\sqrt{ \sigma }\tau _{\mu}F^{\mu \nu}_{a}A^{b}_{\nu}\varepsilon ^{c}
\end{align}$$

which is the Yang-Mills charge associated with gauge parameter $\displaystyle{\varepsilon ^{a}}$. to get results of Maxwell theory, we just set the structure constant $\displaystyle{f^{a}_{~bc}=0}$ and drop the Lie algebra indices. 

### Chern-Simons

the action of Chern-Simons theory in 3-dimension is given by


$$\begin{align}
S & =\frac{k}{4\pi}\int _{M}\mathrm{Tr}\left(A\wedge \mathrm{d}A+\frac{2}{3}A\wedge A\wedge A\right)  \\
& =\frac{k}{4\pi}\int _{M}\mathrm{d}^{3}x\varepsilon ^{\mu \nu \rho}\left(A^{a}_{\mu}\partial _{\nu}A^{a}_{\rho}+\frac{1}{3}f^{abc}A^{a}_{\mu}A^{b}_{\nu}A^{c}_{\rho}\right)
\end{align}$$

take a variation of the action

$$\begin{align}
\delta S & =\frac{k}{2\pi}\int _{M}\mathrm{Tr}\left(\delta A\wedge F\right)+\frac{k}{4\pi}\int _{\Sigma _{f}-\Sigma _{i}}\mathrm{Tr}(A\wedge\delta A) \\
\theta & =\frac{k}{4\pi}\int _{\Sigma}\mathrm{Tr}(A\wedge \delta A) \\
\omega & =\frac{k}{4\pi}\int _{\Sigma}\mathrm{Tr}\left(\delta A\wedge \delta A\right)
\end{align}$$

consider a gauge transformation generated by $\displaystyle{\varepsilon ^{a}(x)}$

$$\begin{align}
X_{\varepsilon} & =\int \mathrm{d}^{3}x(\mathrm{d}\varepsilon ^{a}+f^{a}_{~bc}A^{b}\varepsilon ^{c}) \frac{\delta}{\delta A^{a}}
\end{align}$$

act on the action, we have

$$\begin{align}
X_{\varepsilon}\cdot\delta S & =\frac{k}{2\pi}\int _{M} \mathrm{Tr}\left((\mathrm{d}\varepsilon+[A,\varepsilon])\wedge F\right)+\frac{k}{4\pi}\int _{M}\mathrm{d}\mathrm{Tr}\left(A\wedge(\mathrm{d}\varepsilon+[A,\varepsilon])\right) \\
 & =
\end{align}$$

$$\tag{.}\begin{align}
X_{\varepsilon}\cdot\delta S & =\frac{k}{4\pi}\int _{M} \mathrm{Tr}\left(\delta A\wedge \mathrm{d}A+A\wedge \mathrm{d}\delta A+2\delta A\wedge A\wedge A\right)
\end{align}$$

## gravity

### general relativity with Gibbons-Hawking-York term


# applications & generalization

## entropy

## quantization

## extracting boundary dynamics

## higher form symmetry

## structure of phase space

## non-commutative geometry

## proof of Ryu-Takayanagi formula

