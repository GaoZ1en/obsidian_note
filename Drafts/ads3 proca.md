$$\begin{align}\tag{1.3}
\cot \rho(\partial_{\rho}A_{\theta}- \partial_{\theta}A_{\rho}) & =-\mu A_{t} \\
-\cot \rho(\partial_{\theta}A_{t}-\partial_{t}A_{\theta}) & =-\mu A_{\rho} \\
-\sin \rho \cos \rho(\partial _{t}A_{\rho}-\partial _{\rho}A_{t}) & =-\mu A_{\theta}
\end{align}$$

make the following ansatz

$$\tag{1.4}
\begin{align}
A_{\mu} & =e^{-i \omega t}e^{im\theta}R_{\mu}(\rho)
\end{align}
$$

$$\tag{.}
\begin{align}
\cot \rho\left( \partial _{\rho}R_{\theta}+\frac{m}{\mu}\cot \rho(mR_{t}+\omega R_{\theta}) \right) & =-\mu R_{t} \\
\sin \rho \cos \rho\left( - \frac{\omega}{\mu}\cot \rho(mR_{t}+\omega R_{\theta})+\partial _{\rho}R_{t} \right) & =-\mu R_{\theta} \\
\frac{i}{\mu}\cot \rho(mR_{t}+\omega R_{\theta}) & =R_{\rho} \\
\end{align}
$$

let $\displaystyle{z=\cos ^{2}\rho}$

$$\tag{.}
\begin{align}
 \left( \frac{m^{2}}{\mu}z+(1-z)\mu \right) R_{t}+\left( \frac{m\omega}{\mu}z-2z(1-z)\partial _{z} \right)R_{\theta}  & =0 \\
\left( \frac{m\omega }{\mu} z +2z(1-z)\partial _{z} \right)R_{t}+\left( \frac{\omega ^{2}}{\mu}z-\mu \right) R_{\theta} & =0 \\
\frac{i}{\mu}\sqrt{ \frac{z}{1-z} }(mR_{t}+\omega R_{\theta}) & =R_{\rho}
\end{align}
$$

consider the asymptotic behavior at $\displaystyle{z=0^{+}}$. we have

$$\tag{.}
\begin{align}
R_{t},R_{\theta} & \sim z^{(\mu+2)/2} \\
R_{\rho} & \sim z^{(\mu+3)/2}
\end{align}
$$
and the asymptotic behavior at $\displaystyle{z=1^{-}}$, we have

$$\tag{.}
\begin{align}
R_{t},R_{\theta} & \sim (1-z)^{|m|/2}\\
R_{\rho} & \sim (1-z)^{(|m|-1)/2}
\end{align}
$$

make the following ansatz

$$\tag{.}
\begin{align}
R_{t} & =z^{(\mu+2)/2}(1-z)^{|m|/2}F_{t}(z) \\
R_{\rho} & =z^{(\mu+3)/2}(1-z)^{(|m|-1)/2}F_{\rho}(z) \\
R_{\theta} & =z^{(\mu+2)/2}(1-z)^{|m|/2}F_{\theta}(z)
\end{align}
$$

$$\tag{.}
\begin{align}
 \left( \frac{m^{2}}{\mu}z+(1-z)\mu \right) z^{(\mu+2)/2}(1-z)^{|m|/2}F_{t}+\left( \frac{m\omega}{\mu}z-2z(1-z)\partial _{z} \right) z^{(\mu+2)/2}(1-z)^{|m|/2}F_{\theta} & =0 \\
\left( \frac{m\omega }{\mu} z +2z(1-z)\partial _{z} \right)z^{(\mu+2)/2}(1-z)^{|m|/2}F_{t}+\left( \frac{\omega ^{2}}{\mu}z-\mu \right) z^{(\mu+2)/2}(1-z)^{|m|/2}F_{\theta} & =0
\end{align}
$$