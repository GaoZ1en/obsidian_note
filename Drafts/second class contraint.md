...
\prod _{i}\delta(\varphi _{i})
\det\bigl(\{\varphi _{i},\varphi _{j}\}_{\mathrm{PB}}\bigr)^{1/2}
\exp\Bigl( i \int \mathrm{d}t\, (\pi _{a}\dot\phi ^{a}-H[\phi ^{a},\pi _{a}])\Bigr).
$$
...
- $\varphi_i(\phi,\pi)=0$ 是一组 **第二类约束**，
  $$
  C_{ij}(z) := \{\varphi_i,\varphi_j\}_{\rm PB},\qquad
  \det C_{ij} \neq 0 \text{ 在约束面上}.
  $$
...
可以选定一组正则变量把约束写成
$$
\varphi_i(q,p) = 0 \quad \Rightarrow \quad
	ext{用它们解出一部分 $q,p$，只剩下“物理自由度”的相空间。}
$$
行列式因子就是从“原始相空间测度”
$\prod dq\,dp$
映射到“约化相空间测度”时的雅可比。
...
$$
Z = \int \mathcal{D}\phi\, \exp\Bigl(i S[\phi]\Bigr),
\qquad S[\phi] = \int dt\, L(\phi,\dot\phi).
$$

对一般非约束系统，我们可以通过高斯积分把哈密顿形式积分掉动量：
$$
Z = \int \mathcal{D}\phi\,\mathcal{D}\pi\;
e^{i\int (\pi\dot\phi - H)}
\longrightarrow
...
$$
Z = \int \mathcal{D}z\, \prod_i\delta(\varphi_i)\, \det\{\varphi_i,\varphi_j\}^{1/2}
\exp\Bigl(i\int dt\,(\pi_a\dot\phi^a - H)\Bigr)
$$
写成纯配置变量的积分。
...
$$
L(q,\dot q,\lambda)
= \tfrac12 M_{ab}\,\dot q^a\dot q^b - V(q)
  + \lambda_i f^i(q,\dot q)
$$
...
3. 路径积分中
	$$
	Z = \int \mathcal{D}q\,\mathcal{D}p\,\mathcal{D}\lambda\; 
	\delta(\varphi_i)\det\{\varphi_i,\varphi_j\}^{1/2}
	e^{i\int (p\dot q - H)}.
	$$
