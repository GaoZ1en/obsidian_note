...
\begin{align}
S_{E}[\phi]
 & =\frac{1}{2}\int _{M}\mathrm{d}^{d}X\sqrt{g}\,\phi\mathcal{O}\phi,
& \mathcal{O}
 & =-\nabla ^{2}+\mu ^{2}.
\end{align}
$$
...
K_{\mathcal O}(0;X,Y) & =\delta _{g}(X,Y)
=\frac{\delta ^{(d)}(X-Y)}{\sqrt{g(X)}}.
\end{align}
$$
...
\log Z_{\mathrm{cov}} & =-\frac{1}{2}\log\det _g\mathcal O
=\frac{1}{2}\int _{0^{+}}^{\infty}\frac{\mathrm{d}s}{s}\operatorname{Tr}_{g}e^{-s\mathcal O}.
\end{align}
$$
...
$$
\mathrm{d}s^{2}=N^{2}(x)\mathrm{d}\tau ^{2}+\sigma _{ab}(x)\mathrm{d}x^{a}\mathrm{d}x^{b},
\qquad \sqrt g=N\sqrt\sigma .
$$
...
$$
\nabla ^{2}\phi
=\frac{1}{N^{2}}\partial _{\tau}^{2}\phi
+\frac{1}{N\sqrt\sigma}\partial _a\!\left(N\sqrt\sigma\,\sigma ^{ab}\partial _b\phi\right).
$$
...
$$
\mathcal O=N^{-2}\mathcal D,
\qquad
S_E=\frac{1}{2}\int \mathrm d\tau\,\mathrm d^{d-1}x\,w(x)\,\phi\mathcal D\phi.
$$

After the boundary term is set to zero, $H$ is self-adjoint with respect to the weighted spatial inner product:

$$
\langle u,Hv\rangle _w
=\int _\Sigma \mathrm d^{d-1}x\,N\sqrt\sigma
\left(\sigma ^{ab}\partial _a u^{*}\partial _b v+\mu ^2u^{*}v\right).
$$
...
\sum _j u_j(x)u_j^{*}(y) & =\delta _w(x,y)
=\frac{N(x)}{\sqrt{\sigma(x)}}\delta ^{(d-1)}(x-y).
\end{align}
$$
...
$$
S_E=\frac{1}{2}\sum _j\int \mathrm d\tau\,
q_j^{*}(\tau)\left(-\partial _\tau^{2}+\omega _j^{2}\right)q_j(\tau).
$$
...
$$
\mathcal K_{\mathcal D}(s;\tau,x;\tau',y)
=\frac{1}{\beta}\sum _{k\in\mathbb Z}\sum _j
e^{-s(\nu _k^{2}+\omega _j^{2})}
...
\mathcal K_{\mathcal D}(0;\tau,x;\tau',y)
& =\delta(\tau-\tau')\delta _w(x,y) \\
& =\frac{N(x)}{\sqrt{\sigma(x)}}
\delta(\tau-\tau')\delta ^{(d-1)}(x-y).
\end{align}
$$
...
$$
\int _\Sigma\mathrm d^{d-1}x\,w(x)u_j u_k^{*}
=\frac{\delta _{jk}}{2\omega_j}
$$
...
$$
\langle\phi,\mathcal O\phi\rangle _g
=\langle\phi,\mathcal D\phi\rangle _w.
$$

The remaining difference is the functional measure. The two field-space norms obey

$$
\lVert\delta\phi\rVert _g^{2}
=\langle\delta\phi,N^{2}\delta\phi\rangle _w.
$$
...
Z_{\mathrm{cov}}
& =\left[\det _g\mathcal O\right]^{-1/2}
=J_N\left[\det _w\mathcal D\right]^{-1/2}.
\end{align}
$$
...
$$
\mathrm ds^{2}=(1+r^{2})\mathrm d\tau^{2}
+\frac{\mathrm dr^{2}}{1+r^{2}}+r^{2}\mathrm d\phi^{2},
...
H=-(1+r^{2})\left[
(1+r^{2})\partial _r^{2}
+\frac{1+3r^{2}}{r}\partial _r
+\frac{1}{r^{2}}\partial _\phi^{2}
-\mu ^{2}
\right].
$$
...
$$
u_{n\ell}(r,\phi)
=C_{n\ell}e^{i\ell\phi}r^{|\ell|}
(1+r^{2})^{-(\Delta+|\ell|)/2}
P_n^{(\Delta-1,|\ell|)}\!\left(\frac{r^{2}-1}{r^{2}+1}\right),
$$

with $C_{n\ell}$ chosen to make the modes orthonormal in the measure $r\,\mathrm dr\,\mathrm d\phi/(1+r^{2})$. They obey

$$
Hu_{n\ell}=\omega _{n\ell}^{2}u_{n\ell},
\qquad
\omega _{n\ell}=\Delta+2n+|\ell|,
\qquad
n\in\mathbb N_0,\qquad \ell\in\mathbb Z.
$$
...
$$
Z_{\mathrm{can}}
\propto\prod _{n=0}^{\infty}\prod _{\ell\in\mathbb Z}
\prod _{k\in\mathbb Z}
...
$$
\left[\det(-\partial _\tau^{2}+\omega^{2})\right]^{-1/2}
...
$$
Z_{\mathrm{can,th}}(\beta,\vartheta)
=\prod _{n=0}^{\infty}\prod _{\ell\in\mathbb Z}
\frac{1}{1-e^{-\beta\omega_{n\ell}+i\vartheta\ell}}.
$$
...
\begin{align}
Z_{\mathrm{can,th}}
& =\prod _{p,\bar p=0}^{\infty}
\frac{1}{1-q^{h+p}\bar q^{h+\bar p}}, \\
\log Z_{\mathrm{can,th}}
& =\sum _{k=1}^{\infty}
\frac{|q|^{k\Delta}}{k|1-q^{k}|^{2}}.
\end{align}
$$
...
$$
K_{\mathcal O}^{\mathbb H_3}(s;\chi)
=\frac{e^{-(\mu ^2+1)s-\chi^{2}/(4s)}}{(4\pi s)^{3/2}}
\frac{\chi}{\sinh\chi},
$$
...
$$
\log Z_{\mathrm{cov,th}}
=\frac{1}{2}\int _0^{\infty}\frac{\mathrm ds}{s}
\sum _{\gamma\neq 1}\int _{\mathbb H_3/\mathbb Z}\mathrm d^{3}X\sqrt g\,
K_{\mathcal O}^{\mathbb H_3}(s;X,\gamma X)
=\sum _{k=1}^{\infty}\frac{|q|^{k\Delta}}{k|1-q^{k}|^{2}}.
$$
