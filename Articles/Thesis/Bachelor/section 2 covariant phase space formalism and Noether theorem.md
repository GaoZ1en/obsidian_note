...
1. Hamilton量 $\displaystyle{H\in \Omega^{0}(\mathcal{P})}$，为相空间 $\displaystyle{\mathcal{P}}$ 上的函数
1. 正则方程(2.1)，给出相点 $\displaystyle{(q^{1},\dots,q^{n};p_{1},\dots,p_{n})}$ 的时间演化
...
1. $\displaystyle{\omega}$ 是闭的，即 $\displaystyle{\mathrm{d}\omega=0}$
