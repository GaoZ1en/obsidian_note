...
- [ ] **Operator Algebra verification (`4_quantization.md`):** Replace the `[TODO: Insert specific algebraic steps...]` block with my detailed calculation showing that $\nabla_\nu F^{\nu}_{~~\mu} = (\mathcal{D}^2 A)_\mu$ is precisely true, relying upon $\nabla_\mu A^\mu=0$ and the relationship $[\nabla_\nu, \nabla_\mu] A^\nu = R_{\mu\nu}A^\nu = -2g_{\mu\nu}A^{\nu} = -2A_\mu$.
...
- [ ] **Exceptional cases logic (`5_appendices.md`, Appendix C):** Fill out the physical discussion discussing the resonance gap when $\mu_+ - \mu_- \in \mathbb{Z}$. It is currently stubbed. Explain what happens to the descending ladders and the implications for Hilbert space completeness (such as possible log terms or overlaps).
