...
# correspondence with temporal gauge

in this section we ignore the gauge zero mode and edge modes, and compare only the normalizable propagating modes with those obtained in the temporal gauge.

the temporal-gauge spectrum obtained in `temporal gauge.md` is

$$\begin{align}
\omega_{n,m}^{\mathrm{temp}} & =2n+2+|m|, \qquad m\in \mathbb{Z}, \quad n\in \mathbb{N}
\end{align}$$
...
the Lorenz-gauge solutions above contain two sectors, $\displaystyle{\Phi_{+}}$ and $\displaystyle{\Phi_{-}}$. before quotienting by the residual gauge redundancy, they give two families of frequencies
...
however, the pairs listed in the previous section differ only by a gauge transformation, so after removing the redundant description the physical spectrum should be compared as follows.

| range of $\displaystyle{m}$    | mode in this note                  | frequency                                   | correspondence with temporal gauge                                                     |
| ------------------------------ | ---------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------- |
| $\displaystyle{m\geqslant 1}$  | $\displaystyle{h^{\mu}_{-,m,n}}$   | $\displaystyle{\omega =2+m+2n}$             | same physical mode as the temporal-gauge solution with the same $\displaystyle{(m,n)}$ |
| $\displaystyle{m\geqslant 1}$  | $\displaystyle{h^{\mu}_{+,m,n+1}}$ | $\displaystyle{\omega =m+2(n+1)=2+m+2n}$    | gauge-equivalent to $\displaystyle{h^{\mu}_{-,m,n}}$                                   |
| $\displaystyle{m=0}$           | $\displaystyle{h^{\mu}_{+,0,n}}$   | $\displaystyle{\omega =2+2n}$               | same physical mode as the temporal-gauge solution with $\displaystyle{(m,n)=(0,n)}$    |
| $\displaystyle{m=0}$           | $\displaystyle{h^{\mu}_{-,0,n}}$   | $\displaystyle{\omega =2+2n}$               | gauge-equivalent to $\displaystyle{h^{\mu}_{+,0,n}}$                                   |
| $\displaystyle{m\leqslant -1}$ | $\displaystyle{h^{\mu}_{+,m,n}}$   | $\displaystyle{\omega =2-m+2n=2+m+2n}$      | same physical mode as the temporal-gauge solution with the same $\displaystyle{(m,n)}$ |
| $\displaystyle{m\leqslant -1}$ | $\displaystyle{h^{\mu}_{-,m,n+1}}$ | $\displaystyle{\omega =-m+2(n+1)=2+ m +2n}$ | gauge-equivalent to $\displaystyle{h^{\mu}_{+,m,n}}$                                   |

therefore, after quotienting by the residual gauge redundancy and discarding the gauge zero mode and edge modes, the Lorenz-gauge analysis and the temporal-gauge analysis give the same physical spectrum:
...
in terms of conformal weights, this is also consistent with the relations listed above, since in all physical cases one has $\displaystyle{J=h-\bar{h}=m}$.
