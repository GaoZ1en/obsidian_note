...
after a similar procedure as [[Articles/Quantization in AdS3/preliminary analysis/path integral quantization for constrained system]],
...
we will start from (3.3)

$$\tag{3.3}
\begin{align}
H & =\int \mathrm{d}^{3}x\left( \frac{1}{2}\delta ^{ij}\pi _{i}\pi _{j}+\frac{1}{2m^{2}}\partial ^{i}\pi _{i}\partial ^{j}\pi _{j}+\frac{1}{4}(\partial _{i}A^{j}-\partial _{j}A^{i})^{2}+\frac{1}{2}m^{2}\delta _{ij}A^{i}A^{j} \right)
\end{align}
$$

and prove that the canonical equation of motion is equivalent to the equation of motion given by (3.1)
...
\implies \delta _{ij}\ddot{A}^{j}- & =\delta _{ij}(\vec{\nabla}^{2}-m^{2})A^{j}-\partial _{i}\partial _{j}A^{j}
\end{align}
$$
