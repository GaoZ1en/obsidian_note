...
$$\tag{1.4}
\begin{align}
X_{\varepsilon}\cdot \delta S & =\int \mathrm{d}\tau E_{x,\sigma}(X_{n}\cdot \delta x^{\sigma})+(X_{\xi}\cdot \theta)|_{\Sigma _{f}-\Sigma _{i}} \\
 & =\int \mathrm{d}\tau \left( -mg_{\lambda \sigma}\left( \frac{\mathrm{d}^{2}x^{\lambda}}{\mathrm{d}\tau ^{2}}+\Gamma ^{\lambda}_{~\mu \nu} \frac{\mathrm{d}x^{\mu}}{\mathrm{d}\tau} \frac{\mathrm{d}x^{\nu}}{\mathrm{d}\tau} \right)+qF_{\sigma \rho} \frac{\mathrm{d}x^{\rho}}{\mathrm{d}\tau} \right)\tau ^{n} \frac{\mathrm{d}x^{\sigma}}{\mathrm{d}\tau}+()
\end{align}
$$
