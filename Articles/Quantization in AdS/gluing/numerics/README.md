...
- [rectangle_self_sewing_checks.sage](<rectangle_self_sewing_checks.sage>) and [rectangle_self_sewing_checks.wl](<rectangle_self_sewing_checks.wl>): exhaustive twelve-pairing vertex-link/homology/cone-angle audit and 19 exact scalar/action/group/mode/trace checks for [rectangle self-sewing](<Articles/Quantization in AdS/gluing/models/spacelike regions/non-gauge/rectangle scalar self-gluing.md>). [Results](<rectangle_self_sewing_results.json>) retain the executed output and setup correction; continuum form-domain and algebra claims have separate proof arguments in the note.
