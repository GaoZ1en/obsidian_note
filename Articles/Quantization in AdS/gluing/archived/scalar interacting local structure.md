...
日期：2026-09-07。模型和区域输入沿用 [自由局域结构实现](<Articles/Quantization in AdS/gluing/archived/scalar local structure implementation.md>)；interaction 为

$$\begin{align}
V=-\frac\lambda{4!}\sum_i\int g_i\phi_i^4, \qquad L_0\phi+\frac{\lambda g}{6}\phi^3=0.
\end{align}$$

$g_i$ 实、按 collar 匹配全部 jets、时间支撑紧，保留早期自由窗。本笔记给出实际经典解域上的传播／动力学、原有限插入量子类上的全阶包含／因果性／有限组合，以及指定二阶诊断。**原有限插入类的 time-slice 已有 [反例](<Articles/Quantization in AdS/gluing/archived/scalar insertion time-slice obstruction.md>)；本笔记不将它标为成立。**

## 1. 实际经典解域与背景依赖

使用 [closure §4](<Articles/Quantization in AdS/gluing/archived/scalar observable sewing closure.md>) 的 $C_t^\infty H^1$ 区域域：固定低阶输入界 $M$，收小
