...
从区域二次型及全部配边关系构造 $K_s$、$E_s$、$X_s^\infty$，步骤与 [rectangle §§5–5.2](<Articles/Quantization in AdS/gluing/models/spacelike regions/non-gauge/rectangle scalar self-gluing.md>) 相同。这里只用区域 trace matching；独立整体算子稍后才引入。记 $r_i$ 为场的逐 cell 限制、$e_i$ 为 $L^2$ 源的零延拓，定义
