...
日期：2026-09-07。本文给出 [有限插入 time-slice 反例](<Articles/Quantization in AdS/gluing/archived/scalar insertion time-slice obstruction.md>) 后的**另一份明确目标**：增加 coefficient distribution topology、多插入 coefficients 和显式 $\hbar$ saturation。模型、区域作用量、joint histories、response、interaction 与局域 insertion labels 均保留；改变的是生成操作与完成。
...
沿用 [自由实现](<Articles/Quantization in AdS/gluing/archived/scalar local structure implementation.md>) 的 smooth torus $M_s$ 和已构造的 on-shell polynomial coefficient space
...
外部 switching 的明确过去／未来分割给 [弱耦合笔记 §4](<Articles/Quantization in AdS/gluing/archived/scalar interacting local structure.md>) 的 $S_V(v_-)$ conjugation；现在它及 inverse 与新增完成相容。共同分割下的 $S_V(a)S_{V+a}(b)=S_V(a+b)$ 给复合；不同分割只在相关局域代数上比较。
...
实际经典 switched energy、背景 Peierls derivative 和自由 quantum energy extension 保持 [弱耦合笔记](<Articles/Quantization in AdS/gluing/archived/scalar interacting local structure.md>) 的原式。量子全阶 time-slice 的上述证明只需 causal factorization 和 coefficient control，不从 energy conservation 推出；也未新增全量子 stress Ward identity。
