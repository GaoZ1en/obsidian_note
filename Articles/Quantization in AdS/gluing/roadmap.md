...
- **Current bounded spatial-gauge test: conditional.** `maxwell-2p1-spatial-gluing-kill-test/` isolates second-order Maxwell connections on the fixed cylinder. Distributional sewing and a smooth mixed realization pass; raw weak assembly modulo smooth proper gauge fails stronger slice/kernel claims. An explicit Coulomb-restored core construction has orbit/CPS proofs, but its identification with geometric assembly remains open. This does not reopen 3+1, non-Abelian higher-dimensional, charged-matter, first-order, or null branches.
