...
Thus the classical energy dynamics, interface trace, symplectic form, and intrinsic smeared Gaussian state all have the required continuum limit. The proof, domain statements, and regional correlator formulas are in [[Articles/Quantization in AdS/gluing/state and representation/functional analysis proof of well-definedness of finite truncation|functional analysis proof of well-definedness of finite truncation]]. The corresponding finite partial-trace-to-continuum-state-restriction statement is in [[Articles/Quantization in AdS/gluing/state and representation/reduced-state-open-system-scalar|reduced-state-open-system-scalar]].
