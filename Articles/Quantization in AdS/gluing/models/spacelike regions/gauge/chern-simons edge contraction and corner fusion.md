...
| [U(1) junction draft](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/U(1) Chern-Simons trivalent junction sewing.md>)、[Wilson/vertex draft](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/U(1) Chern-Simons Wilson and vertex extension.md>) | junction gauge jets 与 dynamical sector 分开；sharp collision 需单独 prescription。本文没有审定其全部 extension/renormalization theorem |
...
4. **Charged puncture bridge。** [Classical bridge](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/SU(2) Chern-Simons two-puncture classical bridge.md>) 已给 regular two-puncture model 的 actual current/holonomy maps 与 corrected two-form；[charged sewing](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/SU(2) Chern-Simons charged sewing and channel maps.md>) 已在指定 integrable prescription 下给 $k=2$ 的两个实际 channel maps、完整 Gram kernel、Wilson detector 和四点 $F$-move。它们仍不识别任意 physical geometric corners，也不从 classical residues 自动推出量子 labels。
...
| 工具与可复现输入 | 执行结果 | 证据范围 |
|---|---|---|
| [Mathematica 检查](<Articles/Quantization in AdS/gluing/numerics/cs_edge_contraction_checks.wl>) | 8 组 residual 为零 | cutoff energy、harmonic solution/boundary/response、response energy、SU(2) trace 与 transport、两输出反例、fusion contraction、CS cubic variation |
| [xAct 检查](<Articles/Quantization in AdS/gluing/numerics/cs_edge_contraction_xact.wl>) | 2 组 residual 为零；full pipeline 包含 xTras FullSimplification | CS quadratic variation 的 bulk/boundary 符号；由 Maurer–Cartan 与 invariant alternating tensor 表示的 Cartan 3-form product identity |
| [Sage 检查](<Articles/Quantization in AdS/gluing/numerics/cs_edge_contraction_checks.sage>) | 6 组精确 Boolean checks 全通过 | $S_3$ 的 216 份 triangle assignments；free-group transport/loop identities；$k=1,\ldots,6$ 的 139 个 SU(2) character-quotient products；level-1 通道 |
...
**Not verified:** Scalar/YM 的一般 shrinking-domain 极限；有物理 currents 的任意 corner collapse；任意 singular CS target；任意 geometric charged-corner 到 puncture/holonomy-port identification；超出指定 fundamental integrable prescription 的 Wilson kernel；sharp operator collision；state restriction、partial trace 或 entropy。§9.1 的新增 classical/quantum maps 与检查范围另见 [charged bridge ledger](<Articles/Quantization in AdS/gluing/numerics/charged CS bridge claim ledger.md>)。
