...
几何为 [classical bridge](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/SU(2) Chern-Simons two-puncture classical bridge.md>) 的两个分离 Wilson punctures，cut 避开 worldlines。该 note 的 classical orbit parameters 不由本篇的 $j$ 值反向定义；这里增加量子化处方，而非证明一切 continuum CS 量子化都等于此处方。
...
**Verified:** fundamental compressed fields及其合法 tests／Ward identity；transport-normalized dense channel maps、Gram kernel、onto 和全部 local-net comparison；两通道 Wilson detector；上述 duality／framing 下的 $F$、braid 和非对易 channel operators。外部定理的模型适用关系及实际 maps 均已写出；有限检查见 [本轮 ledger](<Articles/Quantization in AdS/gluing/numerics/charged CS bridge claim ledger.md>)。
