...
指定 $SU(2)_2$ 两个／四个 spin-$1/2$ punctures 的 non-vacuum maps、bounded primary domains、Wilson 和 $F$ comparison 已另见 [charged sewing](<Articles/Quantization in AdS/gluing/models/spacelike regions/gauge/SU(2) Chern-Simons charged sewing and channel maps.md>)；该扩展使用独立的 transport proof，不由本篇的 tensor-unit argument 推出。
