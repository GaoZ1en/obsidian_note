...
**Boundary:** existence/uniqueness is proved separately on each fixed finite angular support, while full cut-history equivariance belongs to the union over finite supports: an arbitrary finite-Fourier cut history can enlarge support, so no fixed-$N$ invariance or inductive-limit topology is claimed. $H_i^{\rm pol}$ is a boundary-polarized regional energy/fixed-history-fiber Hamiltonian representative. Under $\alpha_i\mapsto\alpha_i+\dot f$ it shifts by $e^{-2}\int_SE_{y,i}\dot f$; transmission cancels the two shifts, and only the matched sum is the intrinsic connected Hamiltonian. Kernel equality uses the fully compatible smooth nondegenerate-mod-gauge tangent core; the global temporal/Coulomb tests are only a proof device, the Wilson/electric pair remains, and discrete large gauge is not a tangent vector.

The cusp parameter is Lipschitz/$W^{1,\infty}$; its raw obstruction is specific to globally smooth gauge. The time-slab relation is exact isotropic on admissible differentiable families, not proved Lagrangian/maximal or a smooth infinite-dimensional canonical relation. No identical Sobolev obstruction, functional quotient smoothness, density theorem, energy completion, other-sector, or quantum theorem is claimed. The bounded spatial branch is closed; the general spatial-gauge formalism remains conditional. Preserve this scope and both regression obstructions in any subsequent review.
