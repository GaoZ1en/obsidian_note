...
are the annihilation operators, which contain additional terms linear in $\displaystyle{q}$ and $\displaystyle{\dot{q}}$. See feedback-port-canonical-bridge.md and numerics/feedback_port_canonical_bridge_checks.wl.
...
Thus the classical energy dynamics, interface trace, symplectic form, and intrinsic smeared Gaussian state all have the required continuum limit. The proof, domain statements, and regional correlator formulas are in continuum-closure-theorem.md.
