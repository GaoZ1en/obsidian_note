...
- `history-indexed observable algebra gluing.md`: boundary-controlled affine CCR systems with an explicit source quotient/local net, bulk-source/history transports and smeared response sections. It recovers the regional response matrix from those quantum structures, defines causal interface elimination and CCR reconstruction, proves two-interval cross/same-side corrections and full-energy assembly, and constructs a three-interval intrinsic associator with a two-port middle region. Bare field-preserving fusion of fixed-history on-shell algebras is a no-go; crossed-product/cotensor/Connes realizations remain future representation candidates. Smooth response charts and the full-energy Weyl category have separate proofs and claim boundaries.
