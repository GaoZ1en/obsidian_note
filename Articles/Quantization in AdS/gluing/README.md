...
- `classical regional observable algebra gluing.md`: intrinsic classical observable definitions for static free scalar intervals. A uniform history-family class controls mixed derivatives, smooth history covectors and finite Sobolev loss; it contains smeared composites, suitable energies and boundary responses. Full-derivative release has a continuous section and reconstructs the entire independently defined global class. Each layer and the LF limit are complete, with continuous operations between layers; explicit counterexamples rule out joint multiplication/bracket continuity on the full limit and the enlarged class's canonical independent projective tensor identity. Bare fixed-history fusion remains excluded.
