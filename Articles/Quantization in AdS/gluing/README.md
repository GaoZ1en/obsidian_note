...
- `maxwell-2p1-spatial-gluing-kill-test/`: separate classical spatial-gluing test with explicit mixed Coulomb traces, distributional sewing, harmonic restoration, extension/arrows and original CPS. Overall `conditional`: the raw smooth-gauge comparison fails; an explicit restored core construction passes its stated orbit/CPS tests, with geometric assembly identification still open.
...
The separate Maxwell 2+1 spatial-gluing dossier now proves physical distributional patching and a mixed realization on a smooth finite-Fourier core. It distinguishes raw weak assembly modulo smooth proper gauge, whose slice/restriction/kernel claims fail, from explicit Coulomb-restored assembly with regional based re-fixing, whose set-level orbit and CPS result is derived. The general geometric/functional identification remains conditional; the project is not marked complete by the 30/30 new witnesses or the 20/20 benchmark regression.
