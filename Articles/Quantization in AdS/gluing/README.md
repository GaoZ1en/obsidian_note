...
- `spatial gauge gluing audit.md`: first-pass A--O audit of the spatial-subregion gauge branch, separating exact (1+1) reductions and higher-dimensional Maxwell mode checks from the missing gauge-slice, bulk-unfixing, extension-image, weak-patching, CPS, observable-algebra, and BRST/BV theorems.
- `spatial gauge gluing formalism.md`: first-draft gauge layer built on the full-trace/relation/reconstruction language of `gluing formalism.md`, with explicit hyperbolic gauge realization, collar matching, global gauge saturation, extendable cut group, theorem targets, and model claim table. Its general statements remain conditional.
