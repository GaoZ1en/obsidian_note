...
where $q_i$ is the artificial-cut port value and $u_n$ are the strict half-interval Dirichlet modes. The interconnection $q_1=q_2=q$ together with the flux balance $p_1+p_2=0$ (obtained from the variation of $q$) reduces the feedback spectrum to the truncated regional Dirichlet-to-Neumann map

$$\begin{align}
D_N(\omega)=\frac1L+\frac2L\left(m^2-\omega^2\right)\sum_{n=1}^{N}\frac{1}{\omega_n^2-\omega^2}, \qquad \omega_n^2=m^2+\left(\frac{n\pi}{L}\right)^2,
\end{align}$$

using the exact identity $A_n u_n'(L)=-2/L$. The finite feedback spectrum is the union of the $N$ zeros of $D_N$ and the $N$ poles $\omega_n$: the even global modes $r=2n$ (wave numbers $K_{2n}=n\pi/L$) are exactly the poles, with $q=0$ and $c_1=-c_2=e_n$; the odd modes are the zeros. Verified:

1. **Even tower exact.** For every $N\ge n$, the mode $r=2n$ is an exact eigenvector of the Galerkin pencil with residual below $10^{-30}$.
2. **Odd tower $O(1/N)$.** The maximum relative errors among the first eight global frequencies are

| $N$ | 8 | 16 | 32 | 64 |
|---:|---:|---:|---:|---:|
| Route A (DtN feedback) | $2.5151\times10^{-2}$ | $1.2445\times10^{-2}$ | $6.2371\times10^{-3}$ | $3.1276\times10^{-3}$ |

These agree with the hard-constraint $\kappa\to\infty$ errors of the penalty construction recorded in the Neumann baseline table above, and $D_N\to k\cot(kL)$ pointwise with $O(1/N)$ decay. There is no bare interface coupling to match: the boundary-driven feedback is the hard endpoint itself, and the even tower is exact by structure rather than by tuning.

3. **Galerkin cross-check.** Promoting $q$ to a coordinate in the recovered global variational problem (Sec. 9 of the note) gives the pencil $K_Nv=\omega^2G_Nv$; its $2N$ low modes converge to the same tower with odd-tower errors $1.486\times10^{-3}$, $1.875\times10^{-4}$, $2.406\times10^{-5}$, and $3.063\times10^{-6}$ at $N=8,16,32,64$, but it contains one extra port mode growing linearly with $N$ ($32.0$, $62.2$, $122.5$, $243.1$) that has no continuum counterpart. The port/feedback truncation is therefore the clean finite model: no spurious mode and an exact even tower.

Exact claim tier: fixed-window spectral convergence of the feedback truncation and finite-matrix even-tower exactness. Not claimed: uniform convergence up to the cutoff, Fock-space equivalence, or any interacting statement.
