...
- Do not use a timelike interface response function as the definition of null gluing. A null interface carries characteristic data and a direct null symplectic flux; compose it through the Goursat/characteristic problem with its corner compatibility data. A timelike stretched horizon is an optional regulator/comparison route when one specifically wants to reuse the timelike machinery, and its removal must then be controlled.
...
- For AdS--Rindler work, do not treat the two Lorentzian exteriors as a complete global decomposition or impose pointwise horizon equality as gluing. The completed scalar branch treats the exteriors as causal complements, uses their horizon outputs as the intersecting characteristic inputs for the future/past regions, and retains the bifurcation cut only as corner compatibility data. Keep intrinsic wedge KMS/Unruh statements, global reconstruction, B2 state/cross-covariance data, and Fock-space factorization as separate claims.
