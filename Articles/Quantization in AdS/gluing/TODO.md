...
### Continuous-Spectrum Stress Test, To Be Completed First
...
- [ ] Action: fix the AdS--Rindler geometric target before defining any gluing there.
  Context: the two Lorentzian exteriors omit the future and past regions and do not by themselves form a global decomposition. A null interface requires characteristic data and a different symplectic structure and should not be attacked with an interface response function; the stretched-horizon route keeps the interface timelike instead.
  Check: state which region is reconstructed, which surface carries the initial data, and which interface condition makes the classical evolution well posed. Do not use pointwise equality across the Killing horizon as a substitute for this step, and keep intrinsic wedge KMS statements, global reconstruction, algebraic gluing, and Fock equivalence as four separate claims.

- [ ] Action: build the stretched-horizon regulator and study its removal.
  Context: placing a timelike stretched horizon at $\rho=\epsilon$ keeps the interface timelike, so the Phase 1 machinery applies at fixed $\epsilon$, and it isolates the horizon limit as a single controlled parameter instead of a change of interface causal type. Horizon traces, infinite redshift, and the continuous boost spectrum may obstruct the identifications that are already nontrivial for the interval scalar.
...
- [ ] Action: redo the free-scalar wedge quantization in the interface language.
  Context: the preserved AdS--Rindler calculations provide real-frequency Klein--Gordon-normalized modes, left and right boost generators, Unruh combinations and KMS checks, but they do not establish sharp-wedge factorization or global-to-wedge Fock equivalence.
  Check: retain both horizon branches, track the opposite future-time orientation of the two wedges, reconstruct the appropriate global Cauchy data, and verify the Bogoliubov and KMS relations at fixed stretched-horizon regulator.
