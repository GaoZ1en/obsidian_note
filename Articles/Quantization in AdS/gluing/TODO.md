...
- [x] Action: extract the compact internal gauge-covariant sewing/reduction layer and solve the \(1+1\) compact Yang--Mills benchmark.
  Context: [[Articles/Quantization in AdS/gluing/gauge-covariant sewing and reduction|gauge-covariant sewing and reduction]] separates based regional proper gauge, boundary-frame covariance, and glued diagonal proper gauge; derives non-Abelian transition-function sewing; and keeps the full timelike transmission relation distinct from the Cauchy-cut Gauss moment map. [[Articles/Quantization in AdS/gluing/Yang-Mills 1+1|Yang-Mills 1+1]] supplies the exact non-Abelian test.
  Check: completed at relation level for the general note and exactly for the finite \(1+1\) model: \(T^*G\) regional reduction, \((T^*G\times T^*G)//G_\Delta\simeq T^*G\), normalized-Haar averaging, Peter--Weyl/Casimir spectrum, direct uncut comparison, and finite three-cell reduction by stages. This does not complete a general gauge IBVP or continuum \(\operatorname{Map}(S,G)\) Hilbert reduction.

- [ ] Action: close the analytic gates for the first higher-dimensional non-Abelian Yang--Mills benchmark before computing its mode content.
