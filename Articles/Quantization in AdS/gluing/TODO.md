...
- [x] Action: repair the local formula and wording issues in `formalism.md` using the direct language of prescribed boundary histories, allowed variations, continuity, and flux matching.
...
- [x] Action: construct a finite-cutoff reduced symplectic model for canonical quantum gluing.
...
- [x] Action: construct the finite-$N$ global vacuum from the two regional Gaussian states.
...
- [x] Action: compute finite-cutoff vacuum diagnostics in the regional representation.
...
- [x] Action: compute the reduced regional Gaussian states and their entanglement spectra.
...
- [x] Action: determine whether the finite-cutoff quantum equivalences have a continuum Fock-space limit.
...
- [x] Action: repeat the construction for an off-center cut $[-L_1,0]\cup[0,L_2]$ with $L_1\neq L_2$.
  ...
  Result: the undivided system recovers the full connected spectrum and common-pole $q=0$ modes. A modewise $L^2$-normalized no-parity regulator gives a symplectic map, pure Gaussian covariance, and fixed-window correlators converging to the same uncut answer for three cut positions. The test also shows that the earlier centered quantum calculation used a hybrid symplectic normalization which is not the centered limit of the off-center gradient symmetrizer; its vacuum diagnostics remain regulator-specific.

- [x] Action: prove independence of the lifting function used in the regional decomposition.
...
- [x] Action: test associativity by cutting the interval into three subregions.
...
- [x] Action: generalize the construction to a massive scalar on $[-L,L]\times S^1_R$ with the cut at $x=0$.
...
- [x] Action: solve $1+1$ dimensional Maxwell theory on a cut interval as the minimal gauge-theory gluing model.
...
- [x] Action: extend Maxwell gluing to $[-L,L]\times S^1_R$.
...
- [x] Action: fix the geometric target before defining AdS--Rindler gluing.
...
- [x] Action: redo the free-scalar wedge quantization in the active boundary-feedback language.
...
- [x] Action: study the removal of the AdS--Rindler regulator before extending to gauge fields or gravity.
