...
$$\begin{align}
\text{finite glued Hamiltonian}
&\longrightarrow \text{exact Gaussian ground state}\nonumber\\
...
u_n(y)&=\sqrt{\frac2L}\sin(k_ny),&
A_n&=(-1)^{n+1}\frac{\sqrt{2L}}{n\pi},&h(y)&=\frac yL.
\end{align}$$

Both local coordinates $y_i\in[0,L]$ increase from the physical outer boundary toward the cut. Therefore

$$\begin{align}
\partial_x&=+\partial_{y_1}\quad\text{on region 1},&
\partial_x&=-\partial_{y_2}\quad\text{on region 2},
\end{align}$$
...
D_0&=\operatorname{diag}(\omega_1^2,\ldots,\omega_N^2),\\
\boldsymbol b&=(A_1k_1^2,\ldots,A_Nk_N^2)^{\mathrm T},&
\boldsymbol d&=(\boldsymbol A,\boldsymbol A)^{\mathrm T},&
\boldsymbol g&=(\boldsymbol b,\boldsymbol b)^{\mathrm T}.
\end{align}$$
...
\boldsymbol C&=\boldsymbol c+\boldsymbol d q,&
\boldsymbol V&=\dot{\boldsymbol C},\\
\rho_N&=\frac{2L}{3}-\boldsymbol d^{\mathrm T}\boldsymbol d
=\frac{4L}{\pi^2}\sum_{n>N}\frac1{n^2}>0,&
\pi_q&=\rho_N\dot q.
\end{align}$$
...
$$\begin{align}
L_N={}&\frac12\left(
\dot{\boldsymbol c}^{\mathrm T}\dot{\boldsymbol c}
+2\dot q\,\boldsymbol d^{\mathrm T}\dot{\boldsymbol c}
+\frac{2L}{3}\dot q^2\right)\nonumber\\
&-\frac12\left[
\boldsymbol c^{\mathrm T}D\boldsymbol c
+2m^2q\,\boldsymbol d^{\mathrm T}\boldsymbol c
+\left(\frac2L+\frac{2m^2L}{3}\right)q^2\right],
\end{align}$$

with $D=D_0\oplus D_0$. Directly substituting $\boldsymbol c=\boldsymbol C-\boldsymbol d q$ gives

$$\begin{align}
L_N={}&\frac12\boldsymbol V^{\mathrm T}\boldsymbol V
+\frac{\rho_N}{2}\dot q^2
...
$$\begin{align}
\kappa_N&=\frac2L+\boldsymbol d^{\mathrm T}
\operatorname{diag}(k_1^2,\ldots,k_N^2,k_1^2,\ldots,k_N^2)
\boldsymbol d+m^2\rho_N\nonumber\\
...
\omega_N&=\delta\boldsymbol V^{\mathrm T}\wedge\delta\boldsymbol C
+\delta\pi_q\wedge\delta q.
\end{align}$$
...
$$\begin{align}
[C_{i,n},V_{j,m}]&=\mathrm i\delta_{ij}\delta_{nm},&
[q,\pi_q]&=\mathrm i,
\end{align}$$
...
$$\begin{align}
\boldsymbol C_\pm&=\frac{\boldsymbol C_1\pm\boldsymbol C_2}{\sqrt2},&
\boldsymbol V_\pm&=\frac{\boldsymbol V_1\pm\boldsymbol V_2}{\sqrt2}.
\end{align}$$
...
$$\begin{align}
M_{+q}&=\begin{pmatrix}\mathbf1_N&0\\0&\rho_N\end{pmatrix},&
...
$$\begin{align}
\kappa_N-2\boldsymbol b^{\mathrm T}D_0^{-1}\boldsymbol b
&=\frac2L+m^2\rho_N
+\frac{4m^2}{L}\sum_{n=1}^N\frac1{k_n^2+m^2}>0.
\end{align}$$
...
$$\begin{align}
R_N'(z)&=-\rho_N-2\sum_{n=1}^N
\frac{A_n^2k_n^4}{(k_n^2-z)^2}<0.
\end{align}$$
...
$$\begin{align}
(0,k_1^2),\ (k_1^2,k_2^2),\ldots,
(k_{N-1}^2,k_N^2),\ (k_N^2,\infty).
\end{align}$$
...
$$\begin{align}
\mathcal S_N&=\mathcal S_{1,N}^{\mathrm{int}}
\oplus^\sigma\mathcal S_{2,N}^{\mathrm{int}}
\oplus^\sigma\mathcal S_{\Gamma,N},
\end{align}$$

generated respectively by

$$\begin{align}
(\boldsymbol C_1,\boldsymbol V_1),\qquad
(\boldsymbol C_2,\boldsymbol V_2),\qquad
(q,\pi_q).
\end{align}$$
...
$$\begin{align}
\mathcal H_N&\simeq
\mathcal H_{1,N}^{\mathrm{int}}\otimes
\mathcal H_{2,N}^{\mathrm{int}}\otimes
\mathcal H_{\Gamma,N},\\
\mathcal H_{i,N}^{\mathrm{int}}&=L^2(\mathbb R^N),&
\mathcal H_{\Gamma,N}&=L^2(\mathbb R).
\end{align}$$
...
$$\begin{align}
\mathfrak A_{1,N}^{\mathrm{int}}
&=B(\mathcal H_{1,N}^{\mathrm{int}})\otimes\mathbf1,
\end{align}$$
...
$$\begin{align}
\rho_{1,N}^{\mathrm{int}}
&=\operatorname{Tr}_{2,\Gamma}\rho_N^{\mathrm{gl}}.
\end{align}$$
...
\Pi_{1,N}(g)&=\sum_{n=1}^N(g,u_n)V_{1,n}
+\frac{(g,e_N)}{\rho_N}\pi_q.
\end{align}$$
...
$$\begin{align}
\pi_N\!\left(\mathcal W(\mathcal S_{1\Gamma,N},\sigma)\right)''
&=B(\mathcal H_{1,N}^{\mathrm{int}}\otimes\mathcal H_{\Gamma,N})
\end{align}$$
...
$$\begin{align}
\mathcal H_N&=\mathcal H_{1,N}^{\mathrm{int}}\otimes
\mathcal H_{2,N}^{\mathrm{int}}\otimes\mathcal H_{\Gamma,N},\nonumber\\
\mathfrak A_{1\Gamma,N}
&=B(\mathcal H_{1,N}^{\mathrm{int}})\,\bar\otimes\,
\mathbf1_2\,\bar\otimes\,B(\mathcal H_{\Gamma,N}),\\
\mathfrak A_{2\Gamma,N}
&=\mathbf1_1\,\bar\otimes\,
B(\mathcal H_{2,N}^{\mathrm{int}})\,\bar\otimes\,
...
$$\begin{align}
\mathfrak A_{1,N}^{\mathrm{int}}
&=B(\mathcal H_{1,N}^{\mathrm{int}})\bar\otimes\mathbf1_2
\bar\otimes\mathbf1_\Gamma,\\
\mathfrak A_{2,N}^{\mathrm{int}}
&=\mathbf1_1\bar\otimes B(\mathcal H_{2,N}^{\mathrm{int}})
\bar\otimes\mathbf1_\Gamma
\end{align}$$
...
$$\begin{align}
(1+\Gamma)|2:&\qquad
\rho_{1\Gamma,N}=\operatorname{Tr}_2\rho_N^{\mathrm{gl}},\\
1|(2+\Gamma):&\qquad
...
$$\begin{align}
\rho_{2\Gamma,N}&=\operatorname{Tr}_1\rho_N^{\mathrm{gl}},&
\rho_{\Gamma,N}&=\operatorname{Tr}_{1,2}\rho_N^{\mathrm{gl}},
\end{align}$$

then

$$\begin{align}
\operatorname{Tr}_1\rho_{1\Gamma,N}
=\rho_{\Gamma,N}
=\operatorname{Tr}_2\rho_{2\Gamma,N}.
\end{align}$$
...
$$\begin{align}
X&=(\boldsymbol C_1,\boldsymbol C_2,q)^{\mathrm T},&
P&=(\boldsymbol V_1,\boldsymbol V_2,\pi_q)^{\mathrm T}.
\end{align}$$
...
$$\begin{align}
K_CZ_N&=M_CZ_N\Omega_N^2,&
Z_N^{\mathrm T}M_CZ_N&=\mathbf1.
\end{align}$$
...
$$\begin{align}
\langle XX^{\mathrm T}\rangle
&=Z_N\frac1{2\Omega_N}Z_N^{\mathrm T},\\
\langle PP^{\mathrm T}\rangle
&=M_CZ_N\frac{\Omega_N}{2}Z_N^{\mathrm T}M_C.
\end{align}$$

These expressions are reproducible without choosing a regional reference vacuum. They obey

$$\begin{align}
\langle XX^{\mathrm T}\rangle\langle PP^{\mathrm T}\rangle
&=\frac14\mathbf1_{2N+1},
\end{align}$$
...
$$\begin{align}
\{\nu_{A,j}^2\}&=\operatorname{spec}
\left(\langle X_AX_A^{\mathrm T}\rangle
\langle P_AP_A^{\mathrm T}\rangle\right).
\end{align}$$
...
G_A&=S_A^{-\mathrm T}\operatorname{diag}
(\epsilon_{A,1},\ldots,\epsilon_{A,r},
\epsilon_{A,1},\ldots,\epsilon_{A,r})S_A^{-1}.
\end{align}$$
...
$$\begin{align}
\mathsf V_{\Gamma,N}
&=S_{\Gamma,N}\mathsf V_NS_{\Gamma,N}^{\mathrm T}
=\begin{pmatrix}
\langle q^2\rangle&0\\0&\langle\pi_q^2\rangle
\end{pmatrix},\\
\nu_{\Gamma}(N)&=\sqrt{\det\mathsf V_{\Gamma,N}}
=\sqrt{\langle q^2\rangle\langle\pi_q^2\rangle},\\
S_{\Gamma}(N)&=s(\nu_\Gamma(N)),
\end{align}$$
...
$$\begin{align}
X_{A,N}^{1/2}P_{A,N}X_{A,N}^{1/2}
&=U_N\operatorname{diag}(\boldsymbol\nu_N^2)U_N^{\mathrm T},\\
A_N&=X_{A,N}^{1/2}U_N
\operatorname{diag}(\boldsymbol\nu_N^{-1/2}).
\end{align}$$
...
$$\begin{align}
J_A&=\begin{pmatrix}0&\mathbf1\\-\mathbf1&0\end{pmatrix},
\end{align}$$
...
$$\begin{align}
(J_AG_{A,N})\mathsf V_{A,N}
+\mathsf V_{A,N}(J_AG_{A,N})^{\mathrm T}&=0,
\end{align}$$
...
$$\begin{align}
\Phi_{1,N}^{\mathrm{int}}(f)&=\sum_{n\leq N}f_nC_{1,n},&
\Pi_{1,N}^{\mathrm{int}}(g)&=\sum_{n\leq N}g_nV_{1,n},\\
f_n&=(f,u_n),&g_n&=(g,u_n).
\end{align}$$
...
$$\begin{align}
\delta_N\!\left(\Phi_{1,N}^{\mathrm{int}}(f)\right)
&=(G_{P,N}\boldsymbol f)^{\mathrm T}\boldsymbol V_1,\\
\delta_N\!\left(\Pi_{1,N}^{\mathrm{int}}(g)\right)
&=-(G_{X,N}\boldsymbol g)^{\mathrm T}\boldsymbol C_1.
\end{align}$$
...
$$\begin{align}
f(y)&\propto(y-\tfrac14)^3(\tfrac34-y)^3
\mathbf1_{[1/4,3/4]}(y),\\
g(y)&\propto(y-\tfrac15)^3(\tfrac23-y)^3
\mathbf1_{[1/5,2/3]}(y).
\end{align}$$
...
$$\begin{align}
\omega_N\!\left(O_{a,N}\,\delta_N(O_{b,N})\right),
\qquad
\omega_N\!\left([O_{a,N},\delta_N(O_{b,N})]\right)
\end{align}$$
...
\rho_N\ddot q+\kappa_Nq
-\boldsymbol b^{\mathrm T}(\boldsymbol C_1+\boldsymbol C_2)&=0.
\end{align}$$
...
$$\begin{align}
\Omega_0&=D_0^{1/2},&
G_B^R(t)&=\theta(t)\Omega_0^{-1}\sin(\Omega_0t).
\end{align}$$

For initial time $t_0$, the exact solution of the second equation is

$$\begin{align}
\boldsymbol C_2(t)&=\boldsymbol C_2^{\mathrm h}(t)
+\int_{t_0}^{t}\mathrm ds\,G_B^R(t-s)\boldsymbol bq(s),\\
\boldsymbol C_2^{\mathrm h}(t)&=\cos[\Omega_0(t-t_0)]\boldsymbol C_2(t_0)
+\Omega_0^{-1}\sin[\Omega_0(t-t_0)]\boldsymbol V_2(t_0).
\end{align}$$
...
$$\begin{align}
[\xi_{B,N}(t),\xi_{B,N}(s)]
&=-\mathrm i\boldsymbol b^{\mathrm T}\Omega_0^{-1}
\sin[\Omega_0(t-s)]\boldsymbol b\\
...
$$\begin{align}
H_B^0&=\frac12\boldsymbol V_2^{\mathrm T}\boldsymbol V_2
+\frac12\boldsymbol C_2^{\mathrm T}D_0\boldsymbol C_2
\end{align}$$
...
$$\begin{align}
C_0(t)&=\cos[\Omega_0(t-t_0)],&
G_0(t)&=\Omega_0^{-1}\sin[\Omega_0(t-t_0)].
\end{align}$$

Then

$$\begin{align}
N_{B,N}^{\mathrm{gl}}(t,s)
&=\boldsymbol b^{\mathrm T}\left[
C_0(t)X_{22}C_0(s)+G_0(t)P_{22}G_0(s)
\right]\boldsymbol b.
\end{align}$$
...
\mathcal F_N&=\phi_1'(L)+\phi_2'(L)
=\frac{2q}{L}-\boldsymbol b^{\mathrm T}(\boldsymbol c_1+\boldsymbol c_2).
\end{align}$$
...
$$\begin{align}
\ddot{\boldsymbol c}_2+D_0\boldsymbol c_2
&=-\boldsymbol A(\ddot q+m^2q).
\end{align}$$
...
$$\begin{align}
\boldsymbol c_2^{\mathrm h}(t)
-\int_{t_0}^{t}\mathrm ds\,G_B^R(t-s)\boldsymbol A
...
$$\begin{align}
\kappa_N-\rho_N(m^2+z)
-2\sum_{n=1}^N\frac{b_n^2}{k_n^2-z},
\end{align}$$
...
$$\begin{align}
\frac2L-\rho_Nz
-2z\sum_{n=1}^N\frac{A_n^2k_n^2}{k_n^2-z}.
\end{align}$$
...
$$\begin{align}
\mathcal E_N:\rho_S\longmapsto\rho_{SB},&
\operatorname{Tr}_B\mathcal E_N(\rho_S)&=\rho_S,
\end{align}$$

and then

$$\begin{align}
\Phi_{N,t}(\rho_S)&=\operatorname{Tr}_B\left[
U_N(t)\mathcal E_N(\rho_S)U_N(t)^\dagger
\right].
\end{align}$$
...
$$\begin{align}
\frac12(P_Nf,A_N^{-1/2}e^{-\mathrm i\tau A_N^{1/2}}P_Ng)_H
\longrightarrow
...
$$\begin{align}
\omega_N^{\mathrm{gl}}
\left(W_N(z_{1,N})\cdots W_N(z_{r,N})\right)
\longrightarrow
\omega_{\mathrm{conn}}
\left(W(z_1)\cdots W(z_r)\right).
\end{align}$$
...
$$\begin{align}
\phi_{1,N}&=\sum_{n\le N}C_{1,n}u_n+qe_N,&
2\|e_N\|_{L^2}^2&=\rho_N\longrightarrow0.
\end{align}$$
