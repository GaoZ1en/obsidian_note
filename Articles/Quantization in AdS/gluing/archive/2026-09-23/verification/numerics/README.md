...
- [ads2_canonical_modular_mechanism_checks.wl](<ads2_canonical_modular_mechanism_checks.wl>): 15 exact groups for the massive $\Delta=2$ global modes, boost charge/ladder, radial modes, canonical origin mode sum and KMS/Williamson identities, with two global mode-sum comparisons and three spectral covariance reconstructions with tail bounds. [Mechanism and scope](<../../04-observable-algebra/states-and-representations/global AdS2 canonical vacuum and modular flow.md>) keeps the analytic KMS argument separate from the numerical normalization checks and the unproved full regulator modular-flow limit.

- [scalar_spacelike_gluing_checks.wl](<scalar_spacelike_gluing_checks.wl>): 18 exact groups for the $t=0$ action/Cauchy response, symplectic form, mismatch matrix, interface brackets and composition, plus three smooth massive crossing brackets compared with independent global time integrals. [Model and scope](<../../03-geometric-cuts-and-gauge-data/spacelike/1+1 massive scalar spacelike gluing.md>).

- [scalar_null_gluing_checks.wl](<scalar_null_gluing_checks.wl>): exact null action, Green-identity, energy-flux and Bessel/Laplace checks, plus four massive characteristic-interface integrals compared with independent finite-cavity image sums, retaining wavefront contact terms. Scope and values are recorded in [the null scalar model](<../../03-geometric-cuts-and-gauge-data/null-and-rindler/1+1 massive scalar null gluing.md>).

- [scalar_double_null_gluing_checks.wl](<scalar_double_null_gluing_checks.wl>): two intersecting cuts, explicit Goursat reconstruction and corner recurrence, both null energy fluxes, four symplectic pairings, and five two-sheet Peierls comparisons including wall reflections and spacelike cancellation. The smooth matching domain and polynomial algebra map are proved separately in the null scalar model.
...
- [rectangle_self_sewing_checks.sage](<rectangle_self_sewing_checks.sage>) and [rectangle_self_sewing_checks.wl](<rectangle_self_sewing_checks.wl>): exhaustive twelve-pairing vertex-link/homology/cone-angle audit and 19 exact scalar/action/group/mode/trace checks for [rectangle self-sewing](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/non-gauge/rectangle scalar self-gluing.md>). [Results](<rectangle_self_sewing_results.json>) retain the executed output and setup correction; continuum form-domain and algebra claims have separate proof arguments in the note.
