...
$$\begin{align}
I=[t_i,t_f],\qquad R=[0,L_x]\times[0,L_y],\qquad ds^2=-dt^2+dx^2+dy^2.
\end{align}$$
...
原版 [classical formalism](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/formalism/classical_gluing_formalism.md>) 和 [observable formalism](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/formalism/observable algebra gluing formalism.md>) 限于无空间 junctions 的 pairwise smooth cuts；当前修订已采用 joint trace/domain 与完整 response pullback 定义，并明确其 corner reconstruction 输入。本模型提供具体实现：**边的 transmission、顶点的 link/metric 条件、函数空间的角点域必须一起指定。**

[Scalar closure §§1–4](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar observable sewing closure.md>) 在本模型的 $m>0$ smooth torus/Klein sector 中另给完整 independent history-family 商：区域场为时间光滑的能量解，history covectors 为联合 $H^{1/2}$ trace 空间的对偶。该处证明实际 $R,B$、regular transposes 和逐函数满射；这不是本笔记原先待证的逐面空间光滑 density 类，也不扩大下文锥点量子范围。
...
$$\begin{align}
v_0=(0,0),\ v_1=(L_x,0),\ v_2=(L_x,L_y),\ v_3=(0,L_y), \qquad e_i:v_i\longrightarrow v_{i+1\ {\rm mod}\ 4}.
\end{align}$$

对每一对 $(e_a,e_b)$，用归一化参数 $s\in[0,1]$ 表示识别：

$$\begin{align}
\epsilon=+1:\ e_a(s)\sim e_b(s),\qquad \epsilon=-1:\ e_a(s)\sim e_b(1-s).
\end{align}$$
...
$$\begin{align}
S_R[\phi]=\frac12\int_I dt\int_R dx\,dy\, \bigl(\dot\phi^2-|\nabla\phi|^2-m^2\phi^2\bigr).
\end{align}$$

不加入边界动力学或角点项。直接变分：

$$\begin{align}
\delta S_R= -\int_{I\times R}(\ddot\phi-\Delta\phi+m^2\phi)\delta\phi +\left[\int_R\dot\phi\,\delta\phi\right]_{t_i}^{t_f} +\sum_e\int_{I\times e}\Pi_e\,\delta q_e, \qquad q_e=\gamma_e\phi,\quad \Pi_e=-\partial_{n_e}\phi.
\end{align}$$
...
$$\begin{align}
\delta S_R\big|_{\rm bulk} =\langle\Pi_e+J^\dagger\Pi_{e'},\delta q_e\rangle, \qquad \boxed{q_{e'}=Jq_e,\qquad\Pi_e+J^\dagger\Pi_{e'}=0.}
\end{align}$$
...
$$\begin{align}
p=\dot\phi,\qquad \Theta_R=\int_R p\,\delta\phi,\qquad \Omega_R=\delta\Theta_R=\int_R\delta p\wedge\delta\phi.
\end{align}$$

采用既有约定 $\iota_{X_F}\Omega=-dF$，故 $\{\phi,p\}=1$。匹配消去所有成对的能量与辛通量：

$$\begin{align}
H_R=\frac12\int_R(p^2+|\nabla\phi|^2+m^2\phi^2),\qquad \dot H_R=\sum_e\int_e p\,\partial_{n_e}\phi=0, \qquad \Omega(t_f)=\Omega(t_i).
\end{align}$$
...
$$\begin{align}
\phi(0,y)=\phi(L_x,y),\quad \phi_x(0,y)=\phi_x(L_x,y),
\end{align}$$

$$\begin{align}
\phi(x,0)=\phi(x,L_y),\quad \phi_y(x,0)=\phi_y(x,L_y).
\end{align}$$
...
$$\begin{align}
\phi(0,y)=\phi(L_x,L_y-y),\quad \phi_x(0,y)=\phi_x(L_x,L_y-y),
\end{align}$$
...
$$\begin{align}
\partial_x^r\partial_y^s\phi(0,y) =(-1)^s\partial_x^r\partial_y^s\phi(L_x,L_y-y), \qquad r,s\geq0,
\end{align}$$
...
$$\begin{align}
u_{nk}=e^{i\pi n x/L_x} \left(e^{2\pi i k y/L_y}+(-1)^n e^{-2\pi i k y/L_y}\right), \quad n\in\mathbb Z,\ k>0,
\end{align}$$
...
$$\begin{align}
(0,y)\sim(L_x,L_y-y),\qquad (x,0)\sim(L_x-x,L_y).
\end{align}$$
...
$$\begin{align}
\phi_y(x,0)=\phi_x(L,L-x),\qquad \phi_y(x,L)=\phi_x(0,L-x).
\end{align}$$
...
$$\begin{align}
\alpha_v=\frac{\pi r_v}{2},\qquad \kappa_v=2\pi-\alpha_v.
\end{align}$$
...
$$\begin{align}
u_{xxx}(0,y)-u_{xxx}(L_x,y) =-24L_x\,y^2(L_y-y)^2\neq0.
\end{align}$$
...
$$\begin{align}
\int_0^\delta\!\int_0^\delta \frac{|q_1(r)-q_2(s)|^2}{(r+s)^2}\,dr\,ds<\infty.
\end{align}$$
...
$$\begin{align}
\mathfrak a_R(u,v)=\int_R(\nabla u\cdot\nabla v+m^2uv), \qquad u,v\in V_{\rm sew}.
\end{align}$$

Trace 连续性使 $V_{\rm sew}$ 是 $H^1(R)$ 的闭子空间；它包含 $C_c^\infty(R^\circ)$，所以在 $L^2(R)$ 中稠密。二次型加 $L^2$ 范数与 $H^1$ 范数等价，故闭且非负。表示定理给唯一非负自伴算子

$$\begin{align}
D(A_{\rm sew})= \{u\in V_{\rm sew}:\exists f\in L^2(R),\
\mathfrak a_R(u,v)=\langle f,v\rangle\ \forall v\in V_{\rm sew}\}, \qquad A_{\rm sew}u=f.
\end{align}$$
...
$$\begin{align}
U:L^2(R)\longrightarrow L^2(\Sigma),\qquad U V_{\rm sew}=D(\mathfrak a_\Sigma),\qquad \mathfrak a_\Sigma(Uu,Uv)=\mathfrak a_R(u,v),
\end{align}$$
...
$$\begin{align}
u(t)=\cos(\tau\sqrt{A_{\rm sew}})u_0+ \frac{\sin(\tau\sqrt{A_{\rm sew}})}{\sqrt{A_{\rm sew}}}p_0.
\end{align}$$
...
$$\begin{align}
q\in C^2(I;\mathcal B_R^{\mathrm{sp}}),\quad z_0\in H_0^1(R),\quad z_1\in L^2(R),
\end{align}$$
...
$$\begin{align}
\phi=v+Eq,\qquad \ddot v+A_Dv=-E(\ddot q+m^2q),\qquad (v,\dot v)(t_0)=(z_0,z_1).
\end{align}$$
...
$$\begin{align}
\lambda_\phi[\eta] =\int_I\left(\langle p,E\dot\eta\rangle_{L^2} -\mathfrak a_R(\phi,E\eta)\right)\,dt .
\end{align}$$
...
$$\begin{align}
\boxed{\lambda_\phi^{\mathrm{bdry}}[\eta] =\int_I\left(\langle p,E\dot\eta\rangle_{L^2} -\mathfrak a_R(\phi,E\eta)\right)\,dt -\left[\langle p,E\eta\rangle_{L^2}\right]_{t_i}^{t_f}.}
\end{align}$$
...
$$\begin{align}
p\in C(I;H),\qquad \dot p\in C(I;V_0'),\qquad \langle\dot p,w\rangle_{V_0',V_0}=-\mathfrak a_R(\phi,w).
\end{align}$$

因此对 $w\in C(I;V_0)\cap C^1(I;H)$，

$$\begin{align}
\int_I\left(\langle p,\dot w\rangle_H-\mathfrak a_R(\phi,w)\right)\,dt =\left[\langle p,w\rangle_H\right]_{t_i}^{t_f}.
\end{align}$$
...
$$\begin{align}
\left.\delta S_R[h]\right|_{\mathrm{regional\ solution}} =\left[(p,h)\right]_{t_i}^{t_f}+\lambda_\phi^{\mathrm{bdry}}[\gamma h].
\end{align}$$

这证明该公式恰是分离完整 $\Theta_R(t_f)-\Theta_R(t_i)$ 后的 response。若解光滑到边界，空间 Green identity 再给

$$\begin{align}
\lambda_\phi^{\mathrm{bdry}}[\eta] =\int_{I\times\partial R}(-\partial_n\phi)\eta .
\end{align}$$
...
$$\begin{align}
\mathcal B_{\mathrm{sew}}^{\mathrm{sp}} =\{q\in\mathcal B_R^{\mathrm{sp}}:q_{e'}=Jq_e\text{ 对全部配边成立}\}.
\end{align}$$

由 $\gamma E=1$，

$$\begin{align}
\gamma V_{\mathrm{sew}}=\mathcal B_{\mathrm{sew}}^{\mathrm{sp}},\qquad E\mathcal B_{\mathrm{sew}}^{\mathrm{sp}}\subset V_{\mathrm{sew}},\qquad h=(h-E\gamma h)+E\gamma h,\quad h-E\gamma h\in V_0.
\end{align}$$
...
$$\begin{align}
\int_I\left[(p,\dot h)_H-\mathfrak a_R(\phi,h)\right]dt=0 \quad\text{对所有 }h\in C_c^\infty(I^\circ;V_{\mathrm{sew}}).
\end{align}$$
...
$$\begin{align}
z_0=\phi(t_i)-Eq(t_i)\in H_0^1(R),\qquad z_1=p(t_i)-E\dot q(t_i)\in L^2(R)
\end{align}$$
...
$$\begin{align}
\mathscr D_{\rm sew}=\bigcap_{j\geq0}D(A_{\rm sew}^j), \qquad X_{\rm sew}=\mathscr D_{\rm sew}\oplus\mathscr D_{\rm sew}.
\end{align}$$

取各 graph norms 的 Fréchet 拓扑；$\Omega$ 是连续、弱非退化的。由 regional form/domain 独立定义

$$\begin{align}
\mathcal A_{\rm sew}^{\rm cl} =\{F\in C_B^\infty(X_{\rm sew},\mathbb C):
...
$$\begin{align}
\alpha:\mathcal A_{\rm sew}^{\rm cl}\longrightarrow\mathcal A_\Sigma^{\rm cl}, \qquad \alpha(F)=F\circ(U\oplus U)^{-1},
\end{align}$$
...
$$\begin{align}
L_z(w)=-\Omega(z,w),\qquad X_{L_z}=z,\qquad \{L_z,L_{z'}\}=-\Omega(z,z').
\end{align}$$
...
同一个 $U\oplus U$ 保持 CCR defining relations，其逆也保持 relations，因此由两个 universal homomorphisms 给该 core 的精确量子代数同构，亦可用同一实辛空间构造 Weyl algebra。这不是把原 isolated fixed-history algebra 的 bracket 原样取商。更强的 massive 光滑 torus/Klein Wick 比较见 [free quantum note](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/free scalar renormalized quantum observable algebra gluing.md>)；cone/Friedrichs 情形在这里仅建立指定的 CCR 层，不从能量定理推断 Wick products 或 interacting renormalization。

对于先粘哪一对边，先在同一个区域 $H^1(R)$ 中定义两个闭 trace 条件 $C_1u=0,C_2u=0$。最终域恒为

$$\begin{align}
V_{\rm sew}=\ker C_1\cap\ker C_2 =(\ker C_1)\cap(\ker C_2) =(\ker C_2)\cap(\ker C_1),
\end{align}$$
...
- **Verified:** Sage 完整穷举 12 种配对，检查每个顶点 link、整数 chain complex/homology、orientation、Euler 数和全部角亏损；Mathematica 的 19 组 exact checks 检查 off-shell variation、能量散度、CPS 符号、反射能量、Klein 群关系、sphere 相邻边 collar、双反向不动点、21 个复模的 transmission/eigenvalue 和低阶 seam 反例。输入与实际输出见 [Sage](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/rectangle_self_sewing_checks.sage>)、[Mathematica](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/rectangle_self_sewing_checks.wl>)、[results](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/rectangle_self_sewing_results.json>)。
