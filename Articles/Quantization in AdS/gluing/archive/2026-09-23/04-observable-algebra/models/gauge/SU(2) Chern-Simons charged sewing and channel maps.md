...
几何为 [classical bridge](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/SU(2) Chern-Simons two-puncture classical bridge.md>) 的两个分离 Wilson punctures，cut 避开 worldlines。该 note 的 classical orbit parameters 不由本篇的 $j$ 值反向定义；这里增加量子化处方，而非证明一切 continuum CS 量子化都等于此处方。
...
$$\begin{align}
[J^a_m,J^b_n]=i\epsilon_{abc}J^c_{m+n} +\frac{k}{2}m\delta_{ab}\delta_{m+n,0},\qquad (J^a_n)^*=J^a_{-n}.
\end{align}$$
...
$$\begin{align}
L_0|_{H_j(0)}=h_j=\frac{j(j+1)}{k+2},\qquad h_0=0,\quad h_\sigma=3/16,\quad h_\psi=1/2.
\end{align}$$
...
$$\begin{align}
C_{0\sigma}(|+\!-\rangle)=1/\sqrt2,\quad C_{0\sigma}(|-\!+\rangle)=-1/\sqrt2,
\end{align}$$
...
$$\begin{align}
\alpha_{ab}=h_a-h_b-h_\sigma,\qquad a_{ab}(F)=\phi_{ab}(e^{-i\alpha_{ab}\theta}F).
\end{align}$$
...
$$\begin{align}
J_a(\epsilon)a_{ab}(F)-a_{ab}(F)J_b(\epsilon) =a_{ab}(\epsilon^ct^cF).
\end{align}$$
...
$$\begin{align}
\mathcal X=\operatorname{Hom}_{\mathcal A_k(I^c)}(H_0,H_\sigma),\qquad \mathcal Y=\operatorname{Hom}_{\mathcal A_k(I)}(H_0,H_\sigma).
\end{align}$$

这里是 bounded module intertwiners。定义 $\mathcal X_0$ 为

$$\begin{align}
x=\sum_r\pi_\sigma(g_r)a_{\sigma0}(F_r),\qquad g_r\in L_IG,\quad \operatorname{supp}F_r\Subset\widetilde I
\end{align}$$

的 span，并令

$$\begin{align}
x_a=\sum_r\pi_a(g_r)a_{a\sigma}(F_r):H_\sigma\to H_a, \qquad a=0,\psi.
\end{align}$$
...
$$\begin{align}
\boxed{\pi_\sigma(x^*x')=\lambda_0 x_0^*x'_0+ \lambda_\psi x_\psi^*x'_\psi.}
\end{align}$$
...
$$\begin{align}
G\left(\sum_i x_i\otimes y_i,\sum_jx'_j\otimes y'_j\right) =\sum_{ij}\langle y_i\Omega,\pi_\sigma(x_i^*x'_j)y'_j\Omega\rangle.
\end{align}$$
...
$$\begin{align}
\mathcal N=\{z:G(z,z)=0\},\qquad H_{\mathrm{sew}}=\overline{\mathcal V_0/\mathcal N} =H_\sigma(\partial D_1)\boxtimes_M H_\sigma(\partial D_2).
\end{align}$$
...
$$\begin{align}
C_a^0\left(\sum_i x_i\otimes y_i\right) =\sum_i\widehat{(x_i)}_a\,y_i\Omega,\qquad U_0=C_0^0\oplus C_\psi^0.
\end{align}$$

Transport formula 给 **完整 norm identity**

$$\begin{align}
\boxed{G(z,z')= \langle C_0^0z,C_0^0z'\rangle+ \langle C_\psi^0z,C_\psi^0z'\rangle.}
\end{align}$$
...
$$\begin{align}
C_aC_b^*=\delta_{ab}1_{H_a},\quad P_0+P_\psi=1,\quad \ker C_0=u_\psi H_\psi,\quad \ker C_\psi=u_0H_0.
\end{align}$$
...
$$\begin{align}
U\,\mathcal B_{\mathrm{sew}}(J)\,U^* =(\pi_0\oplus\pi_\psi)(\mathcal A_k(J)) \quad\text{对每个 outer interval }J .
\end{align}$$
...
$$\begin{align}
C_a[a_{\sigma0}(F)\otimes y] =\widehat a_{a\sigma}(F)\,y\Omega.
\end{align}$$
...
为定义包围两个 punctures 的 Wilson observable，在 **source sector category** 中取 spin-$\sigma$ probe 的 standard coevaluation $R:1\to\bar\sigma\boxtimes\sigma$、$R^*R=d_\sigma$。让 probe 绕对象 $X$ 一圈，定义 bounded network morphism

$$\begin{align}
W_\sigma(X)= (R^*\boxtimes1_X) (1_{\bar\sigma}\boxtimes c_{X,\sigma}c_{\sigma,X}) (R\boxtimes1_X).
\end{align}$$
...
$$\begin{align}
\sigma\boxtimes\sigma=0\oplus\psi,\quad \psi\boxtimes\sigma=\sigma,\quad\psi\boxtimes\psi=0,\quad d_0=d_\psi=1,\quad d_\sigma=\sqrt2.
\end{align}$$

$X=0$ 时 loop value 为 $d_\sigma$。$X=\psi$ 时只有 $\sigma$ intermediate sector，double braiding 的标量为 $\theta_\sigma/(\theta_\sigma\theta_\psi)=-1$，所以

$$\begin{align}
\boxed{C_aW_\sigma(H_{\mathrm{sew}})=w_aC_a,\qquad w_0=\sqrt2,\quad w_\psi=-\sqrt2.}
\end{align}$$
...
$$\begin{align}
\mathcal K=\operatorname{Hom}_{\mathcal A_k} \left(H_0,H_\sigma\boxtimes H_\sigma\boxtimes H_\sigma\boxtimes H_\sigma\right).
\end{align}$$
...
$$\begin{align}
s_a=(u_a\boxtimes u_a)r_a,\qquad t_0=(1_\sigma\boxtimes u_0\boxtimes1_\sigma)u_0.
\end{align}$$

再把 $C_\psi$ 的一条 $\sigma$ leg 用 standard $\sigma$ cup $R_\sigma$ 弯转，得到

$$\begin{align}
\widetilde w_\psi=(1_\sigma\boxtimes C_\psi)(R_\sigma\boxtimes1_\sigma): H_\sigma\longrightarrow H_\sigma\boxtimes H_\psi,\qquad w_\psi=\widetilde w_\psi (\widetilde w_\psi^*\widetilde w_\psi)^{-1/2}.
\end{align}$$

Rigidity 和 $C_\psi\ne0$ 保证它非零；Schur lemma 使括号内为正 scalar，故这是实际 bounded isometry。置

$$\begin{align}
t_\psi=(1_\sigma\boxtimes u_\psi\boxtimes1_\sigma) (w_\psi\boxtimes1_\sigma)u_0.
\end{align}$$
...
$$\begin{align}
\nu_\sigma=\frac1{\mathcal D^2}\sum_{a,b}N_{ab}^{\sigma}d_ad_b \left(\frac{\theta_a}{\theta_b}\right)^2=-1, \quad \mathcal D^2=4,\quad (\theta_0,\theta_\sigma,\theta_\psi)=(1,e^{3\pi i/8},-1).
\end{align}$$
...
$$\begin{align}
R_{\sigma\sigma}^0=-e^{-3\pi i/8},\qquad R_{\sigma\sigma}^\psi=e^{\pi i/8},\qquad B_1=\operatorname{diag}(R^0,R^\psi),\quad B_2=F^*B_1F.
\end{align}$$
...
**Verified:** fundamental compressed fields及其合法 tests／Ward identity；transport-normalized dense channel maps、Gram kernel、onto 和全部 local-net comparison；两通道 Wilson detector；上述 duality／framing 下的 $F$、braid 和非对易 channel operators。外部定理的模型适用关系及实际 maps 均已写出；有限检查见 [本轮 ledger](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/charged CS bridge claim ledger.md>)。
