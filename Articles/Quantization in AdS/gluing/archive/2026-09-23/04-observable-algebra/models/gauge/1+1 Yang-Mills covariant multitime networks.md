...
使用 [YM classical note](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/1+1 Yang-Mills classical observable algebra gluing.md>) 的 action
$$
...
**Verified:** temporal covariance、common invariant domain、finite-graph global comparison；all compact-group finite-word reduction、actual section、完整 constructive kernel 与旧 global envelope equality；$U(1)$ 另有简短 normal form 与额外 exponential kernel。可计算部分见 [验算记录](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/CS and YM five work packages verification.md>)。
