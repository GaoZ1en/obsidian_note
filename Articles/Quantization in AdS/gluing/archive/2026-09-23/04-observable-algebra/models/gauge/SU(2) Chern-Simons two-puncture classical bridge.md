...
这里的标记是穿过空间切片的 Wilson worldlines，尚未把任意几何 corner 或 boundary Wilson endpoint 识别成 puncture。量子版本另见 [两个非真空通道](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/SU(2) Chern-Simons charged sewing and channel maps.md>)。
...
**Verified:** sourced Gauss law 与 orbit CPS；actual $(p,\Phi)$ 的双向字典、properness、based-loop fibers；corrected two-form、ordered cross-term；独立 pair-of-pants comparison 与有损 $\mu$-forgetting 的例子。证明使用上列 AMM Sobolev／reduction 定理；可计算项见 [本轮 ledger](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/charged CS bridge claim ledger.md>)。
