...
工作包 A。沿用 [disk/open-chord 模型](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/U(1) Chern-Simons disk and open-chord sewing.md>) 的 action、$k>0$、$\Omega=\delta\Theta$、$\iota_{X_F}\Omega=-dF$ 和 source-free smooth disk。这里的三条 rays 在 disk 内部交于同一点，区别于两条不相交 chords。
...
**Verified:** 上述实际 extension、kernel、current/PBW quotient 与包含 residual junction gauge 的 reduction by stages；可计算部分见 [五工作包验算](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/CS and YM five work packages verification.md>)。
