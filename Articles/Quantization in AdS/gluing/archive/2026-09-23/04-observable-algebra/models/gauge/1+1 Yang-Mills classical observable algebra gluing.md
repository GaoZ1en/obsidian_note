...
本文是 [observable algebra gluing formalism](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/formalism/observable algebra gluing formalism.md>) 的完整 classical gauge 实现。区域对象由 continuum action、proper gauge quotient、active cut action 和实际 history transport 定义；global theory 独立从 uncut action 构造。
...
$$\begin{align}
\boxed{ \frac{C^\infty(T^*G\times T^*G)^{G_\Gamma}}
...
$$\begin{align}
F_{tx}=\partial_tA_x-\partial_xA_t+[A_t,A_x], \qquad D_x=\partial_x+[A_x,\cdot].
\end{align}$$

取 coupling $e^2>0$。一阶作用量为

$$\begin{align}
S[A,E]=\int dt\,dx\left[ \langle E,\dot A_x-D_xA_t\rangle -\frac{e^2}{2}\langle E,E\rangle\right].
\end{align}$$
...
$$\begin{align}
D_xE=0,\qquad D_tE=0,\qquad \dot A_x-D_xA_t=e^2E.
\end{align}$$

Cauchy symplectic potential 与形式是

$$\begin{align}
\Theta_I=\int_I\langle E,\delta A_x\rangle dx, \qquad \Omega_I=\delta\Theta_I.
\end{align}$$
...
$$\begin{align}
-\langle E(b),\delta A_t(b)\rangle +\langle E(a),\delta A_t(a)\rangle.
\end{align}$$
...
$$\begin{align}
A_\mu^k=kA_\mu k^{-1}-(\partial_\mu k)k^{-1}, \qquad E^k=kEk^{-1}.
\end{align}$$

若两 cut frames 以 $k(t)$ 比较，coordinate-covariant matching 写为

$$\begin{align}
q_2=kq_1k^{-1}-\dot k k^{-1},\qquad E_2=kE_1k^{-1}.
\end{align}$$
...
$$\begin{align}
\mathcal G_{00}(I) =\{k\in C^\infty(I,G):k(a)=k(b)=1\}.
\end{align}$$
...
$$\begin{align}
u'(x)=-A_x(x)u(x),\qquad u(a)=1, \qquad U=u(b)\in G.
\end{align}$$
...
$$\begin{align}
\boxed{ \{(A_x,E):D_xE=0\}/\mathcal G_{00}(I) \cong G\times\mathfrak g\cong T^*G.
}
\end{align}$$
...
$$\begin{align}
\Theta_I =-[\langle E,\xi\rangle]_a^b =\boxed{\langle p,U^{-1}\delta U\rangle}.
\end{align}$$
...
$$\begin{align}
\boxed{ \Omega((\xi,\eta),(\zeta,\nu)) =\langle\eta,\zeta\rangle-\langle\nu,\xi\rangle -\langle p,[\xi,\zeta]\rangle.
}
\end{align}$$
...
$$\begin{align}
dF(U\xi,\eta)=\langle\nabla_UF,\xi\rangle +\langle\partial_pF,\eta\rangle.
\end{align}$$

Poisson bracket 为

$$\begin{align}
\boxed{ \{F,G\} =\langle\nabla_UF,\partial_pG\rangle -\langle\nabla_UG,\partial_pF\rangle -\langle p,[\partial_pF,\partial_pG]\rangle.
}
\end{align}$$
...
$$\begin{align}
\{p_\xi,p_\eta\}=-p_{[\xi,\eta]}, \qquad \{U,p_\xi\}=U\xi.
\end{align}$$
...
$$\begin{align}
J_a(\xi)=-\langle p,\xi\rangle, \qquad J_b(\xi)=\langle\operatorname{Ad}_Up,\xi\rangle, \quad \iota_{\xi^\#}\Omega=-dJ(\xi).
\end{align}$$
...
$$\begin{align}
\{J_a(\xi),J_a(\eta)\}=J_a([\xi,\eta]), \quad \{J_b(\xi),J_b(\eta)\}=J_b([\xi,\eta]), \quad \{J_a(\xi),J_b(\eta)\}=0.
\end{align}$$
...
$$\begin{align}
\boxed{ k:(U_1,p_1;U_2,p_2) \mapsto(kU_1,p_1;U_2k^{-1},\operatorname{Ad}_kp_2).
}
\end{align}$$
...
$$\begin{align}
P_{12}=T^*G\times T^*G, \quad C_\Gamma=\mu_\Gamma^{-1}(0).
\end{align}$$
...
$$\begin{align}
\boxed{ C_\Gamma/G_\Gamma\xrightarrow{\cong}T^*G, \quad[(U_1,p_1;U_2,p_2)]\mapsto(U_2U_1,p_1).
}
\end{align}$$
...
$$\begin{align}
\mathcal I_\Gamma =\{F\in\mathcal A_{12}^{G_\Gamma}:F|_{C_\Gamma}=0\}.
\end{align}$$

定义

$$\begin{align}
\boxed{ \mathcal A_{\mathrm{sew}} =\mathcal A_{12}^{G_\Gamma}/\mathcal I_\Gamma.
}
\end{align}$$
...
$$\begin{align}
\boxed{ D_\rho(U_2U_1)_{ab} =\sum_cD_\rho(U_2)_{ac}D_\rho(U_1)_{cb}.
}
\end{align}$$
...
$$\begin{align}
D_\rho(U_2)\,dD_\rho(p_2)\,D_\rho(U_1) \big|_{C_\Gamma} =D_\rho(U_2U_1)\,dD_\rho(p_1).
\end{align}$$
...
$$\begin{align}
H_I=\frac{e^2}{2}\int_a^b\langle E,E\rangle dx =\boxed{\frac{e^2\ell}{2}\langle p,p\rangle}.
\end{align}$$
...
$$\begin{align}
\dot U=-q_bU+Uq_a+e^2\ell Up, \qquad\dot p=[p,q_a].
\end{align}$$

若 $\dot V_a=-q_aV_a$、$\dot V_b=-q_bV_b$、$V_{a,b}(t_0)=1$，实际解为

$$\begin{align}
p(t)=V_a(t)p_0V_a(t)^{-1}, \quad U(t)=V_b(t)U_0e^{e^2\ell(t-t_0)p_0}V_a(t)^{-1}.
\end{align}$$
...
$$\begin{align}
\dot U=e^2(\ell_1+\ell_2)Up_1, \qquad\dot p_1=0.
\end{align}$$

又有

$$\begin{align}
(H_1+H_2)|_{C_\Gamma} =\frac{e^2(\ell_1+\ell_2)}2\langle p_1,p_1\rangle.
\end{align}$$
...
$$\begin{align}
E(t,x)=-u_0(x)p\,u_0(x)^{-1}, \quad A_x(t,x)=A_{x,0}(x)+e^2tE_0(x),\quad A_t=0.
\end{align}$$
...
$$\begin{align}
\boxed{ W_\rho(\gamma) =\operatorname{tr}_\rho \exp\left(e^2p\oint_\gamma t\,dx\right).
}
\end{align}$$
...
$$\begin{align}
[(U_1,p_1),\ldots,(U_N,p_N)] \mapsto(U_N\cdots U_1,p_1).
\end{align}$$
...
$$\begin{align}
\mu_{\circlearrowleft}=\operatorname{Ad}_Up-p, \qquad k:(U,p)\mapsto(kUk^{-1},\operatorname{Ad}_kp).
\end{align}$$
...
$$\begin{align}
U=x1+i\boldsymbol y\cdot\boldsymbol\sigma, \quad x^2+|\boldsymbol y|^2=1, \quad p=\sum_jp_jT_j.
\end{align}$$

约束等价于 $\boldsymbol y\times\boldsymbol p=0$。同时对角化给

$$\begin{align}
\boxed{\mathcal P_{S^1}\cong(T\times\mathfrak t)/W, \quad (\theta,\varpi)\sim(-\theta,-\varpi),\quad W=\mathbb Z_2.}
\end{align}$$
...
$$\begin{align}
x=\tfrac12\operatorname{tr}U, \qquad v=2\boldsymbol y\cdot\boldsymbol p, \qquad w=4|\boldsymbol p|^2
\end{align}$$

描述全部约化点：

$$\begin{align}
x=\cos\theta,\quad v=\sin\theta\,\varpi,\quad w=\varpi^2, \qquad \boxed{|x|\le1,\quad w\ge0,\quad v^2=(1-x^2)w.}
\end{align}$$
...
$$\begin{align}
\boxed{ \frac{C^\infty(T^*SU(2))^{SU(2)}}
...
$$\begin{align}
\boxed{\mathscr P_{S^1} =\mathbb C[x,v,w]/\big((1-x^2)w-v^2\big).}
\end{align}$$
...
$$\begin{align}
\{x,v\}=x^2-1,\qquad \{x,w\}=-2v,\qquad \{v,w\}=2xw, \qquad H_{S^1}=\frac{e^2\ell}{8}w.
\end{align}$$

这些公式在两个 singular points 也有定义。实际演化为

$$\begin{align}
\varpi(t)=\varpi_0,\qquad \theta(t)=\theta_0+\frac{e^2\ell}{4}t\varpi_0 \quad\bmod 2\pi,
\end{align}$$

与 Weyl identification 相容。带共同 $q$ 的 interval Hamiltonian 是 $H-\langle\mu_{\circlearrowleft},q\rangle$；约化后 $q$ 只改变 gauge representative。量子 kernel、全局 image 与由 Haar/Casimir 固定的 operator domain 见 [quantum note §11](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/1+1 Yang-Mills quantum observable algebra gluing.md>)。

## 13. 核验范围

**Verified:** continuum Gauss quotient 的双向 maps、约化 potential 与 symplectic form、全部 smooth observables 的 kernel/image/section、Wilson/electric insertion sewing、真实 global dynamics 和有限 reduction by stages 均在正文给出证明。[Mathematica 检查](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/observable_sewing_ym_audit_checks.wl>) 的 [23 组结果](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/observable_sewing_ym_audit_results.json>) 同时包含本模型与一般 split bracket 的有限恒等式。
...
**Verified (circle):** §12 给 continuum reconstruction、全 strata、smooth extension 与 Poisson quotient；[新增检查](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/ym_circle_cs_corner_checks.wl>) 核对径向 brackets，[Sage 检查](<Articles/Quantization in AdS/gluing/archive/2026-09-23/verification/numerics/ym_circle_self_sewing_topology_checks.sage>) 核对 polynomial elimination。**Assumptions:** circle 部分专取上述 $SU(2)$、全 periodic gauge quotient、零 moment、无带电插入，smooth extension 使用已声明的 Schwarz theorem。**Not verified:** 此处没有把 circle 结论外推到任意紧群的 singular reduction。
