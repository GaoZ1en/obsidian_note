...
$$\begin{align}
H_1(\Sigma_{g,b};\mathbb Z)=\mathbb Z^{2g+b-1},\qquad H_1(N_{r,b};\mathbb Z)=\mathbb Z^{r+b-1},\qquad H_2=0.
\end{align}$$
...
$$\begin{align}
\Sigma_{0,0}=S^2,\quad \Sigma_{1,0}=T^2,\quad \Sigma_{0,2}=\text{annulus},\quad N_{1,0}=\mathbb{RP}^2,\quad N_{2,0}=K,\quad N_{1,1}=\text{Möbius strip}.
\end{align}$$

因此 [rectangle benchmark](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/non-gauge/rectangle scalar self-gluing.md>) 的四种闭 topology 只是“不细分四条整边”的有限子问题。增加 faces 或细分边界后，不能继续把 $S^2,\mathbb{RP}^2,T^2,K$ 当作一般性的完整列表。
...
$$\begin{align}
S=\frac12\int_I dt\int_\Sigma d\mathrm{vol}_h\, \big(\dot\phi^2-|d\phi|_h^2-m^2\phi^2\big).
\end{align}$$

Momentum 和 CPS 使用同一个正 density，

$$\begin{align}
\Theta=\int_\Sigma p\,\delta\phi\,d\mathrm{vol}_h,\qquad \Omega=\delta\Theta.
\end{align}$$
...
$$\begin{align}
A_j=h_{ij}^{-1}A_ih_{ij}+h_{ij}^{-1}dh_{ij}, \qquad h_{ij}h_{jk}h_{ki}=1
\end{align}$$
...
$$\begin{align}
H^2(\Sigma_{g,0};\mathbb Z)=\mathbb Z,\qquad H^2(N_{r,0};\mathbb Z)=\mathbb Z_2.
\end{align}$$
...
这证明 local classical action 的下降条件；完整 compact quantum theory 还需 level lattice、boundary polarization 和 parity/anomaly 数据。一个 anti-isometry 矩阵检验不证明全部 quantum sewing。特别地，[open-chord CS benchmark](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/models/gauge/U(1) Chern-Simons disk and open-chord sewing.md>) 是 oriented disk 上的单份 current theorem，与 doubled nonorientable theory 是两个明确不同的模型。
...
$$\begin{align}
\prod_{j=1}^{g}[A_j,B_j]=1 \quad(\Sigma_g),\qquad \prod_{j=1}^{r}C_j^2=1 \quad(N_r),
\end{align}$$
