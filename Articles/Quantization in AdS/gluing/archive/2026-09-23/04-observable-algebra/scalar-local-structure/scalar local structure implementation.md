...
日期：2026-09-07。本文实现 [研究路线](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar local structure research route.md>) 的 A–C：先从矩形作用量、joint histories 和实际 response 构造传播及局域标签，再证明 CCR 与完整 polynomial Wick 核类的包含、因果性、time-slice、有限切割相容性。相互作用所用的**有限插入生成类**另见 [插入类的 time-slice 障碍](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar insertion time-slice obstruction.md>)；不能把本文的 Wick 定理移给它。
...
$$\begin{align}
S_i^\dagger f=\mathcal E_i^*\widetilde f -(\partial_t^2+m^2)\mathcal E_i^*G_i^A\widetilde f, \qquad G_i^A\widetilde f=G_i^R\widetilde f.
\end{align}$$
...
旧 [classical note §§2.1–2.2、4.3](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/formalism/classical regional observable algebra gluing.md>) 的 onto/time-band 采用 $C_c^\infty(J;L^2(\Sigma_i))$，允许接触边界。它只提供本节的符号检查，不能代替下面 $C_c^\infty(O)$ 的 kernel／support 证明。本文也不声称旧 spatially smooth face-density full-family theorem 已扩展到矩形。
...
$$\begin{align}
H_s=\bigoplus_iL^2(R_i),\qquad V_s=\{(u_i)\in\bigoplus_iH^1(R_i):\gamma u\text{ 按全部配边匹配}\},
\end{align}$$
...
$$\begin{align}
\operatorname{supp}G_s^{R/A}f\subset J_s^{\pm}(\operatorname{supp}f), \qquad \operatorname{supp}E_sf\subset J_s(\operatorname{supp}f).
\end{align}$$
...
$$\begin{align}
\Omega(z,w)=\sum_i\int_{R_i}(p_z u_w-p_wu_z),\qquad \int fE_sg=-\Omega(E_sf,E_sg).
\end{align}$$
...
$$\begin{align}
G^R(t,x;0,0)=\frac{\theta(t)\theta(t^2-|x|^2)}{2\pi} \frac{\cos(m\sqrt{t^2-|x|^2})}{\sqrt{t^2-|x|^2}}.
\end{align}$$
...
$$\begin{align}
\mathcal V_s^{\mathrm{rel}}(O)=\mathscr D_s(O)/\{f:E_s e_Of=0\}, \quad \sigma_s([f],[g])=\int e_Of\,E_s e_Og.
\end{align}$$
...
$$\begin{align}
\ker(E_se_O)=L_OC_c^\infty(O),\qquad \mathcal V_s^{\mathrm{rel}}(O)\cong C_c^\infty(O)/L_OC_c^\infty(O).
\end{align}$$
...
$$\begin{align}
h_\chi f=(1-\chi)G_s^Rf+\chi G_s^Af, \qquad \tau_\chi f=-[L,\chi]E_sf.
\end{align}$$

逐项计算得到

$$\begin{align}
Lh_\chi f=f+[L,\chi]E_sf,\qquad \boxed{f-\tau_\chi f=Lh_\chi f.}
\end{align}$$
...
$$\begin{align}
[L,\chi]u=(\chi_{tt}-\Delta\chi)u +2(\chi_tu_t-\nabla\chi\cdot\nabla u).
\end{align}$$
...
$$\begin{align}
\tau_\chi f-\tau_{\widetilde\chi}f =-L\bigl((\chi-\widetilde\chi)E_Of\bigr),
\end{align}$$
...
$$\begin{align}
\mathcal P_\mu(O)=\bigoplus_{k\ge0}\mathcal E'_{\mu,\mathrm{sym}}(O^k),\qquad \operatorname{WF}(f_k)\cap
...
$$\begin{align}
\kappa_k(f_k)[z_1,\ldots,z_k] =\langle f_k,C_sz_1\otimes\cdots\otimes C_sz_k\rangle=0.
\end{align}$$

取全部 $k$、全部 smooth Cauchy data，常数项计入 $k=0$。这是 [closure §5](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar observable sewing closure.md>) 的完整 kernel；没有有限-mode 替代。局域版本仅要求每个 slot 的支撑在 $O$。
...
$$\begin{align}
f_k-\tau_\chi^{(k)}f_k =\sum_{j=1}^k L_j\Bigl[(\tau_\chi)_1\cdots(\tau_\chi)_{j-1}(h_\chi)_j f_k\Bigr].
\end{align}$$
...
$$\begin{align}
F=\int f\phi^r\quad\Longrightarrow\quad f_r=r!\,f(x_1)\delta(x_1-x_2)\cdots\delta(x_1-x_r).
\end{align}$$
...
$$\begin{align}
\alpha_d(F\star_WG)=\alpha_dF\star_{W'}\alpha_dG, \qquad \alpha_{d_2}\alpha_{d_1}=\alpha_{d_1+d_2}.
\end{align}$$
...
$$\begin{align}
C_{\mathcal D''\mathcal D'}C_{\mathcal D'\mathcal D} =C_{\mathcal D''\mathcal D}.
\end{align}$$

局域包含保持同一 label support，故

$$\begin{align}
C_{\mathcal D'\mathcal D,O_2}\,i^{\mathcal D}_{O_1O_2} =i^{\mathcal D'}_{O_1O_2}\,C_{\mathcal D'\mathcal D,O_1}.
\end{align}$$

对 time-slice 取相同 physical $\chi$；step 3 与 $[L,\chi]$ 的局部性给

$$\begin{align}
C_{\mathcal D'\mathcal D,N}\,\tau^{\mathcal D}_{NO} =\tau^{\mathcal D'}_{NO}\,C_{\mathcal D'\mathcal D,O}
\end{align}$$
...
$$\begin{align}
AK_s=K_MA,\quad AG_s^{R/A}=G_M^{R/A}A,\quad (A\otimes A)W_s=W_M.
\end{align}$$
