...
日期：2026-09-07。**结论：在当前指定的有限插入生成类和仅 $(\lambda,\hbar)$-adic completion 下，研究路线 D2 要求的 time-slice 满射不成立。** 反例已经出现在 $\lambda=\hbar=0$ 的 massive free scalar，空间是平坦二维环面。它不否定 [完整 Wick 分布核类的 time-slice](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar local structure implementation.md>)，也不否定采用额外分布拓扑完成的 perturbative AQFT。
...
$$\begin{align}
\mathcal B_0(O)=\operatorname{Alg}_{\mathbb C} \left\{F|_{\mathrm{Sol}_0}:F(\phi)=\int_O f(x)P(j^r\phi(x))\,dx, \ f\in C_c^\infty(O),\ r<\infty,\ P\text{ polynomial}\right\}.
\end{align}$$
...
$$\begin{align}
F(\phi)=\frac1{4!}\int f\phi^4, \qquad F|_{\mathrm{Sol}_0}\notin\mathcal B_0(N).
\end{align}$$
...
$$\begin{align}
(\epsilon_1,n_1)=(+,e_x),\quad (\epsilon_2,n_2)=(+,-e_x),\quad (\epsilon_3,n_3)=(-,e_y),\quad (\epsilon_4,n_4)=(-,-e_y),
\end{align}$$

$$\begin{align}
x_j=-\epsilon_jTn_j =(-Te_x,Te_x,Te_y,-Te_y)_j.
\end{align}$$
...
$$\begin{align}
(\partial^\alpha u_j^{(\nu)})(t,x) (\partial^\beta u_k^{(\nu)})(t,x)=0 \quad\text{在 }N.
\end{align}$$
...
$$\begin{align}
\theta_j(t,x)=n_j\cdot x-\epsilon_jt, \qquad \sum_{j=1}^4\theta_j=0.
\end{align}$$
...
$$\begin{align}
L(e^{i\nu\theta}a) =e^{i\nu\theta}\left[-2i\nu\epsilon (\partial_t+\epsilon n\cdot\nabla)a+La\right].
\end{align}$$
...
$$\begin{align}
(\partial_t+\epsilon_jn_j\cdot\nabla)a_{j,0}=0, \quad (\partial_t+\epsilon_jn_j\cdot\nabla)a_{j,r+1} =\frac{La_{j,r}}{2i\epsilon_j}, \quad a_{j,r+1}(0,\cdot)=0.
\end{align}$$
...
$$\begin{align}
\sup_t\bigl(\|w_j(t)\|_{H^3}+\|\dot w_j(t)\|_{H^2}\bigr) \le C\|Lv_j^{(\nu)}\|_{L^1_tH^2} \le C_J\nu^{2-J}.
\end{align}$$
...
当前 [Qn §4](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/phin renormalized quantum observable algebra gluing.md>) 的精确定义为

$$\begin{align}
\mathfrak Q_V^{\mathrm{fin}}(O) =\overline{\operatorname{Alg}_{\star}^{*}
\{\sigma_0\mathcal R_V^q(F):F\in\mathcal P_{\mathrm{loc}}(O)\}}^{(\lambda,\hbar)}, \qquad V=-\frac\lambda{4!}\int g\phi^4.
\end{align}$$
...
**Not verified:** 本 no-go 不证明扩大的 relative-$S$ 类的全阶实现，须另看 [completed relative-S construction](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar completed relative-S local structure.md>) 的独立论证及假设；也不能从本 no-go 推断完整 Wick／其他拓扑完成失败。无限或 $\nu$-dependent 插入表达、额外 Poisson closure、带负 $\hbar$ 次数的 saturation 不在本定理的源类中。
