...
日期：2026-09-07。模型和区域输入沿用 [自由局域结构实现](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar local structure implementation.md>)；interaction 为

$$\begin{align}
V=-\frac\lambda{4!}\sum_i\int g_i\phi_i^4, \qquad L_0\phi+\frac{\lambda g}{6}\phi^3=0.
\end{align}$$

$g_i$ 实、按 collar 匹配全部 jets、时间支撑紧，保留早期自由窗。本笔记给出实际经典解域上的传播／动力学、原有限插入量子类上的全阶包含／因果性／有限组合，以及指定二阶诊断。**原有限插入类的 time-slice 已有 [反例](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar insertion time-slice obstruction.md>)；本笔记不将它标为成立。**

## 1. 实际经典解域与背景依赖

使用 [closure §4](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar observable sewing closure.md>) 的 $C_t^\infty H^1$ 区域域：固定低阶输入界 $M$，收小
...
$$\begin{align}
\operatorname{supp}G_{\lambda,\phi}^{R/A}f\subset J^{\pm}(\operatorname{supp}f), \qquad E_{\lambda,\phi}=G_{\lambda,\phi}^A-G_{\lambda,\phi}^R.
\end{align}$$

对两个实际 nonlinear 解之差，势替换为 $\lambda g(\phi_1^2+\phi_1\phi_2+\phi_2^2)/6$，同一证明给因果依赖。Smooth matched backgrounds 在紧时间窗上有足够 Sobolev bounds；此陈述不要求 rough unmatched histories 在角点普通光滑。

局域 smooth finite-jet density $F$ 的物理一阶 variation 为 compact smooth source $F^{(1)}[\phi]$，故

$$\begin{align}
\{F,G\}_{\lambda}(\phi) =\langle F^{(1)}[\phi],E_{\lambda,\phi}G^{(1)}[\phi]\rangle
\end{align}$$
...
$$\begin{align}
DG_{\lambda,\phi}^{R/A}[h] =-G_{\lambda,\phi}^{R/A}(\lambda g\phi h)G_{\lambda,\phi}^{R/A}.
\end{align}$$
...
$$\begin{align}
e_t:U\longrightarrow U_t, \qquad e_t(z)=(\Phi_\lambda(z)(t),\partial_t\Phi_\lambda(z)(t)), \qquad U_t=e_t(U).
\end{align}$$
...
$$\begin{align}
\frac{\delta(H\circ b_N)}{\delta\psi(t)} =-\rho(t)p_t-\partial_t(\rho(t)u_t).
\end{align}$$
...
$$\begin{align}
S(V)=\sum_{r\ge0}\frac{(i/\hbar)^r}{r!}T_{R,r}(V^{\otimes r}), \qquad S_V(F)=S(V)^{-1}_\star\star S(V+F),
\end{align}$$

$$\begin{align}
\mathcal R_V^q(F)=\frac\hbar i\left.\frac{d}{ds}S_V(sF)\right|_{s=0}, \qquad O_V(F)=\sigma_0\mathcal R_V^q(F).
\end{align}$$
...
$$\begin{align}
S_V(sF+tG)=S_V(sF)\star S_V(tG) =S_V(tG)\star S_V(sF).
\end{align}$$
...
$$\begin{align}
\phi_1=-\frac16G^R(g\phi_0^3),\qquad \phi_2=\frac1{12}G^R\bigl[g\phi_0^2G^R(g\phi_0^3)\bigr].
\end{align}$$

二次插入在同一阶为

$$\begin{align}
\phi^2=\phi_0^2+2\lambda\phi_0\phi_1 +\lambda^2(\phi_1^2+2\phi_0\phi_2)+O(\lambda^3).
\end{align}$$
...
$$\begin{align}
-\frac{i\lambda\hbar}{2}(W_F^2-W^2) =\lambda\hbar\left(-G^R W+\frac i2(G^R)^2\right),
\end{align}$$
...
$$\begin{align}
T(A_1,A_2,F)-A_1\star T(A_2,F)-A_2\star T(A_1,F) -T(A_1,A_2)\star F+A_1\star A_2\star F+A_2\star A_1\star F,
\end{align}$$

其中 $A_i=\int g\phi_i^4/24$。实际二阶 contribution 是

$$\begin{align}
-\frac{\lambda^2}{2}\hbar^{a+b+c-2} \frac{r!\,(4!)^2}{24^2\,(r-a-b)!\,(4-a-c)!\,(4-b-c)!\,a!b!c!}
\int f_0g_1g_2D_{abc}\, \phi_0^{r-a-b}\phi_1^{4-a-c}\phi_2^{4-b-c}.
\end{align}$$
...
$$\begin{align}
\widehat{\mathcal R}_V^q(F) =\mathcal R_{Z(V)}^q(DZ(V)F).
\end{align}$$
...
$$\begin{align}
S_{V+v_-}(F)=U^{-1}\star S_V(F)\star U, \qquad U=S_V(v_-),
\end{align}$$
...
$$\begin{align}
H_\lambda(t)=\frac12\sum_i\int_{R_i} (p_i^2+|\nabla\phi_i|^2+m^2\phi_i^2) +\frac\lambda{24}\sum_i\int_{R_i}g_i\phi_i^4.
\end{align}$$

区域 EOM 给

$$\begin{align}
\frac{dH_{\lambda,i}}{dt} =-\int_{\partial R_i}\Pi_i\dot q_i +\frac\lambda{24}\int_{R_i}\dot g_i\phi_i^4.
\end{align}$$

释放人工 interfaces 后 flux 抵消，$\dot g$ 的功保留。xAct 检查的 off-shell identity 是

$$\begin{align}
\nabla^aT_{ab} =(\Box\phi-m^2\phi-\lambda g\phi^3/6)\nabla_b\phi -\lambda\phi^4\nabla_bg/24.
\end{align}$$
