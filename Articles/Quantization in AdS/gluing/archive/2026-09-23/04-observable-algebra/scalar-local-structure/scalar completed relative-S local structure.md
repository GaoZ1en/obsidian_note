...
日期：2026-09-07。本文给出 [有限插入 time-slice 反例](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar insertion time-slice obstruction.md>) 后的**另一份明确目标**：增加 coefficient distribution topology、多插入 coefficients 和显式 $\hbar$ saturation。模型、区域作用量、joint histories、response、interaction 与局域 insertion labels 均保留；改变的是生成操作与完成。
...
沿用 [自由实现](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar local structure implementation.md>) 的 smooth torus $M_s$ 和已构造的 on-shell polynomial coefficient space

$$\begin{align}
\mathcal W_s=\mathcal P_\mu(M_s)/\mathcal I_s, \qquad \mathcal I_s=\{(f_k):\kappa_k(f_k)=0\text{ 对全部 }k\}.
\end{align}$$
...
$$\begin{align}
\mathcal W_s^+=\mathcal W_s[[\hbar]][[\lambda]], \qquad \mathcal W_s^{\mathrm L}=\mathcal W_s((\hbar))[[\lambda]].
\end{align}$$
...
$$\begin{align}
S_V\left(\sum_j s_jF_j\right) =S(V)^{-1}_\star\star S\left(V+\sum_j s_jF_j\right)
\end{align}$$

的全部 auxiliary-source Taylor coefficients；$s_j$ 是独立 formal parameters。等价地可用归一化 coefficients

$$\begin{align}
O_{V;n}(F_1,\ldots,F_n) =\sigma_0\left[S(V)^{-1}_\star\star T_R(e^{iV/\hbar},F_1,\ldots,F_n)\right],
\end{align}$$
...
$$\begin{align}
\boxed{\mathfrak A_V^+(O) =\mathfrak A_V^{\mathrm L}(O)\cap\mathcal W_s^+.}
\end{align}$$
...
$$\begin{align}
S_D(F)=U^{-1}_\star\star S(F)\star U, \qquad U=S(D_-).
\end{align}$$
...
$$\begin{align}
\operatorname{Ad}_{U^{-1}}\mathcal W_s(N_0)((\hbar))[[\lambda]] \subset\mathfrak A_V^{\mathrm L}(N).
\end{align}$$
...
$$\begin{align}
\operatorname{Ad}_{U^{-1}} =\exp\left(-\frac i\hbar\operatorname{ad}_\star H_-\right).
\end{align}$$
...
$$\begin{align}
\mathcal W_s^{\mathrm L} =\operatorname{Ad}_{U^{-1}}\mathcal W_s^{\mathrm L}
...
$$\begin{align}
S_{b_2}(F)=U^{-1}\star S_{b_1}(F)\star U, \qquad U=S_{b_1}(c_-),\quad \operatorname{supp}F\Subset K.
\end{align}$$
...
$$\begin{align}
\alpha(S(F)) =S_v(-b')^{-1}\star S_v(-b'+F) =S(b_-)^{-1}\star S(F)\star S(b_-),
\end{align}$$
...
$$\begin{align}
b_0=\operatorname{id},\qquad b_n=-\sum_{j=1}^n a_jb_{n-j},\qquad \beta=\sum_{n\ge0}\lambda^n b_n.
\end{align}$$
...
$$\begin{align}
b_1=-a_1,\quad b_2=a_1^2-a_2,\quad b_3=-a_3+a_1a_2+a_2a_1-a_1^3.
\end{align}$$
...
外部 switching 的明确过去／未来分割给 [弱耦合笔记 §4](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar interacting local structure.md>) 的 $S_V(v_-)$ conjugation；现在它及 inverse 与新增完成相容。共同分割下的 $S_V(a)S_{V+a}(b)=S_V(a+b)$ 给复合；不同分割只在相关局域代数上比较。
...
实际经典 switched energy、背景 Peierls derivative 和自由 quantum energy extension 保持 [弱耦合笔记](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar interacting local structure.md>) 的原式。量子全阶 time-slice 的上述证明只需 causal factorization 和 coefficient control，不从 energy conservation 推出；也未新增全量子 stress Ward identity。
