...
- [x] Describe general compact surface self-sewing, orientability, vertex links and independent metric/action descent; separate static spatial identification from temporal topology change.
- [x] Extract the three proved gauge-sewing interfaces with their actual source/target types, action/trace/domain inputs, kernels, completions and independent global models; do not conflate classical, core and net arrows.
...
- [x] Extract the constant-coupling classical phi4 construction into `models/spacelike regions/non-gauge/1+1 phi4 classical observable algebra gluing.md`: define regional polynomial Poisson algebras directly, release the histories, identify the mismatch kernel and global image, and display the complete field/bracket and energy checks through $\lambda^2$. The independent 27-group verification distinguishes formal coefficient proofs from the finite canonical diagnostic.
- [x] Complete `models/spacelike regions/non-gauge/1+1 phi4 quantum observable algebra gluing.md` through total coupling degree $\lambda^2$ and two loops: explicit Wick coefficients and finite image terms match regionally/globally; the current contact fixes the spatial-stress correction and its dressing; opposite interface powers cancel and the regional stress integrals sum to the stationary time generator. The 38-group verification and the coefficientwise energy proof are recorded with the note. This completes the chosen endpoint; higher orders and independent multiple-stress sources are outside it.
- Further extensions of that example, such as independent multiple-stress/current sources, higher-derivative probes or actual operator limits, are outside the chosen stopping condition and are not active completion tasks.
...
- [x] Execute phase A of [the scalar local-structure route](<Articles/Quantization in AdS/gluing/archive/2026-09-23/04-observable-algebra/scalar-local-structure/scalar local structure research route.md>): record the actual regional inputs, transpose/matching dependencies and original test classes in the implementation note §1.
...
- [x] Start the [classical BV–BFV comparison](../../06-bv-bfv/README.md): explicit U(1) CS disk/chord BV/BFV data, full classical field/arrow sewing, independent current-polynomial kernel/image, opening/non-equivalence analysis and Abelian YM2 control.
- [x] Audit the candidate comparison through six rounds with 22 failure/obstruction cases; construct a continuous relative linear collar homotopy and pairing correction on the exterior-smooth domain. The package records 55 labelled checks and keeps full gauge directions.
- [x] Specify and construct the chiral disk/cap/corner continuous homotopy-limit model; compute the declared continuous polynomial BV cohomology and reconstruct its intrinsic Hamiltonian class. Point/energy counterexamples keep the old finite-current core distinct; see [the new comparison](../../06-bv-bfv/continuous_observables.md).
- [x] Construct relative continuous smoothing for finite common-conical cuts, coherent refinement comparisons, and the all-arity formal non-Abelian CS inverse with polynomial cdga and BV pairing/action homotopies; see [FC](../../06-bv-bfv/finite_cut_coherence.md) and [NR](../../06-bv-bfv/nonabelian_relative_descent.md).
- [x] Construct finite SU(2) disk full-field/arrows/action/CPS comparison and regular Hamiltonian reconstruction; define a continuum quantum Wick domain containing local composites/energy and prove the declared observable sewing. See [ND](../../06-bv-bfv/nonabelian_disk_observables.md) and [QD](../../06-bv-bfv/quantum_observable_domain.md).
- [x] Construct the declared normalized Abelian quantum collar kernel/mQME and interface/residual pushforward; retain its actual open-face corner and local BV-domain defects. See [QB](../../06-bv-bfv/quantum_collar_and_corners.md).
- [x] Derive the physical disk/chord history phase with cap Ward compensation, normalized source/quartet release and the continuum coherent cap comparison to Wick products/energy; retain the sharp arc/time-window vacuum-domain counterexamples. See [WH](../../06-bv-bfv/physical_wall_and_caps.md); this does not complete the full regional quantum BV state/pushforward.
- [x] Construct the canonical proper-cut BV quartet on the actual smooth endpoint domain, including local normal families, explicit quantum contraction and oriented Gaussian pushforward; see [QT](../../06-bv-bfv/quantum_cut_quartet.md). Its cochain-level extraction from the original regional BV action remains to be derived.
- [ ] Complete the [full BV–BFV programme](../../06-bv-bfv/programme.md): general bundle/self-sewing/source domains, nonformal off-shell BV, arbitrary reopening, actual quantum BV integration/mQME/corner anomalies, non-Abelian quantum and a propagating gauge model.
...
- [x] Repair charged angular smearing with argument-compact or correctly twisted tests, prove the adjoint/Hilbert-limit comparison, and record the ordinary-periodic k=2,q=3,n=1 counterexample; synchronize DtN/response signs and the vacuum scope.
...
- [x] Construct the regular two-puncture SU(2) sourced classical model, actual current-to-port map and reverse fiber product, corrected two-form, ordered fusion and independent pair-of-pants target.
- [x] Construct the specified SU(2)2 non-vacuum two-channel sewing with actual bounded primaries, positive transport norm, complete Gram kernel, onto maps and full outer local-net actions; include the k=1 control.
- [x] Define the source Wilson detector and actual four-puncture intertwiner bases, fix duality/framing phases, and compare the F-move on noncommuting channel observables.
- [x] Prove classical/quantum commuting diagrams for auxiliary rooted-tree contraction retaining both punctures, physical current intervals and every output channel.
- [ ] Before extending beyond this two/four-puncture prescription, define the additional charged networks and their domains/coherence data. Raw corner-current products do not all preserve the smooth-energy domain.
