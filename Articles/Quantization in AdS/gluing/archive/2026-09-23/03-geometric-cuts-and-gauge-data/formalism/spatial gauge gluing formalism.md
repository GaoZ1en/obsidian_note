...
> Status: repaired draft for criticism and iteration; with separately scoped Maxwell 2+1 orbit/collar and time-slab relation theorems and a smooth local non-Abelian radial-collar geometry theorem. This note adds a gauge layer to the spatial-partition/timelike-worldtube architecture of `gluing formalism.md`; it does not modify that manuscript and does not claim a new general PDE, quotient, or quantum theorem. Every theorem schema below is labelled by its current claim level.
...
$$\begin{align}
M^I&=I\times\Sigma, &I&=[t_-,t_+].
\end{align}$$

Take a spatial partition of one Cauchy slice,

$$\begin{align}
\Sigma&=\Sigma_1\cup_S\Sigma_2, &\operatorname{int}\Sigma_1\cap\operatorname{int}\Sigma_2&=\varnothing,
\end{align}$$
...
$$\begin{align}
\text{spatial partition and }\Gamma &\longrightarrow \text{original variational trace spaces }\mathcal Z_{i,\Gamma}^{\mathrm{inv}} \longrightarrow \text{polarization-independent sewing relation}\\
&\longrightarrow \text{admissible dynamical polarization and compatibility domain} \longrightarrow \text{regional time-slab input--output relation}\\
&\longrightarrow \text{on-shell action/exact-isotropic relation} \longrightarrow \text{relation composition, reconstruction, and associativity}.
\end{align}$$

Gauge theory adds three structures that cannot be represented by appending a formal quotient symbol:

$$\begin{align}
\boxed{ \text{constraint-compatible hyperbolic realization} +\text{connection descent/frame data} +\text{bulk unfixing and gauge-orbit completeness}.}
\end{align}$$
...
$$\begin{align}
S_i^{\mathrm{inv}}[A_i] &=\int_{M_i^I}\mathscr L(A_i,F_i)+S_{i,\partial_{\mathrm{phys}}}+S_{i,\mathrm{corner}},\\
\delta S_i^{\mathrm{inv}} &=\langle\mathcal E_i(A_i),\delta A_i\rangle_{M_i^I} +\Theta_i(t_+)-\Theta_i(t_-) +\langle\Pi_i,\delta a_i\rangle_\Gamma +\text{remaining declared corner terms},\\
...
$$\begin{align}
(\mathcal Z_{i,\Gamma}^{\mathrm{inv}},\omega_{i,\Gamma}^{\mathrm{out}}) &:=\text{trace data seen by this original Green identity, with its corner completion},\\
\operatorname{Tr}_{i,\Gamma}^{\mathrm{inv}}: \operatorname{Sol}_i&\longrightarrow\mathcal Z_{i,\Gamma}^{\mathrm{inv}}.
\end{align}$$
...
\left.\delta S_i^{\mathrm{inv}}\right|_\Gamma &=-e^{-2}\int_\Gamma(E_{y,i}\delta\alpha_i+F_{ya,i}\delta q_i^a).
\end{align}$$
...
$$\begin{align}
\widehat{\operatorname{Tr}}_{i,\Gamma}^{\chi_i}: \widehat{\operatorname{Sol}}_i^{\chi_i} &\longrightarrow\widehat{\mathcal Z}_{i,\Gamma}^{\chi_i}, &\widehat z_i&:=\widehat{\operatorname{Tr}}_{i,\Gamma}^{\chi_i}\widehat A_i.
\end{align}$$
...
$$\begin{align}
\mathfrak D_i^{\chi_i}\subset \Xi_i^{\chi_i,-}\times\mathcal B_{i,\mathrm{in}}^{\chi_i}.
\end{align}$$

The solution relation can be defined before existence or uniqueness is proved:

$$\begin{align}
\mathscr U_i^{\chi_i}:=\{(\xi_i^-,\xi_i^+;b_i,\widehat z_i):\;& \widehat A_i\in\widehat{\operatorname{Sol}}_i^{\chi_i},\quad (\xi_i^-,b_i)\in\mathfrak D_i^{\chi_i},\\
&\widehat z_i=\widehat{\operatorname{Tr}}_{i,\Gamma}^{\chi_i}\widehat A_i, \quad b_i=P_{i,\mathrm{in}}^{\chi_i}(\widehat z_i),\quad \xi_i^\pm=\widehat{\operatorname{Tr}}_{\Sigma_i^\pm}^{\chi_i}\widehat A_i\}.
\end{align}$$
...
**Claim level:** definitions and theorem inputs. The separate Maxwell $2+1$ dossier proves this relation and its graph property supportwise on one smooth finite-Fourier mixed Coulomb core. Its full cut-history equivariance is on the union over finite supports because an arbitrary finite-Fourier history can enlarge support; no fixed-$N$ invariance or inductive-limit topology is claimed. Maxwell $3+1$ and the general connection theory still supply no full IBVP theorem.
...
$$\begin{align}
\boxed{\rho_i^{\chi_i}:\widehat{\mathcal Z}_{i,\Gamma}^{\chi_i} \longrightarrow\mathcal Z_{i,\Gamma}^{\mathrm{inv}}}, \qquad \boxed{P_{i,\mathrm{in}}^{\chi_i}:\widehat{\mathcal Z}_{i,\Gamma}^{\chi_i} \longrightarrow\mathcal B_{i,\mathrm{in}}^{\chi_i}},\\
\rho_i^{\chi_i}\circ\widehat{\operatorname{Tr}}_{i,\Gamma}^{\chi_i} &=\operatorname{Tr}_{i,\Gamma}^{\mathrm{inv}}\circ F_i^\chi, &z_i&=\rho_i^{\chi_i}(\widehat z_i),\quad b_i=P_{i,\mathrm{in}}^{\chi_i}(\widehat z_i).
\end{align}$$
...
$$\begin{align}
\left.\delta(S_1+S_2)\right|_\Gamma &=\langle\Pi_1+\operatorname{Ad}_h^*\Pi_2,\delta a_1\rangle_\Gamma, &\boxed{\Pi_1+\operatorname{Ad}_h^*\Pi_2=0}.
\end{align}$$
...
$$\begin{align}
\mathsf S_{\Gamma,h}^{\mathrm{inv}} \subset\overline{\mathcal Z_{1,\Gamma}^{\mathrm{inv},o_\Gamma}} \times\mathcal Z_{2,\Gamma}^{\mathrm{inv},o_\Gamma}.
\end{align}$$
...
\left.\delta(S_1+S_2)\right|_\Gamma &=\langle\Pi_1+\operatorname{Ad}_h^*\Pi_2,\delta a_1\rangle_\Gamma +\int_\Gamma\langle\Pi_2^\alpha,D_\alpha\eta\rangle.
\end{align}$$
...
$$\begin{align}
\boxed{\int_\Gamma\langle\Pi_2^\alpha,D_\alpha\eta\rangle =-\int_\Gamma\langle D_\alpha\Pi_2^\alpha,\eta\rangle +\int_{\partial\Gamma}\langle n_\alpha^\Gamma\Pi_2^\alpha,\eta\rangle.}
\end{align}$$
...
$$\begin{align}
\int_\Gamma\langle\Pi_2^\alpha,D_\alpha\eta\rangle ={}&-\int_\Gamma\langle D_\alpha\Pi_2^\alpha,\eta\rangle -\left[\langle E_2^\perp,\eta\rangle_S\right]_{t_-}^{t_+} +\int_{I\times\partial S}\langle n_A^S\Pi_2^A,\eta\rangle.
\end{align}$$
...
$$\begin{align}
\mathcal G_{\Gamma,i}^{\mathrm{act,ext}} &:=\operatorname{Im}\!\left[\mathcal G_i^{\mathrm{allowed}} \longrightarrow\operatorname{Map}(\Gamma,G)\right],\\
\mathcal G_{S,h}^{\mathrm{ext}} &:=\operatorname{Im}\!\left[\mathcal G_\Sigma^{\mathrm{prop}} \xrightarrow{\operatorname{res}_{S,h}} \operatorname{Map}(S,G)^2\right].
\end{align}$$
...
$$\begin{align}
Q_i[\eta_i]&=\langle E_i^\perp,\eta_i\rangle_S, &\iota_{X_{\eta_i}}\Omega_i&=-\delta Q_i[\eta_i],\\
...
$$\begin{aligned}
\mathcal C_{h,\mathrm{phys}}^\chi:=\{u\in \widehat{\operatorname{Sol}}_1^{\chi_1}\times \widehat{\operatorname{Sol}}_2^{\chi_2}:\;& (\rho_1^{\chi_1}\widehat z_1,\rho_2^{\chi_2}\widehat z_2)
...
$$\begin{aligned}
\mathcal C_{h,\chi_M}^{\chi,\mathrm{slice}} :=\{u\in\mathcal C_{h,\mathrm{phys}}^\chi:\;& \exists\widetilde h, \widetilde h|_\Gamma=h,\\
&\operatorname{Match}_{\chi_M}^{\mathrm{slice}}(u;\widetilde h),\quad \widehat{\operatorname{Asm}}_h(u)\in\operatorname{Sol}^{\chi_M}(M)\}.
\end{aligned}$$
...
$$\begin{align}
\langle\mathcal E_M(A),\eta\rangle &=\sum_i\langle\mathcal E_i(A_i),\eta_i\rangle +\langle\Pi_1+\operatorname{Ad}_h^*\Pi_2,\iota_\Gamma^*\eta_1\rangle_\Gamma=0.
\end{align}$$
...
$$\begin{align}
\chi_M(\widehat{\operatorname{Asm}}_h u)&=0 \quad\text{and the global driver constraints hold distributionally}.
\end{align}$$
...
**Claim level:** two conditional general schemas. Existing mode calculations alone support only their modes. The separate cylinder dossier supplies a smooth finite-Fourier physical patching and collar regularity proof, while retaining the raw optional-slice counterexamples. Independently, `ym-radial-collar-assembly/` proves strong original-solution patching in one smooth exact-product, trivializable/reachable non-Abelian collar sector: tangential descent and full transmission determine all radial jets through the Yang--Mills/Bianchi recursion. This is not a Sobolev, non-product-collar, gauge-fixed IBVP or optional-slice theorem.
...
$$\begin{align}
\operatorname{Asm}_h:\mathcal B_h^{\rm geom} \longrightarrow\operatorname{Sol}(M)/\mathcal G_M^{\rm prop}.
\end{align}$$
...
(u,A)\in\mathscr A_h^\chi &\quad\Longleftrightarrow\quad A\text{ is an allowed assembly of the physical fields in }u.
\end{align}$$
...
$$\begin{align}
\operatorname{Unfix}_h(\mathcal C_{h,\mathrm{phys}}^\chi) &:=\mathcal G_M^{\mathrm{prop}}\cdot \widehat{\operatorname{Asm}}_h(\mathcal C_{h,\mathrm{phys}}^\chi).
\end{align}$$
...
**Claim level:** separate general proof obligations. In the Maxwell $2+1$ smooth finite-Fourier core, dossier Section 9 supplies original objects, based radial collar assembly, a smooth comparison lemma, complete intrinsic cut arrows and the two-sided orbit theorem. In the smooth exact-product non-Abelian collar sector of `ym-radial-collar-assembly/`, the radial ODE, covariant normal-jet recursion and comparison ODE supply the analogous original-solution orbit-set theorem with twisted cut arrows and trivial anchored stabilizers. The full regional based product is legitimate in these declared sectors by their comparison proofs, not because independent raw regional jets automatically patch.
...
$$\begin{align}
F_i^\chi\widehat A_i=((A^g)|_{M_i})^{r_i}, \qquad (\xi_i^-,P_{i,\mathrm{in}}^{\chi_i}\widehat z_i)\in\mathfrak D_i^{\chi_i}.
\end{align}$$
...
\operatorname{Hom}_{\mathcal R_h^\chi}(u,u') &:=\{g\in\mathcal G_M^{\mathrm{prop}}:
...
\ \Longleftrightarrow\ \exists g\in\mathcal G_M^{\mathrm{prop}}:
\widehat{\operatorname{Asm}}_h(u')=(\widehat{\operatorname{Asm}}_h(u))^g.}
\end{align}$$
...
For the smooth local Yang--Mills collar, `ym-radial-collar-assembly/` likewise defines the arrows before global orbit equality: wall-identity pairs obey $u_2|_\Gamma=h^{-1}u_1|_\Gamma h$. After radial repair the comparison equation $\partial_xv=vD_x-C_xv$ glues them to a global smooth proper arrow, and restriction plus inverse repair proves the converse. Fixed wall frames force covariantly constant stabilizers to be trivial on connected regions; without an anchor, holonomy centralizers remain.
...
$$\begin{align}
   \widehat{\operatorname{Asm}}_h\circ\operatorname{Res}_h^\chi &\sim_{\mathcal G_M^{\mathrm{prop}}}\operatorname{id}_{\operatorname{Sol}(M)},\\
   \operatorname{Res}_h^\chi\circ\widehat{\operatorname{Asm}}_h &\sim_{\mathcal R_h^\chi}\operatorname{id}_{\mathcal C_{h,\mathrm{phys}}^\chi}.
\end{align}$$
...
$$\begin{align}
   \widehat{\operatorname{Asm}}_h(u)\sim_{\mathcal G_M^{\mathrm{prop}}}\widehat{\operatorname{Asm}}_h(u') \quad\Longleftrightarrow\quad u\sim_{\mathcal R_h^\chi}u'.
\end{align}$$
...
$$\begin{align}
\boxed{\mathcal G_M^{\mathrm{prop}}\cdot \widehat{\operatorname{Asm}}_h(\mathcal C_{h,\mathrm{phys}}^\chi)=\operatorname{Sol}(M)},\\
\boxed{\mathcal C_{h,\mathrm{phys}}^\chi/\mathcal R_h^\chi \simeq\operatorname{Sol}(M)/\mathcal G_M^{\mathrm{prop}}}.
\end{align}$$
...
$$\begin{align}
\boxed{\widehat{\operatorname{Asm}}_h^*\Omega_M =\left.(\Omega_1+\Omega_2+\Omega_{\mathrm{corner}}^h) \right|_{\mathcal C_{h,\mathrm{phys}}^\chi}}.
\end{align}$$
...
In the smooth local Yang--Mills collar, the same original-CPS statement holds at geometry level. A regional gauge contraction is the Gauss boundary term $-\delta\int_S\langle E_i^\perp,\epsilon_i\rangle$; based terms vanish and common/twisted terms cancel on $E_1^\perp+\operatorname{Ad}_h^*E_2^\perp=0$. The variable-$h$ potential retains $-\langle E_2^\perp,h^{-1}\delta h\rangle_S$ exactly once. No completed tangent-kernel, quotient-manifold or Yang--Mills time-slab theorem is inferred.
...
\ker\left.\bigl(\Omega_1+\Omega_2+\Omega_{\mathrm{corner}}^h\bigr) \right|_{\mathcal C_{h,\mathrm{phys}}^\chi}.
\end{align}$$
...
1\longrightarrow\mathcal K_S\longrightarrow\mathcal G_\Sigma^{\mathrm{prop}} &\xrightarrow{\operatorname{res}_{S,h}}\mathcal G_{S,h}^{\mathrm{ext}}
\longrightarrow1.
\end{align}$$
...
$$\begin{align}
\mathcal C_{h,\mathrm{Cauchy}}^{\mathrm{pre}}/\mathcal G_\Sigma^{\mathrm{prop}} \simeq (\mathcal C_{h,\mathrm{Cauchy}}^{\mathrm{pre}}/\mathcal K_S) /\mathcal G_{S,h}^{\mathrm{ext}}.
\end{align}$$
...
No general functional time-slab reduction theorem is asserted here. Cylinder dossier Section 10 now proves, supportwise on its smooth finite-Fourier core, the regional mixed evolution graphs; full cut-history equivariance is on the union over finite supports and need not preserve an individual fixed-$N$ graph. It also proves the action on endpoints/histories/jets, preservation of the compatibility-domain family, relation-level two-sided assembly and model-specific reduction by stages. Its regional sequence with product based kernel is distinct from the global smooth restriction sequence above; the proof uses the intrinsic comparison groupoid rather than identifying those kernels. It proves no general topological exact sequence or completed evolution theorem.
...
$$\begin{align}
\mu_i^k&=\operatorname{Ad}_{k^{-1}}^*\mu_i, &\delta_\eta\mu_i&=[\mu_i,\eta] \quad\text{under the invariant-pairing identification},\\
Q_i[\eta]&=\langle\mu_i,\eta\rangle, &\iota_{X_\eta}\Omega_i&=-\delta Q_i[\eta].
\end{align}$$
...
$$\begin{align}
(\mathcal K_i^{\mathrm{BRST/BV}},Q_i,\text{ghost/antighost boundary data}) &\longrightarrow
\text{transition- and corner-compatible composition}\\
&\longrightarrow
(\mathcal K_M^{\mathrm{BRST/BV}},Q_M) \longrightarrow
H^\bullet(Q_M).
\end{align}$$
...
$$\begin{align}
h_{ik}=h_{ij}h_{jk}, \qquad \boxed{h_{ij}h_{jk}h_{ki}=1}.
\end{align}$$
...
$$\begin{align}
(T^*G)^3//(G\times G)\simeq T^*G, \qquad [(W_r,J_r)]\longmapsto(W_3W_2W_1,J_1),
\end{align}$$
...
| Yang--Mills smooth local collar | tangential connection $a$ and full original Green-dual momentum $\Pi$ | starts from smooth original regional solutions; no enlarged gauge-fixed trace or regional IBVP is supplied | strong radial-gauge patching proved in the exact-product trivializable/reachable collar sector | no Lorenz/Coulomb/global-slice theorem |
...
| Yang--Mills smooth local collar | Gauss is the electric component; full transmission fixes every common-normal $F_{xa}$ component | original restriction and radial-collar assembly give a two-sided orbit-set theorem in the declared local sector | complete twisted arrows, trivial fixed-wall stabilizers and geometry-level original CPS compatibility; no completed kernel/quotient theorem | no quantum statement |
...
The $1+1$ results prove their exact reduced models, not the analytic hypotheses they avoid. The separate cylinder and non-Abelian collar dossiers supply only the sector-specific positive and negative results stated in these tables.
...
\widehat{\mathcal Z}_{i,\Gamma}^{\chi_i} \xrightarrow{\ (\rho_i^{\chi_i},P_{i,\mathrm{in}}^{\chi_i})\ } \mathcal Z_{i,\Gamma}^{\mathrm{inv}}\times\mathcal B_{i,\mathrm{in}}^{\chi_i},\\
\prod_i\mathscr U_i^{\chi_i}&\longrightarrow\mathcal C_{h,\mathrm{phys}}^\chi
\ \underset{\operatorname{Res}_h^\chi}{\overset{\widehat{\operatorname{Asm}}_h}{\rightleftarrows}}\ \operatorname{Sol}(M),\\
\mathcal C_{h,\chi_M}^{\chi,\mathrm{slice}}&\subseteq\mathcal C_{h,\mathrm{phys}}^\chi \quad\text{optional},\\
\mathcal C_{h,\mathrm{phys}}^\chi/\mathcal R_h^\chi &\simeq\operatorname{Sol}(M)/\mathcal G_M^{\mathrm{prop}} \quad\text{only under the two-sided hypotheses}.
\end{align}$$
...
**Established within the source/model assumptions:** original action signs; fixed-$h$ descent and coadjoint transmission; the variable-$h$ canonical term on the original constraint/on-shell locus after full worldtube integration by parts; full transmission versus Gauss; exact framed $1+1$ Maxwell/Yang--Mills reduction and compact Haar/Peter--Weyl benchmarks; displayed higher-dimensional Maxwell modes and normalizations; the Maxwell $2+1$ smooth-core orbit/collar plus time-slab exact-isotropic relation theorem; and smooth local non-Abelian radial-collar assembly, choice independence, twisted-arrow completeness and geometry-level CPS compatibility in the declared exact-product trivializable/reachable original-solution sector. Non-Abelian raw flux is not a general central observable.
...
**Conditional/open in the general theory or beyond the stated Maxwell core and exact-product local Yang--Mills collar:** actual enlarged trace domains and continuous $\rho$; regional constraint-preserving IBVP; physical weak patching and stronger/non-product-collar regularity; optional global-slice patching; restriction coverage; effective regional arrow construction/extension; independence of assembly choices; time-slab reduction/evolution intertwining; on-shell exact-isotropic composition; completed CPS pullback and nondegeneracy modulo gauge; active restriction-kernel patching; gauge-fixing independence; continuum observable/center reconstruction and corners.

**Not claimed:** a universal gauge IBVP, all-mapping-group extension, a universal residual group containing Gribov copies, a smooth continuum quotient, Lagrangian maximality for the radial infinite-dimensional relation, Maxwell regulator convergence, sharp regional-Fock factorization, automatic disappearance of an electric center, or a BRST/BV gluing theorem. The Maxwell $2+1$ smooth-core orbit/collar and time-slab relation theorems are proved. The non-Abelian upgrade is only the smooth local exact-product original-solution collar geometry; no Yang--Mills time-slab, gauge-fixed PDE or completed functional theorem is started. Their extensions and the general formalism remain conditional.
...
**Not verified:** general IBVP, enlarged trace-domain continuity/surjectivity, physical weak patching in a specified Sobolev class, optional global-slice patching, restriction completeness beyond the declared original-solution cores, effective regional groupoid faithfulness beyond those cores, completed gauge-kernel equality, or continuum observable/BRST/BV results. The separate cylinder and non-Abelian collar dossiers record their narrower passed/failed claims; no general or completed-space theorem is inferred.

**Verified (current scoped tests):** `maxwell-2p1-spatial-gluing-kill-test/verification/checks.wl` passed 30/30 exact witnesses; the collar follow-up passed 9/9; the time-slab sign/action follow-up passed 14/14. The non-Abelian collar follow-up passed 12/12 exact matrix/algebra diagnostics, and an independent xAct/xCoba component check returned 4/4 zero residuals for the two tangential Yang--Mills signs, normal Bianchi sign and normal Yang--Mills/Gauss relation. The existing 20/20 mixed Maxwell regression belongs to the earlier mode round. The dossiers supply the analytical proofs and counterexamples; exact checks are not completeness evidence. The earlier unretained local checks above are not counted in these totals.
