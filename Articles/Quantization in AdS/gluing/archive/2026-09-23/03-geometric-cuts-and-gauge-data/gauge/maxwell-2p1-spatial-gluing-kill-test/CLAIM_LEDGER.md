...
| Kernel equality | proved only in the declared fully compatible smooth nondegenerate-mod-gauge tangent sector | Fixed $h$, or after passive presentation equivalence: based plus intrinsic cut vertical directions, with the Wilson/electric pair retained. The global temporal/Coulomb test class is only a nondegeneracy proof device and supplies no completeness/density theorem. Unquotiented variable-$h$ data also have passive vertical nulls. Large maps are discrete identifications, not tangent vectors. |
...
| Claim | Verdict | Exact evidence and boundary |
|---|---|---|
| Regional $\mathscr U_i^\chi$ | proved, Section 10.1 | Inputs are $(\xi_i^-,\alpha_i,q_i)$ on $\mathfrak D_i^\chi$; $\xi_i^+$ and the normal/gauge/jet entries of $\widehat z_i$ are outputs/constraints. The Section 1 one-dimensional KG/wave/ODE construction gives existence and uniqueness on each fixed finite angular support. |
| Cut-history action/domain invariance | proved on the finite-Fourier smooth inductive family, Section 10.2 | Zero-winding finite-Fourier lifts act on both endpoints, histories and all enlarged jets; they preserve the family of compatibility domains and intertwine evolution. They need not preserve a fixed history fiber, be identity at $t_\pm$, or preserve each fixed-$N$ support subspace. No inductive-limit topology is asserted. |
...
| Symplectic and power balance | proved, Section 10.8 | Original current gives pairwise flux cancellation. $H_i^{\rm pol}=H_i^{\rm raw}+e^{-2}\int\alpha_iE_{y,i}$ is a boundary-polarized regional energy/fixed-history-fiber representative obeying the port-work balance, not a claimed generator on one fixed phase space for time-dependent histories. Under $\alpha_i\mapsto\alpha_i+\dot f$ it shifts by $e^{-2}\int E_{y,i}\dot f$; the matched shifts cancel, and only the sum descends to the connected Hamiltonian. |
