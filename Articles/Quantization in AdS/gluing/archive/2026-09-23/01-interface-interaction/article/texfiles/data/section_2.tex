\section{Gluing two intervals}\label{sec:two-intervals}
...
\subsection{Exact analytic check}\label{subsec:interval-analytic}
...
Both coordinates increase toward the artificial cut. The total action is

\begin{align}
\begin{aligned}
S_g & = \frac{1}{2}\sum_{i=L,R}\int_{t_i}^{t_f}\mathrm{d}t\int_0^L\mathrm{d}s_{i}\left(\dot{\phi}_{i}^2-\phi_{i}'^{2}-m^2\phi_i^2\right) \\
    & -\frac{g}{2}\int_{t_i}^{t_f}\mathrm{d}t \left(\phi_{L}(L)-\phi_{R}(L)\right)^2, \qquad g>0
\end{aligned}
\end{align}
...
at the physical endpoints. The corresponding Hamiltonian is given by
...
\subsection{Quantized regional modes and finite Hamiltonian}\label{subsec:interval-finite-hamiltonian}
...
\qquad
\dot{\phi}_i(s)=\sum_{n=0}^{\infty}P_{i,n}u_n(s),
\end{align}
...
\subsection{Response-matched coupling}\label{subsec:interval-response-match}
...
\subsection{Spectrum reconstruction}\label{subsec:interval-spectrum}
...
\tightlist
\item
  A bare or direct penalty sets $g_N=g$ at fixed finite $g$. It converges because the omitted response vanishes, but its low-energy secular matrix retains the leading $T_N(0)=O(N^{-1})$ error.
\item
  At the joined endpoint, a hard projection imposes $b^{\mathrm T}(Q_L-Q_R)=0$ exactly inside the retained space. This is the naive finite-dimensional image of trace equality, but it discards the response of the omitted modes.
\item
  A response-matched penalty keeps the full retained coordinate space and chooses $g_N$ from the omitted response. It agrees with the continuum secular equation at the matching point.
\end{enumerate}
...
\subsection{Vacuum correlation functions}\label{subsec:interval-correlators}
...
\subsubsection{Cross-interface spacelike covariance}\label{subsubsec:cross-interface}
...
\subsubsection{One-sided interface-to-bulk correlators and sewing residual}\label{subsubsec:one-sided}
...
\subsubsection{Independent joined-theory references and scope}\label{subsubsec:correlator-references}
