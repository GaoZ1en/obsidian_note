...
Review base: `4fb3d286505cec4503e6901d133f5402d434b345` (2026-09-16, 01:32 Beijing time). The pre-review snapshot is retained in Git history. The active exposition is now `../all order perturbation result.md`; this archived extended treatment distinguishes an exact formula, a formal chart, an analytic local inverse, and a representation-theoretic quantization.
...
$$\begin{align}
\kappa^2=16\pi G,\qquad c_{\rm cl}=\frac{24\pi}{\kappa^2},\qquad [\xi_m,\xi_n]=-i(m-n)\xi_{m+n},\qquad D_m=2\pi m(m^2-1).
\end{align}$$
...
$$\begin{align}
\zeta=\sum_{N\ge2}(q_Na_N\xi_{-N}+q_N^*a_N^*\xi_N) =\sum_{|k|\ge2}z_k\xi_k,\qquad q_N=-\frac{(-i)^{N-1}}{\sqrt{D_N}}.
\end{align}$$
...
$$\begin{align}
g[a]=e^{\kappa\mathcal L_{\zeta[a]}}g^{(0)},\qquad g=g^{(0)}+\kappa\sum_{r\ge0}\kappa^rh^{[r]},\qquad h^{[r]}=\frac{\mathcal L_\zeta^{r+1}g^{(0)}}{(r+1)!}.
\end{align}$$
...
$$\begin{align}
h^{[3]}=\frac1{24}\mathcal L_\zeta^4g^{(0)},\qquad h^{[4]}=\frac1{120}\mathcal L_\zeta^5g^{(0)}.
\end{align}$$
...
$$\begin{align}
\Theta=F(\kappa A)\delta\zeta,\qquad \Omega_\zeta(u,v)=B(\Theta(u),\Theta(v)),\qquad H_p(\zeta)=\frac1\kappa B\bigl(\zeta,F(\kappa A)\xi_p\bigr).
\end{align}$$
...
$$\begin{align}
H_p=\sum_{n\ge1}\frac{(-1)^{n-1}\kappa^{n-2}}{n!} B(\zeta,A^{n-1}\xi_p).
\end{align}$$

Its first three terms reproduce the previous signed-mode charge sums. The next two are

$$\begin{align}
H_{p,2}=-\frac1{24}B(\zeta,A^3\xi_p),\qquad H_{p,3}=\frac1{120}B(\zeta,A^4\xi_p),
\end{align}$$
...
Indeed, the unprojected tangent equation is $\kappa F(\kappa A)X_p\zeta=e^{-\kappa A}\xi_p+\sigma_p$ with $\sigma_p$ in the vacuum stabilizer. This gives, without an expansion,

$$\begin{align}
X_0z_k=ikz_k.
\end{align}$$

For a low $u\in\{-1,0,1\}$ its leading high-mode action is $X_u\zeta=-P_>[\zeta,\xi_u]+O(\kappa)$, not half this value. In particular $H_u$ has no $\kappa^{-1}$ term, but is not zero away from the vacuum.

An explicit two-extra-order formula is obtained by setting

$$\begin{align}
M_j=\frac{(-1)^j}{(j+1)!}P_>A^j|_>,\qquad v_j=\frac{(-1)^j}{j!}P_>A^j\xi_p,\qquad X_p\zeta=\sum_{r\ge0}\kappa^{r-1}x_r.
\end{align}$$
...
$$\begin{align}
\Omega_r(u,v)=(-1)^r\sum_{j=0}^r \frac{B(A^ju,A^{r-j}v)}{(j+1)!(r-j+1)!}.
\end{align}$$

In particular

$$\begin{aligned}
\Omega_3(u,v)&=-\frac{B(A^3u,v)+B(u,A^3v)}{24} -\frac{B(A^2u,Av)+B(Au,A^2v)}{12},\\
\Omega_4(u,v)&=\frac{B(A^4u,v)+B(u,A^4v)}{120} +\frac{B(A^3u,Av)+B(Au,A^3v)}{48} +\frac{B(A^2u,A^2v)}{36}.
\end{aligned}$$
...
$$\begin{align}
\alpha=\sum_{r\ge1}\frac{\kappa^r}{r+2}\iota_{\mathcal E}\Omega_r, \quad \Omega_t=\Omega_0+t(\Omega-\Omega_0), \quad\iota_{Y_t}\Omega_t=-\alpha.
\end{align}$$
...
$$\begin{align}
T[f](x)=\frac{c_{\rm cl}}{24}(1-f'^2) -\frac{c_{\rm cl}}{12}\{f,x\},\qquad H_p[f]=\frac1{2\pi}\int_0^{2\pi}e^{ipx}T[f](x)\,dx.
\end{align}$$
...
$$\begin{align}
H_{\pm1}=\frac{c_{\rm cl}}{24} \left\langle e^{\pm ix}(-f'^2+w^2\pm2iw)\right\rangle.
\end{align}$$
...
For a matrix with equal diagonals, $\ell(S)$ denotes multiplication by $S_{++}+S_{+-}e^{-ix}+S_{-+}e^{ix}$. Put
...
$$\begin{align}
H_1=H_{-1}=0,\qquad H_0=\frac{4ab}{c_{\rm cl}} -\frac{176a^2b^2}{5c_{\rm cl}^3}+O(c_{\rm cl}^{-5}).
\end{align}$$
...
$$\begin{align}
\mathsf P_{pq}=-i(p-q)H_{p+q}-\frac{i}{\kappa^2}D_p\delta_{p+q,0}, \qquad \Omega=\frac12(\mathsf P^{-1})_{pq}\delta H_p\wedge\delta H_q, \qquad |p|,|q|\ge2.
\end{align}$$
...
$$\begin{align}
[\widehat H_m,\widehat H_n]=(m-n)\widehat H_{m+n} +\frac c{12}m(m^2-1)\delta_{m+n,0},\qquad \widehat H_n|0\rangle=0\quad(n\ge-1).
\end{align}$$
...
$$\begin{align}
\widehat H_0=\frac14[\widehat H_2,\widehat H_{-2}]-\frac c8, \quad\widehat H_1=\frac15[\widehat H_3,\widehat H_{-2}], \quad\widehat H_{-1}=\frac15[\widehat H_2,\widehat H_{-3}]
\end{align}$$
...
$$\begin{align}
|I\rangle=\widehat H_{-i_1}\cdots\widehat H_{-i_r}|0\rangle, \quad\widehat H_J^{(+)}=\widehat H_{j_s}\cdots\widehat H_{j_1}, \quad G_N(I,J)=\langle I|J\rangle,\quad\Pi_0=|0\rangle\langle0|.
\end{align}$$
...
$$\begin{align}
[\widehat H_1,\widehat H_{-n}]=(n+1)\widehat H_{1-n},\qquad [\widehat H_{-1},\widehat H_{-n}]=(n-1)\widehat H_{-(n+1)}.
\end{align}$$

Thus the $A_u^{(N)}$ require only finite word manipulation. On the algebraic vacuum module,

$$\begin{align}
\boxed{\widehat H_u=\sum_{N\ge2}\sum_{I\in\mathcal P_{\ge2}(N-u),\,J\in\mathcal P_{\ge2}(N)} (A_u^{(N)}G_N^{-1})_{IJ}\, \widehat H_{-I}\Pi_0\widehat H_J^{(+)},\qquad u=-1,0,1.}
\end{align}$$
...
$$\begin{align}
E_N^u(C)=\sum_{I\in\mathcal P_{\ge2}(N-u),J\in\mathcal P_{\ge2}(N)} C_{IJ}\widehat H_{-I}\widehat H_J^{(+)}.
\end{align}$$
...
\sum_{N>N_1>\cdots>N_r\ge2} \mathcal T_{NN_1}\cdots\mathcal T_{N_{r-1}N_r}S_{N_r},}
\end{align}$$
...
$$\begin{align}
J_0=\sum_{n\ge2}n d_n^\dagger d_n,\qquad J_{-1}=\sum_{n\ge2}\sqrt{(n-1)(n+2)}\,d_{n+1}^\dagger d_n, \qquad J_1=J_{-1}^\dagger.
\end{align}$$
...
$$\begin{align}
q^{\rm dD}_{\log}=\frac{3-\sqrt5}{2\sqrt5}\,\beta^2 r^{-(1+\sqrt5)}\log r+\cdots.
\end{align}$$
