...
\author{}
\date{Research draft, revision 2 --- 12 September 2026}
\begin{document}
\maketitle
\vspace{-15pt}
\begin{abstract}
...
To relate this energy coefficient to a logarithmic block coefficient, let $V$ be any first-order perturbation and let $P_{E_0}$ project onto a complete free degenerate block. Euclidean spectral evolution gives the exact first-order identity
\begin{equation}
...
\end{equation}
In addition to the symbolic-mass integrations, an independent coordinate implementation verifies every Einstein component for all three responses at $\Delta=2$. The all-mass source equations follow from the seed, Killing covariance and the three circular constraint identities, which are also checked symbolically.
...
The starting canonical reduction and radial reconstruction are recorded in the source notes at the pinned repository commit \cite{notes}. The first draft added the channel decomposition, local form-factor evaluation, crossed contact source, finite-band uniqueness proof and closed coefficients. Revision 2 adds the off-shell paired collapse, explicit Green currents and endpoint estimates, four-leg and logarithmic normalization, a scalar-primary branch construction, and the directly integrated noncircular matrix element at arbitrary mass. No change is made to the coefficient formulas.

The source package requires Python 3 and SymPy. From its directory, run
\begin{verbatim}
python verify_symbolic.py
python verify_tensor_modes.py
python verify_collapse.py
python verify_lie_casimir.py
python verify_contact.py
python verify_primary_branch.py
python verify_noncircular.py
python verify_noncircular_mass.py
python verify_radial.py --level 12 --output radial_report.json
pdflatex -interaction=nonstopmode -halt-on-error paper.tex
pdflatex -interaction=nonstopmode -halt-on-error paper.tex
\end{verbatim}
The supplied Makefile provides the same commands as \texttt{make check} and \texttt{make paper}. Exact reports are retained beside the scripts. The scripts fail on nonzero tested residuals.

The first script checks 21 rational identities with symbolic parameters, including an arbitrary hypergeometric index and the low-spin uniqueness determinant. The spin-one and spin-two primary tensors are checked component by component. The two new operator scripts independently test the linearized Einstein and Lie-Casimir identities on all six tensor basis components with arbitrary radial functions and symbolic frequency and angular momentum. Thus they check differential operators rather than just particular on-shell modes. The contact script checks a pointwise polynomial identity on arbitrary field values and gradients. The branch script checks the level-two Gram projection, norm and Ward overlap; its finite test supports, but does not replace, the all-grade character and Virasoro-block argument.

The noncircular scripts directly integrate the three four-leg pairings. One implementation checks every Einstein component for the three responses at $\Delta=2$ in radial coordinates. A second uses $x=f^{-1}$ and symbolic $\Delta$, checking the scalar equations, static seed constraints and all three same-sign circular constraints before performing the exact integrals. The angular response at arbitrary mass follows from exact Killing covariance, not from an inferred spectral matrix. Only after integration is its value compared with the spectrum.

Finally, the independent exact-rational reconstruction at $N\leq12$ gives 49 coefficients at each of four masses in three channels, for 588 comparisons, with all unused reconstruction residuals zero. These tests have been rerun for this revision. They do not establish an infinite-mode functional-analysis result. The full bare Fock-space dressing map, genuine additional-primary mixing, higher loops, and an exhaustive novelty assessment are not claimed. The accompanying \texttt{proof\_audit.md} distinguishes analytic arguments, executable checks and physical inputs.
