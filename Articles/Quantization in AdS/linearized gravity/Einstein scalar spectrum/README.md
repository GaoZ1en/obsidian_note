...
3. [OFPT two particle energy shifts.md](OFPT%20two%20particle%20energy%20shifts.md): a fresh derivation from the boundary Hamiltonian and constraint-reduced OFPT, including the lowest-pair integral, degenerate mixing, explicit finite radial sums, and primary reconstruction. Independent Hamiltonian checks reach radial index plus absolute spin eight. Its original sections 9.5–9.6 use high-spin inversion input and are retained as a comparison route. The [direct all-index supplement](OFPT%20direct%20all-index%20spectrum.md) removes that external spectral input using a direct canonical integral and the internal crossed recurrence. Term-by-term summation of the original Jacobi–Hahn formula and complete boundary-graviton mixing are not claimed.
...
6. [OFPT direct all-index spectrum.md](OFPT%20direct%20all-index%20spectrum.md): read after item 3 for the internal all-index proof. A ground-particle/rotating-particle canonical matrix element is evaluated for every angular momentum, free primary projection fixes the entire initial row, and the Einstein–Casimir recurrence determines all radial levels and spins without Lorentzian inversion or a prescribed high-spin tail. Includes the independent verification record and its retained scalar-primary-branch assumptions.
...
| `scripts/ofpt_all_index_checks.wl` | Original arbitrary-index primary normalization, source integrals, stress logarithm, crossed recurrence and low-spin uniqueness certificates. Its inversion-based completion is retained for comparison; the new direct proof does not require that external tail or finite-band uniqueness certificate. |
...
| `scripts/ofpt_direct_boundary_row_checks.py` | Independent Python canonical-moment reconstruction, direct arbitrary-angular-momentum integral, primary initial-row extraction, and internal all-index recurrence checks; no old spectrum data or radial implementation is loaded. |
| `scripts/ofpt_direct_boundary_row_results.json` | Preserved full audit report: 25 generic-mass coefficients, 64 coefficients per rational mass through level 14, initial-row tests, recurrence checks, symbolic identities and radial quadratures. The supplement separately records the PR-time rerun and its smaller generic-mass cutoff. |
