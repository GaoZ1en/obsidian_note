\section{Appendix C: Symplectic positivity and the admissible highest-weight roots}\label{app:symplectic-positivity}

In this appendix, we justify the choice of the highest-weight roots used in \hyperref[subsec:solving-modes]{Section~3.4}. The argument is the direct analogue of the positivity proof in the pure Proca theory, but now expressed in the notation of the Proca-Chern-Simons model.

The basic input is the bilinear identity derived in \hyperref[subsec:noether-killing-symmetries]{Section~2.2}:

\begin{align}
\Omega[\mathcal{L}_{\xi}A_{1},A_{2}] = - H_{\xi}^{(2)}[A_{1},A_{2}].
\end{align}

Although the symplectic form itself contains the Chern-Simons term, the bilinear Noether form $H_{\xi}^{(2)}$ appearing here is exactly the Maxwell-Proca expression written in \hyperref[subsec:noether-killing-symmetries]{Section~2.2}. This is because the Chern-Simons contribution reduces to a boundary term that vanishes under the asymptotic boundary conditions, as shown in \hyperref[app:near-boundary-contributions]{Appendix~A}.

\subsection{C.1 Positive-definite bilinears for separated modes}\label{subsec:positive-definite-bilinears}
...
\subsection{C.2 Consequences for the highest-weight roots}\label{subsec:highest-weight-root-consequences}
...
For the positive-energy one-particle modes selected in \hyperref[sec:quantization]{Section~3}, the result of the previous subsection therefore gives
...
Combining the Casimir equations of \hyperref[subsec:solving-modes]{Section~3.4} with the branch equation, one finds that for the $+$ branch the two candidate pairs of roots are
...
These are exactly the highest-weight roots used in \hyperref[subsec:solving-modes]{Section~3.4}.
