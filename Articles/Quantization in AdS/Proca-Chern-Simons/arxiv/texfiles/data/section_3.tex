\section{3. Representing the Hamiltonian system with a proper set of variables}\label{sec:quantization}
...
\subsection{3.1 The framework}\label{subsec:quantization-framework}
...
\subsection{3.2 Operator factorization for the equations of motion}\label{subsec:operator-factorization}
...
\subsection{3.3 The Killing symmetries}\label{subsec:killing-symmetries}
...
\subsection{3.4 Solving the modes}\label{subsec:solving-modes}
...
The alternative sign choices are excluded by the \hyperref[app:symplectic-positivity]{Appendix~C} argument: admissible positive-energy modes satisfy $\omega \pm m > 0$, which for a primary mode is equivalent to $h_{\pm} > 0$ and $\bar{h}_{\pm} > 0$. Solving the highest-weight equations then gives the primary modes
...
\subsection{3.5 Symplectic form}\label{subsec:symplectic-form}
...
Consider two arbitrary highest-weight modes $f_1$ and $f_2$. They satisfy the eigenvalue equations  \begin{align}
...
For the physical descendant modes constructed above, this implies that the only non-vanishing pairings are between a mode and its complex conjugate. With the normalization chosen in \hyperref[subsec:solving-modes]{Section~3.4}, we have
...
\subsection{3.6 Hamiltonian system}\label{subsec:discrete-hamiltonian-system}
...
Substituting this expansion into the symplectic form obtained in \hyperref[subsec:symplectic-form]{Section~3.5} yields
...
\subsection{3.7 The Noether charges of the Killing symmetries}\label{subsec:noether-charges-killing}
...
Substituting the mode expansion and using the orthogonality relations derived in \hyperref[subsec:symplectic-form]{Section~3.5} yields bilinear expressions in the oscillator variables. For the Cartan generators $\xi_0$ and $\bar\xi_0$, we obtain
...
Moreover, using the brackets of \hyperref[subsec:discrete-hamiltonian-system]{Section~3.6}, one verifies that
...
\subsection{3.8 Canonical quantization}\label{subsec:canonical-quantization}
...
So far, we have completely quantized the free massive Proca-Chern-Simons vector field model in the global AdS$_3$ spacetime. In \hyperref[sec:thermal-partition-function]{Section~5}, we will use this oscillator representation to construct the coherent-state path integral and evaluate the thermal partition function.
