\section{4. Application I: The Euclidean correlation function}\label{sec:euclidean-correlation-function}

In this section, we analytically continue the highest-weight mode expansion obtained in \hyperref[sec:quantization]{Section~3} to Euclidean signature and derive the Green's function equation satisfied by the Euclidean two-point function of the Proca-Chern-Simons theory.

\subsection{4.1 The Euclidean background}\label{subsec:euclidean-background}
...
\subsection{4.2 The Euclidean correlation function}\label{subsec:euclidean-correlation-function}

We now analytically continue the normalizable modes constructed in \hyperref[sec:quantization]{Section~3}. Under the Wick rotation, the field components transform as
...
\subsection{4.3 The explicit Euclidean correlation function}\label{subsec:explicit-euclidean-correlation-function}
...
\subsubsection{4.3.1 Mode sums in the scalar basis}\label{subsubsec:mode-sums-scalar-basis}
...
\subsubsection{4.3.2 Time ordering and branch correlators}\label{subsubsec:time-ordering-branch-correlators}
...
\subsection{4.4 Verification of the covariant Green's function equation}\label{subsec:covariant-green-function-check}

We now verify that the covariant part of the correlation function obtained above satisfies the expected Green's function equation. The non-covariant contact term exhibited in \hyperref[subsec:explicit-euclidean-correlation-function]{Section~4.3} is not included in this check and will be discussed separately.
...
\subsection{4.5 The path-integral interpretation of the contact term}\label{subsec:path-integral-contact-term}
...
Up to boundary terms, the corresponding quadratic kernel is precisely the operator that appeared in \hyperref[subsec:covariant-green-function-check]{Section~4.4}, so $G_{\mathrm{path}}^{\mu _{1},\mu _{2}}$ satisfies
...
These are the path-integral counterparts of the branch equations in \hyperref[subsec:covariant-green-function-check]{Section~4.4}.
...
For a more systematic constrained-Hamiltonian derivation of this statement, based on the Faddeev-Senjanovic path integral for second-class systems, see \hyperref[app:faddeev-senjanovic]{Appendix~B}.
