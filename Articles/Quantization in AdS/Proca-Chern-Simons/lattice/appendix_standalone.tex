...
\title{Lattice Check of the Proca Contact Term}
\author{}
\date{May 3, 2026}
...
\[
A^{\mu}(x)=\int \frac{\mathrm{d}p}{2\pi\sqrt{2\omega_{p}}}
\left(\varepsilon^{\mu}a(p)e^{ip\cdot x}
+\varepsilon^{*\mu}a^{\dagger}(p)e^{-ip\cdot x}\right),
\]
with a convenient polarization vector
\[
\varepsilon^{\mu}
=\left(\frac{p}{\mu},\frac{\omega_{p}}{\mu}\right),
\qquad
...
\[
\int \frac{\mathrm{d}p}{2\pi\,2\omega_{p}}
e^{-\omega_{p}(\tau_{1}-\tau_{2})+ip(x_{1}-x_{2})}
\left(\delta^{\mu\nu}+\frac{p^{\mu}p^{\nu}}{\mu^{2}}\right),
\]
...
{\mu^{2}\left(\mu^{2}+\widehat{\omega}_{n}^{2}+\widehat{k}_{m}^{2}\right)},
\\
\widetilde{G}_{\text{can}}^{xx}(n,m)
&=\frac{\mu^{2}+\widehat{k}_{m}^{2}}
{\mu^{2}\left(\mu^{2}+\widehat{\omega}_{n}^{2}+\widehat{k}_{m}^{2}\right)}.
\end{align}
...
\sum_{n=0}^{N_{\tau}-1}
\sum_{m=0}^{N_{x}-1}
e^{-\omega_{n}j+ik_{m}\ell}
\widetilde{G}_{\text{can}}^{\mu\nu}(n,m).
\end{align}

The corresponding path-integral construction is implemented by sampling Euclidean field configurations directly. The stochastic computation treats the lattice fields $A_{\tau}(\tau,x)$ and $A_{x}(\tau,x)$ as integration variables and samples them with weight $e^{-S_{\mathrm{E}}[A]}$.
The discretized Euclidean action is
\[
S_{\mathrm{E}}[A]
=\sum_{\tau,x}\Delta \tau \Delta x
\left[
\frac{1}{2}\left(D_{\tau}A_{x}-D_{x}A_{\tau}\right)^{2}
+\frac{1}{2}\mu ^{2}\left(A_{\tau}^{2}+A_{x}^{2}\right)
\right],
\]
...
This gives a direct numerical realization of the Euclidean path-integral correlator. Figure~\ref{fig:proca-contact-slice} shows a representative lattice correlator for $\mu=1$, $\beta=L=1$, and $N_{\tau}=N_{x}=32$. The left panel isolates the local difference $G_{\text{path}}-G_{\text{can}}$, and the right panel shows the equal-time slice. The path-integral and canonical curves coincide away from $x=0$, while the origin differs by the lattice delta-function contribution.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{proca_contact_slice_N32.png}
    \caption{The contact term and equal-time slice of the $\tau\tau$ Proca correlator on a $32\times32$ Euclidean lattice.}
    \label{fig:proca-contact-slice}
\end{figure}
