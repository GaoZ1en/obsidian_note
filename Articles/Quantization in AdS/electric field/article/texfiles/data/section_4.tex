\section{Representing the Hamiltonian system with a proper set of variables}\label{sec:quantization}

So far, we have reformulated the Maxwell theory as a Hamiltonian system in Section \ref{sec:hamiltonian-and-symmetries}, and we have constructed a complete basis of propagating normalizable modes in Section \ref{sec:mode-solutions}. In this section, we use the known mode basis to rewrite the solution space in terms of discrete variables, reduce the symplectic form, and carry out the canonical quantization procedure.

\subsection{The framework}\label{subsec:quantization-framework}
...
For the Maxwell theory on global AdS$_3$, the required basis is already known from Section \ref{sec:mode-solutions}. In the rest of this section, we use the Killing symmetries to constrain the symplectic pairing, determine the normalization, and then derive the reduced Hamiltonian system and the quantum oscillator algebra.

\subsection{Symplectic form}\label{subsec:symplectic-form}
...
Evaluating the symplectic form on the explicit modes of Section \ref{sec:mode-solutions} then gives

\begin{align}
\Omega[A_{nm},A_{n'm'}^{*}]&=-i\delta_{nn'}\delta_{mm'}, &\Omega[A_{nm},A_{n'm'}]&=0.
\end{align}

The explicit evaluation of this pairing on the temporal-gauge modes of Section \ref{subsec:temporal-gauge} is recorded in Appendix \ref{app:symplectic-normalization}.

This is the canonical normalization used already in Section \ref{sec:mode-solutions}. In particular, the overall constants in $\Psi_{nm}$, in the dual-scalar primary $\phi_{0,0}$, and in the descendant normalization are fixed by this symplectic condition.

\subsection{Hamiltonian system}\label{subsec:hamiltonian-system-discrete}

with the complete mode basis obtained in Section \ref{sec:mode-solutions}
...
\subsection{The Noether charges of the Killing symmetries}\label{subsec:noether-charges-killing}
...
Using the Noether-charge formula of Section \ref{sec:hamiltonian-and-symmetries},
...
With the phase-space convention fixed in Section \ref{sec:hamiltonian-and-symmetries}, the physical angular momentum is therefore
...
\subsection{Canonical quantization}\label{subsec:canonical-quantization}
