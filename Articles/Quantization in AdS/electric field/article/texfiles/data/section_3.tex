\section{Mode solutions}\label{sec:mode-solutions}
...
\subsection{Temporal gauge}\label{subsec:temporal-gauge}
...
where $\mathbb{N}=\{0,1,2,\ldots\}$. A convenient choice, anticipating the symplectic normalization stated in Section \ref{subsec:symplectic-form} and evaluated explicitly in Appendix \ref{app:symplectic-normalization}, is
...
\subsection{Dual scalar formalism}\label{subsec:dual-scalar-field-strength}
...
A convenient primary, written in the normalization that will be justified in Section \ref{subsec:symplectic-form} and Appendix \ref{app:symplectic-normalization}, is
...
\subsection{Equivalence of the two constructions}\label{subsec:equivalence-two-constructions}

We now compare the two constructions at the level of the field strength. More precisely, we compare the temporal-gauge representatives of Section \ref{subsec:temporal-gauge} with the scalar descendant basis constructed in Section \ref{subsec:dual-scalar-field-strength}. To match the notation used for the temporal-gauge modes, we first relabel the scalar descendants by
...
This is exactly the dual-scalar construction of Section \ref{subsec:dual-scalar-field-strength}, up to a constant phase choice for the scalar mode. In other words, the temporal-gauge representative and the dual scalar give the same field strength after the identification

\begin{align}
\phi_{nm}&\propto\omega_{nm}\Psi_{nm}.
\end{align}

The proportionality factor is a pure phase and does not change the field-strength basis or the symplectic normalization fixed in Section \ref{subsec:symplectic-form} and evaluated explicitly in Appendix \ref{app:symplectic-normalization}. Therefore the two constructions describe the same propagating Maxwell sector. The temporal-gauge basis keeps an explicit potential representative, while the dual-scalar basis keeps the gauge-invariant field strength manifest.
