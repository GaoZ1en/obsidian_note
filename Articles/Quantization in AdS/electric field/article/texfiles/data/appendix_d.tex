\section{Symplectic normalization of the propagating modes}\label{app:symplectic-normalization}

In this appendix, we evaluate the symplectic pairing of the temporal-gauge modes constructed in Section \ref{subsec:temporal-gauge} and fix the overall constant that appears in $\Psi_{nm}$. The selection rule of Section \ref{subsec:symplectic-form} already shows that only the pairing between a mode and its complex conjugate can be non-zero. We therefore only need to evaluate $\Omega[A_{nm},A_{n'm'}^{*}]$ explicitly.

\subsection{Evaluation of the symplectic pairing}\label{app:symplectic-pairing-evaluation}

We start from the bilinear pairing introduced in Section \ref{sec:hamiltonian-and-symmetries},
...
\subsection{Fixing the normalization constant}\label{app:fixing-normalization-constant}
...
This is exactly the overall constant used in Section \ref{subsec:temporal-gauge}. By the field-strength equivalence proved in Section \ref{subsec:equivalence-two-constructions}, the dual-scalar basis inherits the same normalization up to a pure phase, which does not affect the symplectic pairing.
