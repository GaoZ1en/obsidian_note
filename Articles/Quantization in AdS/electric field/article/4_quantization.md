...
$$\begin{align}
\mathcal{C}f_{j}&=c_{j}f_{j}, & \bar{\mathcal{C}}f_{j}&=\bar{c}_{j}f_{j}, \\
\mathcal{L}_{\xi_{0}}f_{j}&=-ih_{j}f_{j}, & \mathcal{L}_{\bar{\xi}_{0}}f_{j}&=-i\bar{h}_{j}f_{j},
\end{align}$$
...
$$\begin{align}
\Omega[\mathcal{C}A,B]&=\Omega[A,\mathcal{C}B], & \Omega[\bar{\mathcal{C}}A,B]&=\Omega[A,\bar{\mathcal{C}}B].
\end{align}$$

Applying these identities to the eigenvalue equations above gives

$$\begin{align}
(c_{1}-c_{2})\Omega[f_{1},f_{2}]&=0, & (\bar{c}_{1}-\bar{c}_{2})\Omega[f_{1},f_{2}]&=0, \\
(h_{1}+h_{2})\Omega[f_{1},f_{2}]&=0, & (\bar{h}_{1}+\bar{h}_{2})\Omega[f_{1},f_{2}]&=0.
\end{align}$$
...
$$\begin{align}
\Omega[A_{nm},A_{n'm'}^{*}]&=-i\delta_{nn'}\delta_{mm'}, &\Omega[A_{nm},A_{n'm'}]&=0.
\end{align}$$

The explicit evaluation of this pairing on the temporal-gauge modes of Section 3.1 is recorded in Appendix D.
...
$$\begin{align}
X_{a_{nm}}&=i\frac{\delta}{\delta a_{nm}^{*}}, & X_{a_{nm}^{*}}&=-i\frac{\delta}{\delta a_{nm}}.
\end{align}$$
...
$$\begin{align}
\mathcal{L}_{\partial_{t}}A_{nm}^{\mu}&=-i\omega_{nm}A_{nm}^{\mu}, & \mathcal{L}_{\partial_{\phi}}A_{nm}^{\mu}&=im\,A_{nm}^{\mu}.
\end{align}$$
...
H_{\partial_{\phi}}&=-\sum_{n=0}^{\infty}\sum_{m\in\mathbb{Z}}m\,a_{nm}^{*}a_{nm}.
\end{align}$$

With the phase-space convention fixed in Section 2, the physical angular momentum is therefore

$$\begin{align}
J&\equiv -H_{\partial_{\phi}}=\sum_{n=0}^{\infty}\sum_{m\in\mathbb{Z}}m\,a_{nm}^{*}a_{nm}.
\end{align}$$

These are the only charges needed in the later applications. The remaining Killing generators act non-diagonally on the mode basis and therefore give bilinear expressions that mix neighboring modes.
...
$$\begin{align}
a_{nm}&\mapsto\hat{a}_{nm}, & a_{nm}^{*}&\mapsto\hat{a}_{nm}^{\dagger},
\end{align}$$
...
$$\begin{align}
\hat{a}_{nm}\ket{0}&=0, & n&\in\mathbb{N}, & m&\in\mathbb{Z}.
\end{align}$$
...
\hat{H}_{\partial_{\phi}}&=-\sum_{n=0}^{\infty}\sum_{m\in\mathbb{Z}}m\,\hat{a}_{nm}^{\dagger}\hat{a}_{nm}, \\
\hat{J}&\equiv-\hat{H}_{\partial_{\phi}}=\sum_{n=0}^{\infty}\sum_{m\in\mathbb{Z}}m\,\hat{a}_{nm}^{\dagger}\hat{a}_{nm}.
\end{align}$$
