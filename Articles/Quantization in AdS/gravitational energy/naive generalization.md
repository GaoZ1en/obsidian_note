...
$$\begin{align}
\delta H_\xi[X]
=\int_\Sigma \mathrm{d}^2x\sqrt{\sigma^{(0)}}\,\tau^{(0)\mu}\xi^\nu S_{\mu\nu},
\end{align}$$
...
$$\begin{align}
\mathcal{W}_\Sigma[h^{(1)};\xi]
:=\int_\Sigma \omega\left(g^{(0)};h^{(1)},\mathcal{L}_\xi h^{(1)}\right).
\end{align}$$
...
$$\begin{align}
E^{(2)}
=\int_\Sigma \mathrm{d}^2x\sqrt{\sigma^{(0)}}\,\tau^{(0)\mu}(\partial_t)^\nu T_{\mu\nu}
+H^{\mathrm{grav}}_{\partial_t}[h^{(1)}].
\end{align}$$
...
$$\begin{align}
J^{(2)}
=\int_\Sigma \mathrm{d}^2x\sqrt{\sigma^{(0)}}\,\tau^{(0)\mu}(\partial_\phi)^\nu T_{\mu\nu}
+H^{\mathrm{grav}}_{\partial_\phi}[h^{(1)}].
\end{align}$$
