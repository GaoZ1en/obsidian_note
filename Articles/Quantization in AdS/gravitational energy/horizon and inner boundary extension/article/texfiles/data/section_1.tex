...
\subsection{Boundary polarization and slice flux}\label{subsec:boundary-polarization}
...
\tightlist
\item
  the allowed variations make the action differentiable, possibly after retaining a declared source-work term;
\item
  the pullback of $\delta\mathcal B_a$ and the joint curvature vanishes on a Lagrangian boundary polarization, is field-space exact with its Hamiltonian retained, or is bounded uniformly and vanishes in the regulator limit;
\item
  the variations satisfy the linearized bulk equations, so the bulk constraint current vanishes;
\item
  the same topology and orientation are used along the regulator family, so the limiting two-form is well defined.
\end{enumerate}
