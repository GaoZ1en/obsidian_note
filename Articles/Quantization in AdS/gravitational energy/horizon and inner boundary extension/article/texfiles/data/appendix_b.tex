\section{Quadratic area and the Hollands--Wald boundary problem}\label{app:area-hw}

\subsection{Fixed-embedding expansion}
...
For a one-dimensional surface in three dimensions, the tangential quadratic term in (\ref{eq:determinant-expansion}) is nonzero. The Noether density contains the same tangential and second-order terms, so they cancel in the Noether-minus-area difference. The remaining difference is the normal-plane determinant. In the project null frame with $k\cdot l=+1$, this gives
...
\subsection{Derivation of the displacement terms}
...
\subsection{Compact and noncompact Jacobi problems}
...
\subsection{Field dependence and the cocycle}
...
\subsection{Independence of the ambient extension}\label{subsec:extension-independence}
