...
=g^{(0)\mu\nu}
-\kappa h_1^{\mu\nu}
+\kappa^2\left(h_1{}^\mu{}_\rho h_1^{\rho\nu}-h_2^{\mu\nu}\right)
+O(\kappa^3).
\end{align}$$
...
$$\begin{align}
S_{\mathrm{bulk}}
&=\frac{1}{2\kappa^2}\int_M\mathrm{d}^3x\sqrt{-g^{(0)}}
\left[
\mathcal{L}^{(0)}
+\kappa\mathcal{L}^{(1)}[h^{(1)}]\right.\\
&\quad\left.
+\kappa^2\left(
\mathcal{L}^{(1)}[h^{(2)}]
+\mathcal{L}^{(2)}[h^{(1)}]
