...
| C0-boost | $m\mapsto m-\beta_+-\beta_-$ and $\omega_A^{(+)}\mapsto\omega_A^{(+)}+D_A\beta_+$ | proved | branch-adapted normal-connection definition | direct rescaling | choice quotient versus retained remains open beyond Stage 1 |
| C0-shared-normal | $\omega_A^{(+)}+\omega_A^{(-)}=-D_A m$ and $\widehat\omega_A=(\omega_A^{(+)}-\omega_A^{(-)})/2$ is the shared torsion datum | proved | $\ell_+\cdot\ell_-=-e^{-m}$ | differentiated inner product; calculation 03; V16 | spin-1 symplectic partner still fixed |
...
| C2-outer-shape | a variable outer traceless metric requires a shape port $\pi_s^{AB}\delta\bar q_{AB,sL}$; it vanishes on the fixed-outer-two-metric pullback | conditional | variable outer metric and differentiable extended port | moving-endpoint analysis in calculation 03 | unique $\pi_s^{AB}$ needs a selected closing action |
...
| C2-Reis-corner | the positive HF corner, the normal-scale terms, and the moving lower limit reproduce $+C_G\delta\lambda_R\wedge\delta\Omega_0$ plus Reisenberger's shared conformal endpoint term | proved | monotonic area, fixed endpoint maps, fixed outer two-metric | calculation 03; Reisenberger eqs. (105)--(118); V18--V19 | varying endpoint maps belong to Stage 3 |
| C2-Reis-full | the affine reduced form equals Reisenberger's fixed-map area-gauge form on the monotonic-area, fixed-outer-two-metric sector | conditional | $\theta_s\neq0$ along generators; fixed endpoint maps and outer two-metric | calculation 03; V17--V19 | not a global chart; variable outer shape needs C2-outer-shape |
| C2-compose | two consecutive spin-0+spin-2 segments compose under continuity of $(q_{AB},B_{AB})$, equivalently $(\Omega,\bar q_{AB},\theta,\sigma_{AB})$ | proved | one common forward generator; $C^1$ matching; coherent cut incidences | calculation 03 equation (4.3); V20--V21 | off locus the explicit obstruction is $\mathcal J_{AB}=[B_{AB}]$ |
| C2-associative | three-segment composition is associative | proved | the same first-jet matching at both cuts | integral additivity; calculation 03 equation (4.7); finite-mode V21 | finite-mode check is a regression, not the analytic proof |
...
| C3-spin1 | reduced spin-1 form with $\widehat\omega_A$ | open | embedding/endpoint-map polarization must be chosen | Damour transport is ready; branch dictionary established | pullback of $-\bar\eta_A\delta\ell^A$ and gauge quotient |
...
| V17 | affine-to-area focusing Jacobian, $B_0=2/\theta_0$, and field variations | exact Wolfram finite-mode regression | diagonal constant-slope conformal profile |
...
| V19 | cancellation of the two normal-scale corner terms against $m-\lambda_R$ | exact Wolfram two-form matrix algebra | corner-scale sector only |
...
| V21 | two-segment additivity and three-segment associativity | exact Wolfram finite-mode regression | diagonal conformal mode; analytic statement uses integral additivity |
| V22 | diagonal focusing and shear-free limit | exact Wolfram identity/limit | tested profile only |

Passing V0--V22 proves only the identities stated.  It does not prove a
unique variable-shape closing-wall action, an area chart through $\theta=0$,
a reduced spin-1 block, full-gauge nondegeneracy, completeness, surjectivity,
