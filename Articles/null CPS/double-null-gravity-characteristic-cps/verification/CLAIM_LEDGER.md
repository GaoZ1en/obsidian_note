...
| C3-spin1 | reduced spin-1 form with $\widehat\omega_A$ | open | retain $(s_\pm^A,\widetilde\tau_{\pm A})$ and impose the generator-chart constraint | Stage-3 polarization gate fixed in calculation 04; Damour transport ready | pullback of $-\bar\eta_A\delta\ell^A$ and gauge quotient not started |
...
| V19 | one-form corner/normal-scale transformation after $m=\lambda_R+\log|B_{+0}B_{-0}|$ | exact Wolfram one-form algebra | lower-scale remainder is derived analytically in calculation 04 and independently exercised by V23 |
...
Passing V0--V25 proves only the identities stated.  It does not prove a
closing-wall action or its port momenta, an area chart through $\theta=0$, a
reduced spin-1 block, full-gauge nondegeneracy, completeness, surjectivity,
