...
- [ ] Action: choose the paper's compact cut: $S^2$ in a fixed bundle sector or $T^2$ with harmonic modes displayed.
  ...
  Check: the corner compatibility list is complete.
- [ ] Action: compute the reduced Poisson brackets of transverse Hodge modes.
  Context: the current result identifies the two-form but does not invert every generator zero mode.
  Check: compare with the spacelike Coulomb/radiative decomposition.
- [ ] Action: perform the Peierls comparison for compactly supported $F$-smearings.
  Context: B1 only.
  Check: the characteristic inverse equals the causal propagator bracket.

## Questions to Resolve Before Manuscript Drafting

- Which endpoint polarization is physically intended at the two future cuts?
...
- B2 cross covariance and state reconstruction.
- Sharp regional tensor-product factorization.
- Non-Abelian and gravitational constraint algebras.
- Caustics and infinite null hypersurfaces.
