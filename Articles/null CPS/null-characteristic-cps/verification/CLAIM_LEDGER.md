...
| A5c | $(F_{vA}|_{N_+},F_{uA}|_{N_-})$ alone form complete double-null characteristic data | false | smooth product solutions before fixing every flux sector | pure electric Coulomb regression within fixed Chern sector; pure magnetic regression as local/cross-Chern diagnostic | in the main $Q_E=n=0$ energy tangent space, radiative completeness instead follows from Q2--Q4d |
...
| Q3 | Theorem I: $\operatorname{Res}_{U,V}:\mathcal E^1_{U,V}(a_\ell)\to\mathcal G^1_{U,V}$ is a bounded isomorphism with the Bessel inverse | proved + checked symbolically/numerically | matched $H^1$ traces, fixed $\lambda_\ell>0$ | calculation 07, explicit estimates (3.4), (3.11), kernel and reconstruction regressions | finite patch is a local restriction/germ chart, not an autonomous regional gauge phase space; no uniform same-order full-tower estimate |
| Q4a | Theorem II: complete-half-cross restriction $R_X:\mathcal S_{E,\mu}\to\mathfrak X_{E,\mu}$ is injective and symplectic | proved | $\mu>0$, Cauchy data $H^1\oplus L^2$, zero-sector radiative tower | calculation 12: finite Stokes identity, tail/exterior-flux lemma, Cauchy nondegeneracy | massless limit and curved backgrounds excluded |
| Q4b | $\operatorname{Ran}R_X$ has the intrinsic fixed-point characterization $P_\mu(f,g)=(f,g)$ with spectral integrability | proved | closed two-sheet analysis transform on matched $H^1$ energy profiles | calculation 13: explicit $T_\mu$, $S_\mu$, $T_\mu S_\mu=1$, $P_\mu^2=P_\mu$ | generic matched half-line pairs need not satisfy the nonlocal projector condition |
...
| Q6 | the boxed two-sheet transform $a_k[f,g]$ is injective with explicit inverse/Plancherel relation | proved + checked symbolically | intrinsic complete profiles, $\alpha\beta=\lambda_\ell/2$ | calculations 07 and 13; both principal-value cancellations and delta Jacobians | finite-cross data alone have no unique global coefficient map |
| Q6b | $J_X$ is explicit on profiles: $T_XJ_X=-iT_X$ and kernel (6.3) mixes both sheets | proved + checked symbolically | stationary ultrastatic complex structure, intrinsic one-particle completion | calculation 13 and $J_X$ kernel-coefficient regression | finite rectangle does not select this B2 input; no locality on either sheet |
...
| Q10 | radiative composition is canonically weak-symplectically associative mode by mode | proved; checked exactly/numerically on a finite basis | linear product benchmark, matched traces, explicit Goursat uniqueness | calculations 11 and 14; distinct $G_{(12)3}$/$G_{1(23)}$, full-rank internal-edge cancellation, exact $S^T\Omega_{\rm outer}S=S^T\Omega_{\rm cells}^{\rm red}S$ identity | full Maxwell charged-frame, nonlinear, non-Abelian and singular reductions excluded |
...
All four scripts were executed with Wolfram Engine 14.3.0. They finished with `ALL CHECKS PASSED`, `ALL S2 MASTER/QUANTIZATION CHECKS PASSED`, `ALL MASTER MODE NUMERICAL CHECKS PASSED`, and `ALL THREE-CELL SYMPLECTIC CHECKS PASSED`. The single-mode reconstruction errors were below $1.5\times10^{-15}$; the finite-$k$ timelike and spacelike commutator errors were below $6\times10^{-4}$. In the three-cell check both internal edge matrices had rank six, all exact orientation/symplectic matrix residuals vanished—including $S^T\Omega_{\rm outer}S=S^T\Omega_{\rm cells}^{\rm red}S$—and the two parenthesized reconstruction maps agreed with direct reconstruction to approximately 29 decimal digits. The independent earlier `xact_verify_residuals` run used the `components` profile, loaded xTensor/xPert/xTras/xCoba without errors, and returned `allZero=true` for the scalarized inverse-metric, Hodge, null-pullback and corner-moment-map residuals.

The analytic proof of Q3 is the Volterra/Riemann theorem and estimates in calculation 07; the numerical mode reconstruction is only a regression. Q4a--Q4d are proved by the trace/tail/density and analysis--synthesis arguments, not by the finite symbolic weights alone. Likewise, Q5 is proved by the continuum kernel argument, not by the finite Galerkin rank; Q9 is proved by the Fourier--Bessel/Green identities, not by a finite momentum cutoff; and Q10 is proved by Goursat uniqueness/current conservation, while the three-cell script is an orientation/trace-map regression.

## Verified

- All exact finite component and orientation identities listed as checked symbolically.
- The pure Coulomb kernel within a fixed Chern sector and the pure magnetic local/cross-Chern diagnostic.
- Formal smooth frame reduction plus the nontrivial radiative three-cell symplectic regression.
