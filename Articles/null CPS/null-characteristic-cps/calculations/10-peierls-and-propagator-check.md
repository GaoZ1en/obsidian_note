...
Equation (5.1) concerns \(\ell\geq1\) in the main $Q_E=n=0$ theorem. The \(\ell=0\) electric sector has no normally hyperbolic local oscillator and therefore no kernel (2.4) in the radiative sum. A fixed nonzero flux or Chern class is an infinite-energy background/sector label on the noncompact $z$ line, not a radiative Peierls mode. If an extended electric frame is retained, its finite-dimensional canonical bracket is added separately and is not generated by \(E_{{\mathrm P},\lambda_\ell}\).
