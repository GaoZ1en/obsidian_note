...
| Z-05 | Every vector of the fixed-switch smooth trace core has a unique radiative/$\widehat a$/memory decomposition with finite weakly nondegenerate symplectic pairing. | proved | `calculations/08`, sections 4 and 11 | `T4-kin`; changing the switch gives a canonical shear, not an invariant raw-$a$ oscillator. |
...
| T1-aff(k) | For fixed $k$, a center-regular master extends $C^{k+1}$ through $B$ iff the incoming and $S_j$-scattered traces satisfy the two-ended weighted affine limits and corner matching. | proved | `calculations/10`, Theorems 3.1 and 6.1 | Fixed harmonics or rapid-angular smooth core; Maxwell tensor is $C^k$. |
| T1-aff | The maximal fixed-Sobolev affine Maxwell/corner range is a continuous bijective restriction target. | conditional | `calculations/10`, sections 6--9 | Missing completed two-ended Hardy topology and uniform full-tower Goursat estimate. |
| T1-aff-reg | The finite affine-derivative, Mellin-pole, Schwartz-counterexample, angular-loss, and switch-shear samples satisfy their thresholds. | proved | `scripts/affine_horizon_checks.py`; `calculations/10`, section 10 | Finite regression only; it does not prove `T1-aff(k)` or the maximal range. |
...
| T4-kin | On a fixed-switch trace core, Coulomb/topology, $\widehat a$, endpoint memory, large gauge, relative frame, and soft radiative sectors have the stated unique kinematic decomposition and finite canonical form. | proved | `calculations/08`, Theorem 11.1 | Memory is conjugate to $\widehat a=a+2\ell_h(x)$; switch changes act by canonical shear. |
| T4-bulk | Restriction onto the maximal trace/memory subspace realized by smooth-across-$B$ center-regular bulk solutions is a completed range theorem. | conditional | `calculations/08`, section 12; `calculations/10` | `T1-aff(k)` gives a finite-order subrange; maximal completion remains open. |
...
| T5-C* | The BD covariance satisfies the KMS boundary relation on the dense analytic Weyl *-algebra. | proved | `calculations/07`, sections 6 and 8 | This does not assert point-norm continuous dynamics on the full abstract Weyl algebra. |
| T5-C*-norm | The static Weyl automorphisms form a point-norm strongly continuous $C^*$-dynamical system on the full abstract Weyl algebra. | false | `calculations/05`, section 6; `07`, equation (6.6) | Generic distinct Weyl generators have norm distance two. |
...
| N-06 | The two-ended $S_j$-dependent affine/Mellin range can become a nontrivial paper core. | conditional | `calculations/10`; `literature/literature-comparison.md` | Finite-order theorem is proved, but maximal completion and novelty relative to global-horizon results remain open. |
...
| N-10 | The repaired $dS_4$ project presently clears the article novelty gate. | false | `literature/literature-comparison.md`, section 11 | Continue only the `T1-aff/T4-bulk` kill test; freeze and pivot only if it fails. |
