...
$$\begin{align}
K(t,r) & =\frac{e^{-(\mu ^{2}+1)t}}{(4\pi t)^{3/2}}\frac{r}{\sinh r}e^{-\frac{r^{2}}{4t}}
\end{align}$$
...
$$\begin{align}
(\tau,\phi)\sim (\tau+\beta,\phi+\theta)
\end{align}$$

where $\displaystyle{t}$ is the Euclidean time coordinate and $\displaystyle{\phi}$ an angular coordinate. 