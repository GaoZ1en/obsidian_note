the first relation

$$\tag{.}\begin{align}
AP_{n-1}^{(\mu+1,m)}(z)+BP_{n-1}^{(\mu,m+1)}(z)+CP_{n}^{(\mu,m-1)} =0 \\
A{}_{2}F_{1}(-n+1,n+m+\mu+1;\mu+2;z)+B{}_{2}F_{1}(-n+1,n+m+\mu+1;\mu+1;z)+C{}_{2}F_{1}(-n,n+m+\mu;\mu+1;z) \\
AF(\gamma+1)+BF+CF(\alpha-1,\beta-1)=0
\end{align}$$

where $\displaystyle{\alpha=-n+1,\beta=n+m+\mu+1,\gamma=\mu+1}$.

$$\tag{.}\begin{align}
\gamma F(\alpha-1,\beta-1) & =\gamma(1-z)F+(\gamma-\alpha-\beta+1)zF(\gamma+1) \\
\gamma F(\alpha-1) & =\gamma(1-z)F+(\gamma-\beta)zF(\gamma+1) \\
\gamma F(\beta-1) & =\gamma(1-z)F+(\gamma-\alpha)zF(\gamma+1) \\
(\alpha-\beta)F & =\alpha F(\alpha+1)-\beta F(\beta+1)
\end{align}$$

then

$$\tag{.}\begin{align}
2m  P_{n}^{(\mu,m-1)'}\left( \frac{r^{2}-1}{r^{2}+1} \right)=(\mu+n)(\mu+m+n)r^{2}P^{(\mu,m+1)}_{n-1}\left( \frac{r^{2}-1}{r^{2}+1} \right)-n(\mu+m+n)(1+r^{2}) P^{(\mu,m-1)}_{n}\left( \frac{r^{2}-1}{r^{2}+1} \right)
\end{align}$$

---

second relation

$$\tag{.}\begin{align}
AP^{(\mu,m+1)'}_{n}+BP_{n}^{(\mu,m+1)}+CP_{n+1}^{(\mu,m-1)} & =0 \\
AP^{(\mu+1,m+2)}_{n-1}+BP_{n}^{(\mu,m+1)}+CP_{n+1}^{(\mu,m-1)} & =0 \\
A{}_{2}F_{1}(-n+1,n+m+\mu+3;\mu+2;x)+B{}_{2}F_{1}(-n,n+m+\mu+2;\mu+1;x)+C{}_{2}F_{1}(-n-1,n+m+\mu+1;\mu+1;x) & =0 \\
AF(\alpha+1,\beta+1,\gamma+1)+BF+CF(\alpha-1,\beta-1) & =0
\end{align}$$

where $\displaystyle{\alpha=-n,\beta=n+m+\mu+2,\gamma=\mu+1}$

$$\tag{.}\begin{align}
\gamma F(\alpha-1,\beta-1) & =\gamma(1-z)F+(\gamma-\alpha-\beta+1)zF(\gamma+1) \\
(\alpha-\beta)(1-z)F(\alpha+1,\beta+1,\gamma+1) & =(\gamma-\beta)F(\alpha+1,\gamma+1)-(\gamma-\alpha)F(\beta+1,\gamma+1) \\
(\alpha-(\gamma-\beta)z)F(\alpha+1,\gamma+1) & =\gamma(1-z)F(\alpha+1)-(\gamma-\alpha)F(\gamma+1) \\
(\beta-(\gamma-\alpha)z)F(\beta+1,\gamma+1) & =\gamma(1-z)F(\beta+1)-(\gamma-\beta)F(\gamma+1) \\
\alpha \gamma(1-z)F(\alpha+1) & =\gamma(\alpha-(\gamma-\beta)z)F+(\gamma-\alpha)(\gamma-\beta)zF(\gamma+1) \\
\beta \gamma(1-z)F(\beta+1) & =\gamma(\beta-(\gamma-\alpha)z)F+(\gamma-\alpha)(\gamma-\beta)zF(\gamma+1)
\end{align}$$

$$\tag{.}\begin{align}
\alpha F(\alpha+1,\gamma+1) & =\gamma F-(\gamma-\alpha)F(\gamma+1) \\
\beta F(\beta+1,\gamma+1) & =\gamma F-(\gamma-\beta)F(\gamma+1) \\
\alpha \beta(1-z)F(\alpha+1,\beta+1,\gamma+1) &=\gamma(\alpha+\beta-\gamma)F+(\gamma-\alpha)(\gamma-\beta)F(\gamma+1)
\end{align}$$
$$\tag{.}\begin{align}
\implies n \beta(\gamma-\alpha-\beta+1)z(1-z){}_{2}F_{1}(\alpha+1,\beta+1;\gamma+1;z) \\
+\gamma[(\alpha+\beta-\gamma)(\gamma+n-\beta+1)z-(\gamma+n)(\gamma-\beta)(1-z)]{}_{2}F_{1}(\alpha,\beta;\gamma;z) \\
+\gamma(\gamma-\alpha)(\gamma-\beta){}_{2}F_{1}(\alpha-1,\beta-1;\gamma;z) & =0
\end{align}$$