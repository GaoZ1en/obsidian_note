...
### Minimal Models分类
$(p,q)$ minimal models：
$$c = 1 - \frac{6(p-q)^2}{pq}, \quad \gcd(p,q)=1, \quad p > q \geq 2$$
...
### ADE分类
某些格点模型对应**ADE系列**的CFT，与Lie代数分类相关^cite{Cappelli1987}。

## 现代发展与开放问题

### [[Lieb-Schultz-Mattis定理]]
限制了1D量子自旋链的可能基态，影响对应CFT的性质。

### [[Bootstrap方法]]
利用**crossing symmetry**和**unitarity**约束，无需格点实现直接构造CFT^cite{Poland2019}。

### [[重要开放问题]]
1. **高维临界现象**：$d>2$时格点模型与CFT对应的一般理论
2. **量子临界点**：量子相变对应的$(1+1)$D CFT分类  
3. **拓扑序**：如何通过格点模型实现非平凡的拓扑CFT

### 计算工具
- **DMRG/TEBD**：数值计算中心荷和算符维数
- **Monte Carlo**：统计临界指数
- **精确对角化**：有限系统的能谱分析

这种格点-连续对应关系是现代理论物理的核心工具，连接了凝聚态物理、统计力学和量子场论，为理解强关联系统提供了重要途径。