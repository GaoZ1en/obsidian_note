...
where we define $\displaystyle{\pi _{a}=\delta _{ab}\dot{\xi}^{b}}$ as canonical momentum. in order to propose quantization, we need to consider Poisson brackets between two classical observables $\displaystyle{f(\xi ^{a}, \pi _{a})}$ and $\displaystyle{g(\xi ^{a},\pi _{a})}$. the Hamilton vector corresponding to $\displaystyle{f(\xi ^{a},\pi _{a})}$ is ...