...
$$
\phi\in\Omega^\bullet(\mathbb R^{d+1})\otimes L,
\qquad
\eta\in\Omega^\bullet(\mathbb R^{d+1})\otimes L^\vee[d].
$$
...
$$
\mathcal Z(C)=\operatorname{Pol}(C,d-2),
\qquad
d_{\mathcal Z}=d_{\mathrm{int}}+[\pi,-].
$$

Using Poisson additivity,

$$
\operatorname{Alg}_{P_d}
=\operatorname{Alg}\bigl(\operatorname{Alg}_{P_{d-1}}\bigr),
\tag{2.34}
$$
...
$$
[\pi,\widetilde\varphi(x)]=0,
\tag{2.37}
$$

$$
\widetilde\varphi(\{x,x'\}_A)
=[\widetilde\varphi(x),\widetilde\varphi(x')].
\tag{2.38}
$$
...
S_{\mathrm{enrich}}
=\int_{\mathbb R^d}
\sum_{n\geq0}\frac{1}{n!}
\Phi_{i,k_1\cdots k_n}(\theta)
\eta_i\chi_{k_1}\cdots\chi_{k_n}.
\tag{2.40}
$$
...
\sum_{k_1,\ldots,k_n}
\Phi_{i,k_1\cdots k_n}(\theta)
\prod_{a=1}^n\partial_{\theta_{k_a}}O_a.
\tag{3.35}
$$
...
$$
HH_{E_d}(A)\simeq\operatorname{Sym}_A(T_A[-d]),
\tag{3.39}
$$
...
$$
K_{E_k}(A)=\operatorname{Bar}^{(k)}(A)^\vee.
\tag{4.6}
$$
...
$$
\Phi=1+\frac{1}{24}[t_{12},t_{23}]+\cdots,
\tag{4.36}
$$
...
$$
\mathcal P\exp\left[
\sum_{n\geq0}\int\frac{1}{n!}
\partial_z^nA^a\,\rho_a[n]
\right].
\tag{6.30}
$$
...
$$
\Delta\rho_c[1]
=\rho_c[1]\otimes1+1\otimes\rho_c[1]
+\frac{\hbar}{2\pi}[\rho_c[0],t].
\tag{6.44}
$$
...
$$
P_{\mathcal E}(x,y)
=\int_0^\infty du\,Q_x^\dagger K_u(x,y).
\tag{7.19}
$$
