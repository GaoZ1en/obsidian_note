...
+\xi\left[G^{ab}+g^{ab}\Box-\nabla^a\nabla^b\right]\phi^2.
$$

The full second metric variation of the one-loop effective action is

$$
\mathcal S_2^{abcd}(x,y)
=i\langle T^{ab}(x)T^{cd}(y)\rangle_{\rm con}
+2\left\langle
\frac{\delta[\sqrt{-g}T^{ab}(x)]}{\delta g_{cd}(y)}
\right\rangle_\eta.
$$
...
$$
P^{(T)\,abcd}
=\frac12(\theta^{ac}\theta^{bd}+\theta^{ad}\theta^{bc})
-\frac13\theta^{ab}\theta^{cd}.
$$
...
$$
\mathrm T|_{\rm pole}
=-\frac1{16\pi^2\bar\epsilon}
\left[
\frac1{60}k^4
-k^2m^2\left(\xi-\frac16\right)
\right],
$$
...
=-\frac1{16\pi^2\bar\epsilon}
\left[
6k^4\left(\xi-\frac16\right)^2
+2k^2m^2\left(\xi-\frac16\right)
\right].
$$
...
$$
S_{\rm loc}
=\frac1{16\pi G}\int\sqrt{-g}(R-2\Lambda)
-\frac12\int\sqrt{-g}\left(\alpha C^2+\beta R^2\right).
$$
...
$$
B_R(k^2)
=\int_0^1dx\,\log\frac{M^2}{m^2}(M^2)^2
-\frac16k^2m^2-\frac1{20}k^4,
$$
...
$$
\Sigma_R=k^4F^{(S)}(k^2/m^2),
\qquad
\mathrm T_R=k^4F^{(T)}(k^2/m^2),
$$
...
$$
\alpha_R(k^2)=\alpha_R(0)+\frac12F^{(T)},
\qquad
\beta_R(k^2)=\beta_R(0)+\frac1{12}F^{(S)}.
$$
...
1-\frac{10m^2}{k^2}
+\frac{60m^4}{k^4}
\left(z\log\frac{z+1}{z-1}-2\right)
\right],
$$
...
$$
\frac12\eta_{ab}F_R^{abcd}(k)
=\frac1{16\pi^2}\frac1{180}
k^2(k^ck^d-k^2\eta^{cd}),
$$
...
$$
\rho^{(S)}(s)
=\frac{\Theta(s-4m^2)}{24\pi^2}
\sqrt{1-\frac{4m^2}{s}}
\left[\frac{(1-6\xi)s}{2}+m^2\right]^2,
$$

$$
\rho^{(T)}(s)
=\frac{\Theta(s-4m^2)}{60\pi^2}
\sqrt{1-\frac{4m^2}{s}}
\left(\frac s4-m^2\right)^2.
$$
...
=k^2\int_0^\infty
\frac{\rho^{(S,T)}(s)}
{s^3(s+k^2-i\epsilon)}\,ds.
$$

At conformal coupling,

$$
\sigma(s)=\frac{\rho^{(S)}(s)}{s^3}
=\frac{m^4}{24\pi^2s^3}
\sqrt{1-\frac{4m^2}{s}}\,
\Theta(s-4m^2).
$$
