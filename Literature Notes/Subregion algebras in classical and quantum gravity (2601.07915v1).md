...
2. In general relativity the explicit boundary data give a concrete corner charge. Using the Wald-Zoupas decomposition with $\gamma = 0$, the supertranslation corner charge becomes
   $$
...
3. The paper then tests half-sided supertranslations by truncating the symmetry at a cut $u=u_0$. The formal charge variation is written with a Heaviside function,
   $$
...
4. The resolution is to split the horizon across a thin Cauchy splitting region and introduce doubled corner edge modes
   $$
...
5. After adding the edge modes, the half-sided generators become honest canonical generators. The corner symplectic form yields
   $$
...
6. For perturbations around a Killing horizon, the gravitational constraints let the generators be rewritten in terms of horizon null energy. In the linearized theory one obtains
   $$
