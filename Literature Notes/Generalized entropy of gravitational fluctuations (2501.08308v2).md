...
$$
\begin{align}
\Delta S_B^{\mathrm{CFT}}
= \frac{\Delta\langle :A[\widehat g,\widehat\gamma]:\rangle}{4G_N}
...
\boxed{
\Delta K_b
=E_{\mathrm{can}}[\kappa h]
+\int_{\gamma^{(0)}}\Upsilon
}
\tag{4.36, 4.49}
\end{align}
$$
...
$$
\begin{align}
\delta L=E\,\delta g+d\theta
\Longrightarrow
\omega
\Longrightarrow
\delta H^\partial_\xi
=\delta H^\gamma_\xi+\Omega(\delta g,\mathcal L_\xi\delta g)
\Longrightarrow
K_B=\frac{A}{4G_N}+K_b.
\end{align}
$$
...
$$
\begin{align}
\text{raw fixed-surface area}
\quad\text{and}\quad
\text{raw canonical energy}
\end{align}
$$
...
\begin{align}
S_B^{\mathrm{CFT}}
=\frac{\langle:A[\widehat g,\widehat\gamma]:\rangle_\Psi}{4G_N}
+S(\rho_b^\Psi)
+O(\kappa).
\tag{3.15}
\end{align}
...
$$
\begin{align}
\mathbf L=\frac1{\kappa^2}(R-2\Lambda)\epsilon.
\tag{4.8}
\end{align}
$$
...
\delta\mathbf L
=\mathbf E^{\mu\nu}\delta g_{\mu\nu}
+d\boldsymbol\theta[g;\delta g],
\qquad
\boldsymbol\omega[g;\delta_1g,\delta_2g]
...
\begin{align}
\mathbf J_{(u)}
=\boldsymbol\theta[g;\mathcal L_u g]-\iota_u\mathbf L
=\mathbf C_{(u)}+d\mathbf Q_{(u)}.
\tag{4.19, 4.21}
\end{align}
$$
...
\begin{align}
\boxed{
\boldsymbol\omega[g;\dot g,\mathcal L_u g]
=\dot{\mathbf C}_{(u)}
+\iota_u\mathbf E^{\mu\nu}\dot g_{\mu\nu}
+d\left(
\dot{\mathbf Q}_{(u)}
-\iota_u\boldsymbol\theta[g;\dot g]
\right)
}
\tag{4.24}
\end{align}
...
\begin{align}
\delta K^\partial_{(\xi)}
=\frac{2\mathfrak s}{\kappa^2}\delta A.
\tag{4.27}
\end{align}
$$
...
$$
\begin{align}
\delta^2K^\partial_{(\xi)}
=\frac{2\mathfrak s}{\kappa^2}\delta^2A
+E_{\rm can}[\delta g],
\tag{4.31}
\end{align}
$$
...
E_{\rm can}[\delta g]
\equiv
\Omega[G;\delta g,\mathcal L_\xi\delta g].
\tag{4.32}
\end{align}
$$
...
=2\mathfrak s\left(
A^{\rm lin}[g^{(2)},\gamma^{(0)}]
+A^{\rm quad}[h^{\rm HW},\gamma^{(0)}]
\right)
+E_{\rm can}[\kappa h^{\rm HW}].
\tag{4.33}
\end{align}
$$
...
\text{canonical-energy corner term}.
\end{align}
$$
...
\begin{align}
\delta^2K^\partial_{(\xi)}
=2\mathfrak s A^{\rm lin}[g^{(2)},\gamma^{(0)}]
+\int_b d^dx\sqrt{-G_b}\,
T_{\mu\nu}^{\rm grav}\tau^\mu\xi^\nu.
\tag{4.39}
\end{align}
...
\omega\,b^\dagger_{\omega,\lambda}b_{\omega,\lambda}
+\mathrm{const.}
\tag{4.47}
\end{align}
$$
...
$$
\begin{align}
|\widetilde\epsilon\!\cdot\!\widetilde T\rangle,
\qquad
...
\begin{align}
|S\rangle
=\frac{|0\rangle+|\widetilde\epsilon\!\cdot\!\widetilde T\rangle}{\sqrt2},
\end{align}
$$
...
\begin{align}
\Delta K_B^S
=\frac12\Delta K_B^{\widetilde\epsilon\cdot\widetilde T}
+O(\sqrt{C_T})\sin^d\theta_0.
\tag{5.22}
\end{align}
$$
...
$$
\begin{align}
S_{\rm rel}(\rho_B^{\widetilde\epsilon\cdot\widetilde T}|\omega_B)
=
\frac{\sqrt\pi\,\Gamma(2d+1)}
{2\Gamma\left(2d+\frac32\right)}
\theta_0^{4d}
\left(1+O(\theta_0^2)\right).
\tag{5.27}
\end{align}
$$
...
\sqrt{
\frac{\Gamma(d+2)}
{4d(d-1)\Gamma(\frac d2+1)\Gamma(\frac d2+2)V_{S^{d-1}}}
}.
\tag{5.71}
\end{align}
$$
...
$$
\begin{align}
|g\rangle=a^{S\dagger}_{0,2,0}|0\rangle.
\tag{5.72}
\end{align}
$$
...
\begin{align}
E^{(1)}_{\mu\nu}
\left[\langle\widehat g^{(2)}\rangle_g\right]
...
$$
\begin{align}
\left(\nabla^2_{H^{d-1}}-(d-1)\right)\varpi_m
+(-1)^m\delta_{\kappa h}K_{(m)}=0.
\tag{5.103}
\end{align}
$$
...
\begin{align}
\widehat K_b^0
=:\widehat E_{\rm can}[\widehat h^{\rm HW}]:
=\sum_q
\left(
E_{\rm can}[h_q]
+\int_{\gamma^{(0)}}\Upsilon_q
\right)a_q^\dagger a_q.
\tag{5.112}
\end{align}
$$
...
\Delta K_B
=\frac{2\mathfrak s}{\kappa^2}
\Delta\langle A^{\rm lin}[\widehat g^{(2)},\gamma^{(0)}]\rangle
+\int_b\sqrt{-G_b}\,
\Delta\langle:T_{\mu\nu}^{\rm grav}:\rangle\tau^\mu\xi^\nu.
\tag{5.119, with explicit area normalization}
\end{align}
$$
...
W_{\mu\nu\alpha\beta}^{(1)}
\text{ is perturbatively gauge invariant}.
\end{align}
$$
...
\text{arbitrary gauge}
\Longrightarrow
K_b=E_{\rm can}+\int_\gamma\Upsilon,
\end{align}
$$

$$
\begin{align}
K_B=\frac{A[\widehat g,\widehat\gamma]}{4G_N}+K_b
\Longrightarrow
\text{graviton JLMS/generalized entropy}.
\end{align}
$$
...
\begin{align}
\int_{-\infty}^{\infty}ds\,
\operatorname{sech}^{4d+2}\left(\frac s2\right)
=
\frac{2\sqrt\pi\,\Gamma(2d+1)}
{\Gamma(2d+\frac32)},
\end{align}
$$
...
$$
\begin{align}
A^{\rm lin}[h_{0,2,0},\gamma^{(0)}]
=V_{S^2}|\mathcal N^S_{0,2}|\frac45\sin^4\theta_0.
\end{align}
$$
...
$$
\begin{align}
\frac{\text{CFT cross term}}
{\langle s|A^{\rm lin}|s\rangle/\kappa}
=12\sqrt2\,\pi,
\end{align}
$$
